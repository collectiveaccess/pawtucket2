		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit()) {
				...
				if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result')){
					...
					$vs_idno_detail_link 	= caDetailLink($this->request, $qr_res->get("{$vs_table}.type_id",array('convertCodesToDisplayText' => true)), '', $vs_table, $vn_id);
					...
					if ($vs_table == 'ca_objects') {
						...
						if ($qr_res->get('ca_objects.date.date_value')) {
							$vs_date = "<p>".$qr_res->getWithTemplate('<unit delimiter="<br/>"><ifdef code="ca_objects.date.date_value">^ca_objects.date.date_value (^ca_objects.date.date_types)</ifdef></unit>')."</p>";
						}
