...
function printLevel($po_request, $va_collection_ids, $o_config, $vn_level, $va_options = array()) {
	...
	if($qr_collections->numHits()){
		while($qr_collections->nextHit()) {
			$vs_icon = "";
			# --- related objects?
			$va_rel_object_ids = $qr_collections->get("ca_objects.object_id", array("returnAsArray" => true, 'checkAccess' => $va_access_values));
			$vn_rel_object_count = sizeof($va_rel_object_ids);
			...
			if($vn_level == 1){
				...
				$vs_output .= "<div class='collectionTitle'>";
			}
			if($vb_link){
				$vs_output .= $vs_icon." ";
				if($vb_collapse_link){
				...
				if($vb_collapse_link){
					...
					$vs_output .= $qr_collections->get("ca_collections.preferred_labels")." <small>(no preview available)</small>";
				}
				$vs_output .= "</span>";
			}
			if($vn_rel_object_count){
				$vs_output .= " <small>(See all ".$vn_rel_object_count." record".(($vn_rel_object_count == 1) ? "" : "s").")</small>";
			} 
			/*if($vn_rel_object_count){
				$vn_i = 0;
				$vs_output.= "<div class='row first'>";
				foreach ($va_rel_object_ids as $va_key => $va_rel_object_id ) {
					$t_rel_item = new ca_objects($va_rel_object_id);
					$vs_output.= "<div class='col-sm-3 collectionPreview'>".$t_rel_item->get('ca_object_representations.media.iconlarge')."<br>".$t_rel_item->get('ca_objects.preferred_labels', array('returnAsLink' => true))."</div>";
					$vn_i++;
					if ($vn_i == 4) {
						$vs_output.= "</div><div class='row'>";
					}
					if ($vn_i == 8) {
						break;
					}
