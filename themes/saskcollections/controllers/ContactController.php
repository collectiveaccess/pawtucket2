 		...
 		public function Send() {
 			...
 			if(sizeof($va_errors) == 0){
 				# --- send email
					...
					# --- try to send email to the member institution's contact, fall back to the site admin configured in contact.conf contact_email
					$va_emails = $this->config->get("contact_email");
					...
					if($ps_table && $pn_id){
						if($ps_table == "ca_objects"){
							...
							$t_item->load($pn_id);
							# --- get the item's contributing member institution
							if($vn_source_id = $t_item->get("ca_objects.source_id")){
								$vn_entity_id = getEntityIdForSource($this->request, $vn_source_id);
								$t_entity = Datamodel::getInstanceByTableName("ca_entities");
								$t_entity->load($vn_entity_id);
								$vs_member_institution_email = $t_entity->get("ca_entities.email");
								if($vs_member_institution_email){
									$va_emails = explode(";", $vs_member_institution_email);
								}
							}					
						}
					}
					if(caSendmail($va_emails, $this->request->config->get("ca_admin_email"), $vs_subject_line, $vs_mail_message_text, $vs_mail_message_html)){
