...
<?php
			while($qr_res->nextHit()){
				...
				print "<li class='col-md-3'>".caNavLink($this->request, $qr_res->getWithTemplate($vs_thumbnail."<div class='pb-5'><h3 class='fs-4 fw-semibold mb-0 pt-2 text-start'>^ca_collections.preferred_labels.name <span class='fw-normal'>(<unit relativeTo='ca_objects' limit='1'>^count</unit>)</span></h3><ifdef code='ca_collections.short_des'><div class='fs-5 fw-light blurb'>^ca_collections.short_des</ifdef></div>", array("checkAccess" => $va_access_values)), "text-decoration-none", "", "Browse", "Objects", array("facet" => "collection_facet", "id" => $qr_res->get("ca_collections.collection_id")))."</li>";
			}
?>
			</ul>
		</div>
