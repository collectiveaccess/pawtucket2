...
<div class="wrap browse-results-wrap">
	...
	<div class="container browse-controls-container">
		...
		<div class="row justify-content-center browse-page-label">
			<h1>Browse <?php print ucwords($va_browse_info["labelPlural"]);?></h1>
		</div>

		<div class="row" style="margin: 10px 5px;">
			<div class="col text-center">
				<?= caFormTag($this->request, 'objects', 'basicSearch', 'Search', 'post', 'multipart/form-data', null, []); ?>
					<input class="browse-search-input" type="text" name="search" placeholder="Search Collections by keyword..." aria-label="Search collections">
					<button type="submit" title="Submit Search" aria-label="Submit Search" style="background-color: transparent; border: none">
		...
		<div class="row browse-control-items">
			<div class='col'>
				<div class="browse-result-amount">
			...
			<div class="col justify-content-end">
				<div class="browse-view-mode">View Mode: </div>
				<?php
					if(is_array($va_views) && (sizeof($va_views) > 1)){
						foreach($va_views as $vs_view => $va_view_info) {
							if ($vs_current_view === $vs_view) {
								print '<a href="#" class="browse-view-button active">
										<i class="bi bi-grid-3x3-gap-fill" '.$va_view_icons[$vs_view]['icon'].'" aria-label="'.$vs_view.'" role="button"></i>
									</a> ';
							} else {
								print caNavLink($this->request, '<i class="bi bi-list-task" '.$va_view_icons[$vs_view]['icon'].' 
									aria-label=" '.$vs_view.' " role="button"></i>', 'browse-view-button disabled', '*', '*', '*', array('view' => $vs_view, 'key' => $vs_browse_key)).' ';
							}
						}
					}
				?>
		...
		<div class="row browse-criteria">
			<?php
				if (sizeof($va_criteria) > 0) {
					...
					foreach($va_criteria as $va_criterion) {
						...
						$vs_criteria .= caNavLink($this->request, '
							<button type="button" class="btn btn-default btn-sm">
								<span class="filter-val">'.$va_criterion['value'].' <span style="font-size: 10px; color: #767676;">'.($va_criterion['facet'] ? '('.$va_criterion['facet'].')' : '').'</span></span>
					...
					if($vb_start_over){
						$vs_criteria .= caNavLink($this->request, '<button type="button" class="btn btn-default btn-sm reset-btn">'._t("Reset").'</button>', 'browseRemoveFacet', '', 'Browse', '*', array('view' => $vs_current_view, 'key' => $vs_browse_key, 'clear' => 1, '_advanced' => $vn_is_advanced ? 1 : 0));
					}
				}
	...
	<div class="browse-results-grid">
		<div class='browse-results-grid-item'>
			<!-- If in list view -->
			<?php 
				...
				<?php
					...
					switch (strval($vs_current_view)) {
						case "images":
							...
							break;
						case "list":
							print  "<div class='browseResults row row-cols-1 row-cols-xs-1 row-cols-sm-1 row-cols-md-2'> $vs_result_page </div>" ;
							break;
						default:
							print  "<div class='browseResults row row-cols-1 row-cols-xs-1 row-cols-sm-1 row-cols-md-2 row-cols-lg-3 row-cols-xl-4'> $vs_result_page </div>" ;
					}	
