...
<?php 
		...
		if(sizeof($va_criteria) > 1){
			print caNavLink($this->request, '<div class="clearAll"><button type="button" class="btn btn-default btn-sm">'._t("Clear All Filters").'</button>', '', '*', '*', '*', array('view' => $vs_current_view, 'key' => $vs_browse_key, 'clear' => 1, '_advanced' => $vn_is_advanced ? 1 : 0))."</div>";
		}
?>
		<H1>
<?php
...
<?php
					...
					if (sizeof($va_criteria) > ($vb_is_search ? 1 : 0)) {
						print "<li role='menuitem'>".caNavLink($this->request, _t("Clear All Filters"), '', '*', '*', '*', array('view' => $vs_current_view, 'key' => $vs_browse_key, 'clear' => 1, '_advanced' => $vn_is_advanced ? 1 : 0))."</li>";
					}
