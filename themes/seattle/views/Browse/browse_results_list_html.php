...
// 				}else{
					...
					if (strlen($vs_label_detail_link) >215){
						
					$vs_label_detail_link 	= substr(caDetailLink($this->request, $qr_res->get("{$vs_table}.preferred_labels"), '', $vs_table, $vn_id),0,215)."...";
					}
					$vs_thumbnail = "";
					$vs_type_placeholder = "";
					$vs_typecode = "";
					...
					if ($qr_res->get('ca_objects.date.dates_value') && $qr_res->get("type_id")!=116 && $qr_res->get("type_id")!=2302 && $qr_res->get("type_id")!=24) {
						$vs_date = "<p>".$qr_res->get('ca_objects.date.dates_value')."</p>";
					}
					elseif ($qr_res->get('ca_objects.date.dates_value') && $qr_res->get("type_id")==24) {
						$vs_date = "<p>".$qr_res->get('ca_objects.date.dates_value')."</p>";
						if (strpos($qr_res->get('ca_objects.date.dates_value'), ';') !== false){
							$pieces = explode(";", $qr_res->get('ca_objects.date.dates_value'));
							$vs_date = "<p>".$pieces[1]."</p>";
						}
						else {$vs_date = "<p>".$qr_res->get('ca_objects.date.dates_value')."</p>";}
						
					}
					
					
					elseif ($qr_res->get('ca_objects.date.dates_value') && $qr_res->get("type_id")==25 or $qr_res->get('ca_objects.date.dates_value') && $qr_res->get("type_id")==23) {
						$vs_date = "<p>".$qr_res->get('ca_objects.date.dates_value')."</p>";
					}
					else {$vs_date = "";}
					if ($qr_res->get('ca_objects.body_name') ) {
						$vs_date = "<br>".$qr_res->get('ca_objects.body_name.preferred_labels').$vs_date;
					}
