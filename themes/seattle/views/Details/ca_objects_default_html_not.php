...
{{{<ifcount code="ca_collections" restricttotypes="series" min="1">					<H6>Series:</H6>
			<span id="seriesspan">	<H4><unit relativeTo="ca_collections" delimiter="<br/>" restricttotypes="series"><l>^ca_collections.idno: <span id="series_identifier">^ca_collections.preferred_labels.name</span></l></unit><ifcount min="1" code="ca_collections">  </ifcount></ifcount>}}}</H4></span>
...
{{{<ifdef code="ca_objects.mi_nature">
	...
	{{{<ifdef code="ca_objects.di_sound">
				...
				{{{<ifdef code="ca_objects.location.folder_ti">
					<H6>Folder Title:</H6><span class="trimText">^ca_objects.location.folder_ti<ifdef code="ca_objects.location.folder_date">, ^ca_objects.location.folder_date</span><span id="link_finding_aid" ><b><i style="color:rgb(25,190,209)"><br><a href="^ca_objects.location.link_finding_aid" style="color:rgb(25,190,209)">Go to folder description in finding aid</a></i></b></span>
...
for(var i=0; i < clerk.length; i++) {
	...
	li.href = "http://archives.seattle.gov/digital-collections/index.php/MultiSearch/Index?search=ca_objects.clerk_subject_terms%3A"+$.trim(clerk[i]).replace(/ /g, "_") ;
    list.appendChild(li);
	list.appendChild(br);
...
{{{<ifcount code="ca_entities.related" restrictToRelationshipTypes="subject" min="1"><H6>Related People/Organizations:</H6><unit relativeTo="ca_entities" restrictToRelationshipTypes="subject" sort="ca_entities" sortDirection="ASC" delimiter="<br/>"><a href="<?php print __CA_URL_ROOT__?>/index.php/Detail/entities/^ca_entities.entity_id">^ca_entities.preferred_labels.displayname </a>
<ifdef code="ca_entities.biography.position_title" >(^ca_entities.biography.position_title)</ifdef></unit></ifcount>}}}


{{{<ifdef code="ca_objects.agenda_minutes"> <span id="agenda" style="display:none">^ca_objects.agenda_minutes</span><H6>Agenda/Minutes:</H6><a id="agendalink" href='#' target="_blank">Click here</a></ifdef>}}}
<script>
$('#agendalink').attr('href',decodeURI(document.getElementById('agenda').innerText).replace('&amp;','&'));

</script>
{{{<ifdef code="ca_objects.audio_format"> <H6>Audio File(s):</H6><ifdef code="ca_objects.idno">^ca_objects.idno</ifdef><if rule="^ca_objects.audio_format =~ /Wave/">.wav</if><if rule="^ca_objects.audio_format =~ /MPEG3/">.mp3</if><if rule="^ca_objects.audio_format =~ /MPEG/">.mp3</if>: <span id="originalformattype">^ca_objects.audio_format</span> File (Original) <?php
...
{{{<ifdef code="ca_objects.physical_carrier"> <H6>Physical Format(s):</H6> <unit relativeTo="ca_objects.physical_carrier" delimiter="<br>" sort="ca_objects.physical_carrier.carrier_id">Item No. ^ca_objects.physical_carrier.carrier_id, ^ca_objects.physical_carrier.media_Type<ifdef code="ca_objects.physical_carrier.generation_physical"> (^ca_objects.physical_carrier.generation_physical)</ifdef></unit>
</ifdef>}}}
...
<?php			
				...
				$id= $t_object->getPrimaryKey();
				if ($t_object->get('ca_objects.type_id') != '24'){
...
?>
<a href='#' id="versiontodownload"><i class='caIcon fa fa-download fa-1x'></i></a>
<script>
//document.write("hello");
checkdownload();
...
var $path = __CA_URL_ROOT__.'digital-collections/index.php/Detail/DownloadRepresentation/representation_id/' + $rep.toString() + '/object_id/' + $id.toString() + '/download/1/version/' + $downloadversion;
