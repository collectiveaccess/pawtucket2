		...
		?>
			...
			if ($va_related_representations = $t_object->get('ca_object_representations', array('returnWithStructure' => true))) { 
				...
				$object_rep = new ca_object_representations($representation_id);
					...
					if ($t_object->get('ca_objects.type_id') == '9673' || $t_object->get('ca_objects.type_id') == '26'){
						print("<span id=\"VideoDownload\" style=\"display:none;position:fixed;top:400px;\"><a href=\"#\" onclick=\"document.getElementById('Downloadtips').style.display = 'block';\">Trouble downloading?</a><br></span><br><span id=\"Downloadtips\" style=\"font-size:90%;display:none;position:fixed;top:450px;\"><ul><li>Wait a few seconds between hitting the download button and saving.</li><li>Refresh your browser.</li><li>Close and reopen your browser and try again.</li><li>Contact <a href=\"mailto:archives@seattle.gov\">archives@seattle.gov</a></li></ul></span>");
						print("<span id='VideoCheck' style='display:none'>Yes</span>");
					}
			}}
				...
				<script type="text/javascript">
				var vid = document.getElementById('VideoCheck').innerHTML;
...
{{{<ifdef code="ca_objects.language">
	...
	{{{<ifdef code="ca_objects.RL_location"><H6>Location:</H6> ^ca_objects.RL_location</ifdef>		}}}
...
{{{<ifcount code="ca_collections" restricttotypes="subseries" min="1">					<H6>Subseries:</H6>
			<span id="seriesspan">	<unit relativeTo="ca_collections" delimiter="<br/>" restricttotypes="subseries"><l><span id="series_idno">^ca_collections.idno</span>: <span id="series_identifier">^ca_collections.preferred_labels.name</span></l></unit><ifcount min="1" code="ca_collections">  </ifcount></ifcount>}}}</span>

{{{<ifcount code="ca_collections" restricttotypes="libraries" min="1">					<H6>Collection:</H6>
			<span id="seriesspan">	<unit relativeTo="ca_collections" delimiter="<br/>" restricttotypes="libraries"><l><span id="series_idno">^ca_collections.idno</span>: <span id="series_identifier">^ca_collections.preferred_labels.name</span></l></unit><ifcount min="1" code="ca_collections">  </ifcount></ifcount>}}}</span>
...
{{{<ifcount code="ca_objects.subjects_research"  min="1"><H6>Subjects:</H6><unit relativeTo="ca_objects.subjects_research" sort="ca_objects.subjects_research" sortDirection="ASC" delimiter="<br/>"><a href="<?php print __CA_URL_ROOT__?>/index.php/MultiSearch/Index?search=ca_objects.subjects_research:^ca_objects.subjects_research">^ca_objects.subjects_research</a>
</unit></ifcount>}}}			
...
<span id="originalformattypefirst" style="display:none">{{{<unit relativeTo="ca_objects.audio_file" delimiter="<br>">Item No. ^ca_objects.audio_file.file_identifier, <span id="originalformattype">^ca_objects.audio_file.file_format</span> <ifdef code="ca_objects.audio_file.generation"> (^ca_objects.audio_file.generation)</ifdef</unit>
</ifdef>}}}</span>					
...
var $format =document.getElementById('originalformattypefirst').innerHTML;

if ($format.includes('Wave') || $format.includes('WAVE')){
document.getElementById('versiondownload').append(new Option('wave', 'original'));	
}
...
var $downloadversion = document.getElementById("versiondownload").options[document.getElementById("versiondownload").selectedIndex].value;
...
{{{<ifcount code="ca_entities.related" restrictToRelationshipTypes="sponsor" min="1"><H6>Sponsor:</H6><unit relativeTo="ca_entities" restrictToRelationshipTypes="sponsor"  sort="ca_entities" sortDirection="ASC" delimiter="<br/>"><a href="<?php print __CA_URL_ROOT__?>/index.php/Detail/entities/^ca_entities.entity_id">^ca_entities.preferred_labels.displayname </a>
<ifdef code="ca_entities.biography.position_title" >(^ca_entities.biography.position_title)</ifdef></unit></ifcount>}}}		
...
{{{<ifcount code="ca_entities.related" restrictToRelationshipTypes="performer" min="1"><H6>Performer(s):</H6><unit relativeTo="ca_entities" restrictToRelationshipTypes="performer" sort="ca_entities" sortDirection="ASC" delimiter="<br/>"><a href="<?php print __CA_URL_ROOT__?>/index.php/Detail/entities/^ca_entities.entity_id">^ca_entities.preferred_labels.displayname </a>
<ifdef code="ca_entities.biography.position_title" >(^ca_entities.biography.position_title)</ifdef></unit></ifcount>}}}
