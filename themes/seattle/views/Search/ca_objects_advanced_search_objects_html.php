...
<div class='row'>
	...
	<script type="text/javascript">
		...
		$.ajax({
			type: "GET",
			url: "http://archives.seattle.gov/digital-collections//media/series_dropdown2.xml",
			dataType: "xml",
		...
		$.ajax({
			type: "GET",
			url: "http://archives.seattle.gov/digital-collections//media/image_dropdown.xml",
			dataType: "xml",
			success: function (result) {
				$(result).find("p").each(function () {
					if ($(this).attr("id") == localStorage.getItem("collection_name") && includes(document.referrer, 'Detail/collections')){
		 ...
		 $.ajax({
			type: "GET",
			url: "http://archives.seattle.gov/digital-collections//media/image_series_dropdown2.xml",
			dataType: "xml",
			success: function (result) {
				$(result).find("p").each(function () {
		...
		$.ajax({
			type: "GET",
			url: "http://archives.seattle.gov/digital-collections//media/movingimage_series_dropdown2.xml",
			dataType: "xml",
			success: function (result) {
				$(result).find("p").each(function () {
		...
		$.ajax({
			type: "GET",
			url: "http://archives.seattle.gov/digital-collections//media/audio_series_dropdown.xml",
			dataType: "xml",
			success: function (result) {
				$(result).find("p").each(function () {
		...
		$.ajax({
			type: "GET",
			url: "http://archives.seattle.gov/digital-collections//media/drawing_series_dropdown.xml",
			dataType: "xml",
			success: function (result) {
				$(result).find("p").each(function () {
		...
		document.getElementById('subjectterms').value = localStorage.getItem("subjects"); 	
</script>
	...
	<div class='row'>
		<div class="advancedSearchField col-sm-12" style="display:none;">
			...
			<select id="_fulltext[]" name="_fulltext[]">
				<option value="">-</option>
				<option value="Cowen Park"> Cowen Park</option>
				<option value="Denny Park"> Denny Park</option>
				<option value="Discovery Park"> Discovery Park</option>
				<option value="Fort Lawton"> Fort Lawton</option>
				<option value="Frink Park"> Frink Park</option>
				<option value="Green Lake Park"> Green Lake Park</option>
				<option value="Hamilton Viewpoint"> Hamilton Viewpoint</option>
				<option value="Interlaken Park"> Interlaken Park</option>
				<option value="Kinnear Park"> Kinnear Park</option>
				<option value="Jefferson Park"> Jefferson Park</option>
				<option value="Leschi Park"> Leschi Park</option>
				<option value="Lincoln Park"> Lincoln Park</option>
				<option value="Madison Park"> Madison Park</option>
				<option value="Madrona Park"> Madrona Park</option>
				<option value="Mt. Baker Park"> Mt. Baker Park</option>
				<option value="Schmitz Park"> Schmitz Park</option>
				<option value="Seward Park"> Seward Park</option>
				<option value="Volunteer Park"> Volunteer Park</option>
		...
		<span id="imageseriesreal" style="display:block" >
		...
		<span id="imageseries" style="display:block">
...
$('#adding').append('<span><input name="ca_objects.date_lc.date_value[]" id="ca_objects.date_lc.date_value[]" value="1973" maxlength="255" class="dateBg" rows="1" style="width: 220px;" size="" type="text"></span>');
	...
	<span class="formLabel" data-toggle="popover" data-trigger="hover" data-content="Geographic">Geographical Area/Neighborhoods</span>
			...
			<select id="geographic3" name="geographic[]" onClick="checkgeographic();" onChange="checkgeographic();">
				...
				<option value='"ARBOR_HEIGHTS"'>ARBOR-HEIGHTS</option>
				...
				<option value='"CENTRAL_AREA"'>CENTRAL-AREA</option>
				<option value="CENTRAL-BUSINESS-DISTRICT">CENTRAL-BUSINESS-DISTRICT </option>
				<option value="CENTRAL-WATERFRONT">CENTRAL-WATERFRONT </option>
				...
				<option value="DENNY-BLAINE">DENNY-BLAINE </option>
				<option value="DENNY-REGRADE">DENNY-REGRADE </option>
				...
				<option value="PORTAGE-BAY-NEIGHBORHOOD">PORTAGE-BAY-NEIGHBORHOOD </option>
				...
				<option value="SEWARD-PARK-NEIGHBORHOOD">SEWARD-PARK-NEIGHBORHOOD  </option>
			...
			<span style="display:none">{{{geographic}}}</span>

		</div>
	</span>
	<div>
		<span id="TextSearch" style="display:none" 
			><h2>Textual Record only fields:</h2>
			...
			<span class='formLabel' data-toggle="popover" data-trigger="hover" data-content="Search Box">Box</span>
						...
						<div class="advancedSearchField">
						<span class='formLabel' data-toggle="popover" data-trigger="hover" data-content="Search Folder">Folder Number</span>
...
<script type="text/javascript">
	...
	function consistentseries(dropdownid){
		if(document.getElementById(dropdownid).value != ""){
			...
			if (dropdownid != 'ca_collections_idno5'){
				for (i = 0; i < document.getElementById('ca_collections_idno5').length; ++i){
					if (document.getElementById('ca_collections_idno5').options[i].value == document.getElementById(dropdownid).value){
						hit = document.getElementById(dropdownid).value;
				}}
				if (hit.length > 0){
						document.getElementById('ca_collections_idno5').value = hit;
						}	
				else {
