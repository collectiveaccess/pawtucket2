...
<?php
					...
					if($qr_collection_children->numHits()){
						while($qr_collection_children->nextHit()) {
							...
							$vs_idno = "";
							if($qr_collection_children->get("type_id") == $vs_series_type_id){
								$vs_idno = mb_substr($qr_collection_children->get("idno"), -2, 2);
							}
							if($qr_collection_children->get("type_id") == $vs_file_type_id){
								$vs_idno = mb_substr($qr_collection_children->get("idno"), -6, 6);
							}
							...
							if($vb_link_sublist){
								...
								if($vb_link_to_detail){
									print caDetailLink($this->request, $vs_icon." ".$vs_idno." - ".$qr_collection_children->get('ca_collections.preferred_labels').(($qr_collection_children->get("ca_collections.collection_date2.collection_date_inclusive")) ? ", ".$qr_collection_children->get("ca_collections.collection_date2.collection_date_inclusive") : "")." ".(($o_collections_config->get("link_out_icon")) ? $o_collections_config->get("link_out_icon") : ""), '', 'ca_collections',  $qr_collection_children->get("ca_collections.collection_id"));
								}else{
