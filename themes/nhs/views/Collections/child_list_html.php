...
function printLevel($po_request, $va_collection_ids, $o_config, $vn_level, $va_options = array()) {
	...
	if($qr_collections->numHits()){
		while($qr_collections->nextHit()) {
			...
			$vs_item_label = $vs_idno." - ".$qr_collections->get('ca_collections.preferred_labels');
			if($vs_tmp = $qr_collections->get('ca_collections.collection_date2.collection_date_inclusive')){
				$vs_item_label .= ", ".$vs_tmp;
			}
			if($vb_link){
				$vs_output .= $vs_icon." ";
				if($vb_collapse_link){
					$vs_output .= "<a href='#' onClick='jQuery(\"#level".$qr_collections->get('ca_collections.collection_id')."\").toggle(); return false;'>".$vs_item_label."</a>";
				}else{
					$vs_output .= caDetailLink($po_request, $vs_item_label, '', 'ca_collections',  $qr_collections->get("ca_collections.collection_id"));
				}
				...
				if($vb_collapse_link){
					...
					$vs_output .= $vs_item_label;
				}
				$vs_output .= "</span>";
			}
			if($vn_rel_object_count){
				$vs_output .= " <small>(digitized items available)</small>";
			}
			if($vn_level == 1){
				$vs_output .= "</div>";
			}
