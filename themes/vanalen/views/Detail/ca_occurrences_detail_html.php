 ...
 */
	...
	# --- get parent occ
	if($t_occ->load($t_occurrence->get("parent_id"))){
		$vs_parent_occ = $t_occ->getLabelForDisplay();
		$vn_grandparent_id = $t_occ->get("parent_id");
		$vs_parent_occ_label = $t_occ->getTypeName();
	}
	# --- get grandparent occ
	if($vn_grandparent_id && $t_occ->load($vn_grandparent_id)){
		$vs_grandparent_occ = $t_occ->getLabelForDisplay();
		$vn_competition_series_id = $t_occ->get("occurrence_id");
		$vs_grandparent_occ_label = $t_occ->getTypeName();
	}	
...
<?php
			if ($this->getVar('previous_id')) {
				print "<div id='previous'>".caNavLink($this->request, "<img src='".$this->request->getThemeUrlPath()."/graphics/arr_black_l.gif' width='7' height='7' border='0'> "._t("Previous"), '', 'Detail', 'Object', 'Show', array('object_id' => $this->getVar('previous_id')), array('id' => 'previous'))."</div>";
			}
			if ($this->getVar('next_id') > 0) {
				print "<div id='next'>".caNavLink($this->request, _t("Next")." <img src='".$this->request->getThemeUrlPath()."/graphics/arr_black_r.gif' width='7' height='7' border='0'>", '', 'Detail', 'Object', 'Show', array('object_id' => $this->getVar('next_id')), array('id' => 'next'))."</div>";
			}
			print ResultContext::getResultsLinkForLastFind($this->request, 'ca_objects', _t("Back to Results"), '');

?>
		</div><!-- end nav -->
		<h1><?php print unicode_ucfirst($t_occurrence->getTypeName()).': '.$vs_title; ?></h1><br/>
		<div id="leftCol">	
<?php
			# --- identifier
			if($this->getVar('idno')){
				print "<div class='unit'><span class='heading'>"._t("Program number")."</span>: ".$this->getVar('idno')."</div><!-- end unit -->";
			}
			# --- competition date
			if($this->getVar('competition_date')){
				print "<div class='unit'><span class='heading'>"._t("Competition Date")."</span>: ".$this->getVar('competition_date')."</div><!-- end unit -->";
			}
			
			# --- parent occ 
			if($vs_parent_occ){
				print "<div class='unit'><span class='heading'>".$vs_parent_occ_label."</span>: ".$vs_parent_occ."</div><!-- end unit -->";
			}
			
			# --- grandparent occ
			if($vs_grandparent_occ){
				print "<div class='unit'><span class='heading'>".$vs_grandparent_occ_label."</span>: ".$vs_grandparent_occ."</div><!-- end unit -->";
			}

			# --- description
			if($this->getVar('description')){
				print "<div class='unit'><div id='description'>".$this->getVar('description')."</div></div><!-- end unit -->";				
?>
...
?>
					<div><?php print (($this->request->config->get('allow_detail_for_ca_entities')) ? caNavLink($this->request, $va_entity["label"], '', 'Detail', 'Entity', 'Show', array('entity_id' => $va_entity["entity_id"])) : $va_entity["label"])." (".unicode_ucfirst($va_entity_rel_types[$va_entity['relationship_type_id']]['typename_reverse']).")"; ?></div>
<?php					
				}
?>
				</div><!-- end unit -->
