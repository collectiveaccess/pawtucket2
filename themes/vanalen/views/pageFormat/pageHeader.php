...
?>
	...
	<!-- code for browse menus -->
	<script src="<?php print $this->request->getThemeUrlPath(true); ?>/SpryAssets/SpryMenuBar.js" type="text/javascript"></script>
	<script src="<?php print $this->request->getThemeUrlPath(true); ?>/SpryAssets/SpryCollapsiblePanel.js" type="text/javascript"></script>
	
	
	<link href="<?php print $this->request->getThemeUrlPath(true); ?>/SpryAssets/SpryMenuBarHorizontal.css" rel="stylesheet" type="text/css" />
	<link href="<?php print $this->request->getThemeUrlPath(true); ?>/SpryAssets/SpryMenuBarVertical.css" rel="stylesheet" type="text/css" />
	<link href="<?php print $this->request->getThemeUrlPath(true); ?>/SpryAssets/SpryCollapsiblePanel.css" rel="stylesheet" type="text/css" />
	<!-- end browse menu code -->
</head>
<body>
<?php
...
?>
		...
		<div id="pageArea">
			<div id="header">
<?php
				print caNavLink($this->request, '<img src="'.$this->request->getThemeUrlPath().'/graphics/title.gif" width="360" height="23" border="0" />', "", "", "", "")."<br/>";
				print caNavLink($this->request, _t("A Digital Collection of Architecture and Design Competitions, 1894-Present"), "subtitle", "", "", "");
?>				
			</div><!-- end header -->
<?php
...
?>
			<div id="nav">
				<div id="search"><form name="header_search" action="<?php print caNavUrl($this->request, '', 'Search', 'Index'); ?>" method="get">
						<input type="text" name="search" value="<?php print ($vs_search) ? $vs_search : ''; ?>" onclick='jQuery("#quickSearch").select();' id="quickSearch"  autocomplete="off" size="100"/><a href="#" name="searchButtonSubmit" onclick="document.forms.header_search.submit(); return false;">Search</a>
				</form></div>
<?php
				print caNavLink($this->request, _t("Home"), (($this->request->getController() == "Splash") ? "currentSection" : ""), "", "", "");
				print caNavLink($this->request, _t("Browse the Archive"), (($this->request->getController() == "Browse") ? "currentSection" : ""), "", "Browse", "clearCriteria");
				print caNavLink($this->request, _t("About"), (($this->request->getController() == "About") ? "currentSection" : ""), "", "About", "index");
?>
			</div><!-- end nav -->
<?php
}
