...
$result_desc = $context->getResultDesc();
$table = $qr_results->tableName();
...
if ($qr_results->numHits() > 0) {
	...
	while($qr_results->nextHit()) {
		$id = $qr_results->getPrimaryKey();
		
		print "<dd class='col-12 col-sm-6 col-md-4 col-lg-3 mb-3 text-center'>";
		
		//$transcript_hit = caGetResultDescWordList($table, $id, $result_desc, ['table' => 'ca_object_representation_captions', 'first' => 1]);
		//print $qr_results->getWithTemplate($vs_caption_template, array('extraParameters' => $transcript_hit['params'] ?? null, "checkAccess" => $va_access_values));
