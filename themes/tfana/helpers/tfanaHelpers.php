 ...
 * 
 */	
function caGetCollectionLevelSummaryTFANA($po_request, $va_collection_ids, $vn_level) {
	...
	if($qr_collections->numHits()){
		while($qr_collections->nextHit()) {
			...
			# --- related objects?
			# --- don't check access, they want all objects all to show
			#$va_object_ids = $qr_collections->get("ca_objects.object_id", array("returnAsArray" => true, 'checkAccess' => $va_access_values));
			$va_object_ids = $qr_collections->get("ca_objects.object_id", array("returnAsArray" => true));
			...
			if(sizeof($va_child_ids)) {
				$vs_output .=  caGetCollectionLevelSummaryTFANA($po_request, $va_child_ids, $vn_level + 1);
			}
		}
	}
	return $vs_output;
