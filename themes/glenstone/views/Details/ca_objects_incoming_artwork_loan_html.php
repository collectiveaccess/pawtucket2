<?php
	...
	foreach($va_sets as $vn_set_id => $va_set){
		$va_lightbox_crumbs[] = caNavLink($this->request, _t("Lightbox"), "", "", "Sets", "Index")." &#8594; ".caNavLink($this->request, $va_set["name"], "", "", "Lightbox", "SetDetail", array("set_id" => $vn_set_id))." &#8594; ".$t_object->get("ca_objects.preferred_labels.name");
	}
	$vs_lightbox_crumbs = "";
...
<?php
		...
		if(sizeof($va_reps) > 1){		
			...
			foreach($va_reps as $vn_rep_id => $va_rep){
				...
				$va_links[$vn_rep_id] = "<a href='#' onclick='flipIt(this, {$vn_rep_id}); return false;' ".(($vs_class) ? "class='".$vs_class."'" : "").">".$vs_icon.$vs_thumb."</a>\n";
			}
...
<?php			
					...
					if ($this->request->user->hasUserRole("founders_new") || $this->request->user->hasUserRole("admin") || $this->request->user->hasUserRole("curatorial_all_new")){
						if ($t_object->get('ca_objects.artwork_provenance')) {
							$vs_exhibtion_tab = true;
						}
					}
					if ($t_object->get('ca_objects.exhibition_history')) {
						$vs_exhibtion_tab = true;
					}
					if ($t_object->get('ca_objects.literature')) {
						$vs_exhibtion_tab = true;
					}					
...
<?php 			}  ?>	
				...
				<div id="artworkInfo" class="infoBlock">
					...
					<div class='unit'><i>{{{ca_objects.preferred_labels.name}}}</i>, &nbsp;{{{ca_objects.creation_date_display}}}</div>
...
<?php					
					...
					if ($va_exhibition_history = $t_object->get('ca_objects.exhibition_history', array('returnWithStructure' => true, 'idsOnly' => true, 'sort' => 'ca_objects.exhibition_history.exhibition_date', 'sortDirection' => 'DESC'))) {
						...
						foreach ($va_exhibition_history as $ex_key => $va_exhibition_t) {
							foreach ($va_exhibition_t as $va_key => $va_exhibition) {
...
<?php	
				if ($this->request->user->hasUserRole("founders_new") || $this->request->user->hasUserRole("admin") || $this->request->user->hasUserRole("curatorial_all_new") || $this->request->user->hasUserRole("art_location_new")){
					...
					if (($t_object->get('ca_objects.loan_status.loan_status_list', array('convertCodesToDisplayText' => true)) != 'N/A') && ($t_object->get('ca_objects.loan_status.loan_status_list', array('convertCodesToDisplayText' => true)) != '')) {
						print "<div class='unit'><span class='metaTitle'>Loan Status: </span><span class='meta'>";
						if ($t_object->get('ca_objects.loan_status.loan_status_list', array('convertCodesToDisplayText' => true)) == "Yes") {
							print "<div><i style='color:#80E680;' class='fa fa-circle'></i> Yes</div>";
						}
						if ($t_object->get('ca_objects.loan_status.loan_status_list', array('convertCodesToDisplayText' => true)) == "No") {
							print "<div><i style='color:#FF4D4D;' class='fa fa-circle'></i> No</div>";
						}
						if ($t_object->get('ca_objects.loan_status.loan_status_list', array('convertCodesToDisplayText' => true)) == "Maybe") {
							print "<div><i style='color:#FFFF66;' class='fa fa-circle'></i> Maybe</div>";
						}
						if ($vs_assessor = $t_object->get('ca_objects.loan_status.loan_status_assessor', array('convertCodesToDisplayText' => true))) {
							print "<div>".$vs_assessor.", ".$t_object->get('ca_objects.loan_status.loan_status_date')."</div>";
						}	
						if ($vs_notes = $t_object->get('ca_objects.loan_status.loan_status_comments')) {
							print "<div>".$vs_notes."</div>";
						}											
						print "</span></div>";
					}
					$vs_complex_buf = "";
...
<?php
					if ($this->request->user->hasUserRole("founders_new") || $this->request->user->hasUserRole("admin") || $this->request->user->hasUserRole("curatorial_all_new")){
						if ($va_source = $t_object->get('ca_entities.preferred_labels', array('delimiter' => ', ', 'restrictToRelationshipTypes' => array('source', 'advisor'), 'returnAsLink' => true))) {
						...
						if ($t_object->get('ca_objects.insurance_valuation.insurance_value_price')) {
							$va_appraisal = $t_object->get('ca_objects.insurance_valuation', array('returnWithStructure' => true, 'convertCodesToDisplayText' => true, 'sort' => 'ca_objects.insurance_valuation.insurance_valuation_date', 'sortDirection' => 'DESC')); 
							
							print "<div class='unit'><span class='metaTitle'>Current Insurance Value: </span><span class='meta'>";
						
							foreach ($va_appraisal as $ar_key => $va_appraisal_t) {
								foreach ($va_appraisal_t as $ar_key => $va_appraisal_r) {
						...
						if ($t_object->get('ca_objects.insurance_valuation.insurance_value_price')) {
							$va_appraisal = $t_object->get('ca_objects.insurance_valuation', array('returnAsArray' => true, 'returnWithStructure' => true, 'convertCodesToDisplayText' => true, 'sort' => 'ca_objects.insurance_valuation.insurance_valuation_date')); 
							print "<div class='unit'><span class='metaTitle'>Current Insurance Value: </span><span class='meta'>";
...
<?php
				if ($this->request->user->hasUserRole("founders_new") || $this->request->user->hasUserRole("admin") || $this->request->user->hasUserRole("curatorial_all_new") || $this->request->user->hasUserRole("art_condition_new") || $this->request->user->hasUserRole("art_conservation_new")){
					...
					if ($va_general_condition = $t_object->get('ca_objects.general_condition', array('returnWithStructure' => true, 'rawDate' => 1, 'convertCodesToDisplayText' => true))) {
						
						foreach ($va_general_condition as $va_gen_key => $va_general_t) {
							foreach ($va_general_t as $va_gen_key => $va_general) {
								if ($va_general['is_primary_overall'] == "Yes") {
...
#						}
#					}					
#					if ($va_frame_condition = $t_object->get('ca_objects.frame_condition', array('returnAsArray' => true, 'returnWithStructure' => true, 'rawDate' => 1, 'convertCodesToDisplayText' => true))) {
#						foreach ($va_frame_condition as $va_frame_key => $va_frame) {
#							$va_condition_array[$va_frame['frame_date']['start']][] = $va_frame; 
#						}
#					}
#					if ($va_glazing_condition = $t_object->get('ca_objects.glazing_condition', array('returnAsArray' => true, 'returnWithStructure' => true, 'rawDate' => 1, 'convertCodesToDisplayText' => true))) {
#						foreach ($va_glazing_condition as $va_glaze_key => $va_glazing) {
#							$va_condition_array[$va_glazing['glazing_date']['start']][] = $va_glazing; 
#						}
#					}	
#					if ($va_support_condition = $t_object->get('ca_objects.support_condition', array('returnAsArray' => true, 'returnWithStructure' => true, 'rawDate' => 1, 'convertCodesToDisplayText' => true))) {
#						foreach ($va_support_condition as $va_sup_key => $va_support) {
#							$va_condition_array[$va_support['support_date']['start']][] = $va_support;
#						}
#					}
#					if ($va_vitrine_condition = $t_object->get('ca_objects.vitrine_condition', array('returnAsArray' => true, 'returnWithStructure' => true, 'rawDate' => 1, 'convertCodesToDisplayText' => true))) {
#						foreach ($va_vitrine_condition as $va_vit_key => $va_vitrine) {
#							$va_condition_array[$va_vitrine['vitrine_date']['start']][] = $va_vitrine;
#						}
#					}
#					if ($va_mount_condition = $t_object->get('ca_objects.mount_condition', array('returnAsArray' => true, 'returnWithStructure' => true, 'rawDate' => 1, 'convertCodesToDisplayText' => true))) {
#						foreach ($va_mount_condition as $va_mount_key => $va_mount) {
#							$va_condition_array[$va_mount['mount_date']['start']][] = $va_mount; 
#						}
#					}	
#					if ($va_base_condition = $t_object->get('ca_objects.base_condition', array('returnAsArray' => true, 'returnWithStructure' => true, 'rawDate' => 1, 'convertCodesToDisplayText' => true))) {
#						foreach ($va_base_condition as $va_base_key => $va_base) {
#							$va_condition_array[$va_base['base_date']['start']][] = $va_base; 
#						}
#					}
#					if ($va_pedestal_condition = $t_object->get('ca_objects.pedestal_condition', array('returnAsArray' => true, 'returnWithStructure' => true, 'rawDate' => 1, 'convertCodesToDisplayText' => true))) {
						...
						foreach ($va_condition_images as $vn_condition_id => $va_condition_image_t) {
							foreach ($va_condition_image_t as $vn_condition_id => $va_condition_image) {
								if ($va_condition_image['condition_images_primary'] == 162) {
									...
									if ($qr_res->nextRow()) {
										#print "<a href='#' onclick='caMediaPanel.showPanel(\"".caNavUrl($this->request, '', 'Detail', 'GetMediaOverlay', array('object_id' => $vn_object_id, 'value_id' => $qr_res->get('value_id')))."\"); return false;'>".$va_condition_image['condition_images_media']."</a>";
						...
						foreach ($va_conservation_images as $vn_conservation_id => $va_conservation_image_t) {
							foreach ($va_conservation_image_t as $vn_conservation_id => $va_conservation_image) {
								if ($va_conservation_image['legacy_conservation_primary'] == 162) {
									...
									if ($qr_res->nextRow()) {
										#print "<a href='#' onclick='caMediaPanel.showPanel(\"".caNavUrl($this->request, '', 'Detail', 'GetMediaOverlay', array('object_id' => $vn_object_id, 'value_id' => $qr_res->get('value_id')))."\"); return false;'>".$va_conservation_image['legacy_conservation_media']."</a>";
							...
							foreach ($va_condition_holder as $va_condition) {																																				
								...
								*/																															
								if ($va_condition['condition_images_date']['start']) {
									...
									print "<a href='#' class='conditionImage' onclick='caMediaPanel.showPanel(\"".caNavUrl($this->request, '', 'Detail/artworks', 'GetMediaOverlay', array('context' => 'artworks', 'overlay' => 1, 'id' => $vn_object_id, 'value_id' =>  $va_condition['value_id']))."\"); return false;'>".$va_condition['condition_images_media']."</a>";
									#print "<div class='clearfix'></div>";
									$vn_i++;
									if ($vn_i == 3) {
										print "<div class='clearfix'></div>";
								...
								if ($va_condition['legacy_conservation_date']['start']) {
									...
									print "<a href='#' class='conditionImage' onclick='caMediaPanel.showPanel(\"".caNavUrl($this->request, '', 'Detail/artworks', 'GetMediaOverlay', array('context' => 'artworks', 'overlay' => 1, 'id' => $vn_object_id, 'value_id' =>  $va_condition['legacy_value_id']))."\"); return false;'>".$va_condition['legacy_conservation_media']."</a>";
									#print "<div class='clearfix'></div>";
									$vn_i++;
									if ($vn_i == 3) {
										print "<div class='clearfix'></div>";
								...
								if ($va_condition['conservation_reports_date']['start']) {
									#print "<b>".caGetLocalizedHistoricDateRange($va_condition['legacy_conservation_date']['start'], $va_condition['legacy_conservation_date']['end'])."</b>: <br/>";
									print "<a href='#' class='conditionImage' onclick='caMediaPanel.showPanel(\"".caNavUrl($this->request, '', 'Detail/artworks', 'GetMediaOverlay', array('context' => 'artworks', 'overlay' => 1, 'id' => $vn_object_id, 'value_id' =>  $va_condition['outside_value_id']))."\"); return false;'>".$va_condition['conservation_reports_media']."</a>";
									#print "<div class='clearfix'></div>";
									$vn_i++;
									if ($vn_i == 3) {
										print "<div class='clearfix'></div>";
...
if ($this->request->user->hasUserRole("founders_new")) {
					...
					if ($va_non_excl = $t_object->get('ca_objects.non_exclusive', array('returnWithStructure' => true, 'convertCodesToDisplayText' => true))) {
						foreach ($va_non_excl as $va_non_key => $va_non_exc_t) {
							foreach ($va_non_exc_t as $va_non_key => $va_non_exc) {
					...
					if ($va_transparencys = $t_object->get('ca_objects.transparency', array('returnWithStructure' => true, 'convertCodesToDisplayText' => true))) {
						foreach ($va_transparencys as $va_tran_key => $va_transparency_t) {
							foreach ($va_transparency_t as $va_tran_key => $va_transparency) {
					...
					if ($va_photo_records = $t_object->get('ca_objects.photo_record', array('returnWithStructure' => true , 'convertCodesToDisplayText' => true))) {
						foreach ($va_photo_records as $va_photo_key => $va_photo_record_t) {
							foreach ($va_photo_record_t as $va_photo_key => $va_photo_record) {
...
<?php
					if ($va_notes = $t_object->get('ca_objects.other_notes', array('returnWithStructure' => true, 'convertCodesToDisplayText' => true))) {
...
<?php
	if ($t_object->get('ca_objects.related', array('checkAccess' => caGetUserAccessValues($this->request), 'limit' => 30, 'restrictToTypes' => array('audio', 'document', 'ephemera', 'image', 'moving_image')))) {
		$va_object_ids = $t_object->get('ca_objects.related.object_id', array('checkAccess' => caGetUserAccessValues($this->request), 'returnAsArray' => true, 'returnWithStructure' => true, 'restrictToTypes' => array('audio', 'document', 'ephemera', 'image', 'moving_image')));
...
<?php
	...
	if ($t_object->get('ca_objects.related', array('checkAccess' => caGetUserAccessValues($this->request), 'limit' => 30, 'restrictToTypes' => array('book')))) {
?>	
<div class="row" style="clear:both;">
	<div class='col-xs-12 col-sm-12 col-md-12 col-lg-12'>
		<div class="container">
		...
		<div id='libraryBlock'>
			...
			<div class='blockResults'><div id="libraryscrollButtonPrevious" class="scrollButtonPrevious"><i class="fa fa-angle-left"></i></div><div id="libraryscrollButtonNext" class="scrollButtonNext"><i class="fa fa-angle-right"></i></div>
				<div id='libraryResults' class='scrollBlock'>
					<div class='blockResultsScroller'>
<?php
			$va_object_ids = $t_object->get('ca_objects.related.object_id', array('checkAccess' => caGetUserAccessValues($this->request), 'returnAsArray' => true, 'returnWithStructure' => true, 'restrictToTypes' => array('book')));
			...
			if (is_array($va_object_ids) && sizeof($va_object_ids)) {
				...
				while($qr_res->nextHit()) {
					...
					print "<p>".caNavLink($this->request, $qr_res->get('ca_entities.preferred_labels', array('restrictToRelationshipTypes' => array('author'), 'delimiter' => ', ')), '', '', 'Detail', 'library/'.$qr_res->get('ca_objects.object_id'))."</p>";
					print "<p>".$qr_res->get('ca_entities.preferred_labels', array('restrictToRelationshipTypes' => array('publisher'), 'delimiter' => ', '))."</p>";
...
<?php
			...
			if (is_array($va_object_ids) && sizeof($va_object_ids)) {
				...
				while($qr_res->nextHit()) {
					...
					print "<div class='resultImg'>".caNavLink($this->request, $qr_res->get('ca_object_representations.media.widepreview'), '', '', 'Detail', 'artworks/'.$qr_res->get('ca_objects.object_id'))."</div>";
					print "<p class='artist'>".$qr_res->get('ca_entities.preferred_labels', array('restrictToRelationshipTypes' => 'artist'))."</p>";					
					print "<p><i>".caNavLink($this->request, $qr_res->get('ca_objects.preferred_labels.name'), '', '', 'Detail', 'artworks/'.$qr_res->get('ca_objects.object_id'))."</i>, ".$qr_res->get('ca_objects.creation_date')."</p>";
...
<script type='text/javascript'>
	...
	function flipIt(l, rep_id) {
		$(".active").removeClass("active"); 
		$(l).parent().addClass("active"); 
		$(l).addClass("active"); 
		$(".jcarousel").jcarousel("scroll", $("#slide" + rep_id), false);
	}
</script>