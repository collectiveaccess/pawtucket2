		...
		while($q_set_items->nextHit() && ($vn_c < $vn_hits_per_block)){
			...
			if($t_set_item->get("item_id")){
				...
				if ($q_set_items->get('ca_objects.type_id') == 30) {
					...
					$vs_label_detail_link 	= "<p><i>".$q_set_items->get("ca_objects.preferred_labels.name")."</i>, ".$q_set_items->get("ca_objects.creation_date_display")."</p>";
				...
				print "<div class='col-xs-12 col-sm-4 lbUnit lbItem".$t_set_item->get("item_id")."' data-row_id='{$vn_object_id}' id='row-{$vn_object_id}'><div class='lbItemContainerList'>";
