		...
		while($q_set_items->nextHit() && ($vn_c < $vn_hits_per_block)){
			...
			if($t_set_item->get("item_id")){
				...
				if ($q_set_items->get('ca_objects.type_id') == 30) {
					...
					$vs_label_detail_link 	= "<p><i>".caDetailLink($this->request, $q_set_items->get("ca_objects.preferred_labels.name"), '', 'ca_objects', $vn_id)."</i>, ".$q_set_items->get("ca_objects.creation_date_display")."</p>";
				...
				$vn_object_id = $q_set_items->get("object_id");
				print "<div class='col-xs-6 col-sm-4 col-md-3 col-lg-3 lbUnit lbItem".$t_set_item->get("item_id")."' data-row_id='{$vn_object_id}' id='row-{$vn_object_id}'><div class='lbItemContainer'>";
		 ...
		 jQuery(document).ready(function() {
			...
			$(".sortable").sortable({ 
				...
				update: function( event, ui ) {
					var row_ids = [];
					jQuery.each(jQuery(".lbUnit"), function(k, v) {
						row_ids.push(jQuery(v).data('row_id'))
					});
					
					// POST to server using $.post or $.ajax
					$.ajax({
						type: 'POST',
						url: '<?php print caNavUrl($this->request, "", "Sets", "AjaxReorderItems"); ?>',
						data: { row_ids: row_ids.join(";") }
					});
				}
			});
