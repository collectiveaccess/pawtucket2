		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit() && ($vn_c < $vn_hits_per_block)) {
				...
				if ($qr_res->get('ca_objects.type_id') == 30) {
					...
					$vs_label_detail 	= "<p style='text-decoration:underline;'>".caDetailLink($this->request, $qr_res->get("{$vs_table}.preferred_labels"), '', $vs_table, $vn_id)."</p>";
					...
					$vs_label_author	 	= "<p class='artist'>".$qr_res->get("ca_entities.parent.preferred_labels", array('restrictToRelationshipTypes' => 'author', 'delimiter' => '; ', 'template' => '^ca_entities.parent.preferred_labels.forename ^ca_entities.parent.preferred_labels.middlename ^ca_entities.parent.preferred_labels.surname'))."</p>";
					$vs_label_detail 	= "<p style='text-decoration:underline;'>".$qr_res->get("{$vs_table}.parent.preferred_labels")."</p>";
					...
					if ($qr_res->get('ca_objects.dc_date.dc_dates_value')) {
						$vs_date_link = "<p>".$qr_res->get('ca_objects.dc_date.dc_dates_value', array('returnAsLink' => true, 'delimiter' => '; '))."</p>";
					}else {
						$vs_date_link = "";
					}
