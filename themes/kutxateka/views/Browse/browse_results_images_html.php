 ...
 */
	...
	if($qr_res->numHits()){
		...
		if ($vn_start < $qr_res->numHits()) {
			...
			if ($vs_table == 'ca_objects') {
				...
				if (sizeof($va_rel_object_ids)) {
					...
					while($qr_rel_objects->nextHit()) {
						$va_related_images[$qr_rel_objects->get("ca_objects.object_id")] = $qr_rel_objects->getMediaTag('ca_object_representations.media', 'mediumlarge', array('class'=>'esta'));
			...
			while($qr_res->nextHit() && ($vn_c <= $vn_hits_per_block)) {
				$vs_tt_class = "";
				if(in_array($vn_c, array(1,2,5,6,9,10))){
					$vs_tt_class = "left";
				}else{
					$vs_tt_class = "right";
				}
				...
				$vs_label_detail_link 	= caDetailLink($this->request, ((mb_strlen($vs_label) > 60) ? mb_substr($vs_label, 0, 57).'...' : $vs_label), 'verde', $vs_table, $vn_id);
				...
				if ($vs_table == 'ca_objects') {
					...
					if($qr_res->get("type_id") == $vn_photo_report_type_id){
						$vs_rep_detail_link = caDetailLink($this->request, $va_related_images[$va_rel_object_ids[$vn_id]], '', $vs_table, $vn_id);
						$vs_rep_tooltip_img = $va_related_images[$va_rel_object_ids[$vn_id]];
					}else{
						...
						$vs_rep_tooltip_img = $qr_res->getMediaTag('ca_object_representations.media', 'mediumlarge', array('class'=>'esta'));
					}
				}
				print "<article>";
				if($vs_rep_tooltip_img){
					print "<figure><span class='resultImg'>".$vs_rep_detail_link."</span><div class='visor ".$vs_tt_class."'>".$vs_rep_tooltip_img."</div></figure><!-- end figure -->";
				}else{
					print "<figure><br/><br/><br/><br/><p>"._t("No media available")."</p></figure>";
				}
	...
	jQuery(document).ready(function() {
		$(".resultImg img").each(function(){
			...
			if ($this.width() > $this.height()) {
				$this.addClass("resultImgHor");
			}else{
				$this.addClass("resultImgVert");
			}
		});
	});
</script>