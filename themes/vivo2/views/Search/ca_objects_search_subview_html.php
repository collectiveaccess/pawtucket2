 ...
 */
	...
	$vs_default_placeholder_tag = "<div class='placeholderImage'>".caGetThemeGraphic($this->request, 'placeholderWide.jpg', array("alt" => "Placeholder"))."</div>";
...
?>
				<span class='multisearchFullResults'><?php print caNavLink($this->request, _t('Full results').' <span class="material-symbols-outlined">arrow_outward</span>', '', '', 'Search', '{{{block}}}', array('search' => str_replace("/", "", $vs_search))); ?></span> | 
<?php
				}
?>
				
				<span class='multisearchSort'><?php print _t("sort by:"); ?> {{{sortByControl}}}</span>
				<?php print $vs_sort_direction_control; ?>
			</small>
<?php
			if(in_array($vs_block, $va_browse_types)){
?>
...
?>
			<div class='blockResults objects'><div id="{{{block}}}scrollButtonPrevious" class="scrollButtonPrevious" aria-label="previous" role="link" tabindex="0"><i class="fa fa-angle-left" aria-label="previous"></i></div><div id="{{{block}}}scrollButtonNext" class="scrollButtonNext" aria-label="next" role="link" tabindex="0"><i class="fa fa-angle-right" aria-label="next"></i></div>
...
?>
				<div class='multisearchResultCaption'><?php print $qr_results->getWithTemplate($vs_caption_template, array("checkAccess" => $va_access_values)); ?></div>
			</div><!-- end blockResult -->
<?php
			$vn_count++;
...
?>
				<script type="text/javascript">
					jQuery('#<?php print $vs_block; ?>_sort_direction').find('span').removeClass('glyphicon-sort-by-alphabet').removeClass('glyphicon-sort-by-alphabet-alt');
					jQuery('#<?php print $vs_block; ?>_sort_direction').find('span').text('south');
					$('#<?php print $vs_block; ?>_sort_direction').show();
				</script>
<?php
			}else{
?>
				<script type="text/javascript">
					jQuery('#<?php print $vs_block; ?>_sort_direction').find('span').removeClass('glyphicon-sort-by-alphabet').removeClass('glyphicon-sort-by-alphabet-alt');
					jQuery('#<?php print $vs_block; ?>_sort_direction').find('span').text('north');
					$('#<?php print $vs_block; ?>_sort_direction').show();
				</script>
<?php
			}
...
?>
	<script type="text/javascript">
		$('#<?php print $vs_block; ?>_sort_direction span').click(function(){
			$('#<?php print $vs_block; ?>_sort_direction').hide();
		});				
	</script>
