	...
	$va_add_to_set_link_info = caGetAddToSetInfo($this->request);
		...
		if ($vn_start < $qr_res->numHits()) {
			...
			if ($vs_table != 'ca_objects') {
				...
				$qr_res->seek($vn_start);
				$va_images = caGetDisplayImagesForAuthorityItems($vs_table, $va_ids, array('version' => 'iconlarge', 'relationshipTypes' => caGetOption('selectMediaUsingRelationshipTypes', $va_options, null), 'objectTypes' => caGetOption('selectMediaUsingTypes', $va_options, null), 'checkAccess' => $va_access_values));
			} else {
				$va_images = null;
			}
			...
			while($qr_res->nextHit()) {
				...
				if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result')){
					...
					if(!$vs_image){
						$vs_image = "<div class='placeholderImage'>".caGetThemeGraphic($this->request, 'placeholder.jpg', array("alt" => "Placeholder"))."</div>";
						#if ($vs_table == 'ca_objects') {
...
if(in_array($vs_table, array("ca_entities"))){
		$vs_result_output = "
		<div class='col-xs-{$vn_col_span_xs} col-sm-{$vn_col_span_sm} col-md-{$vn_col_span}'>
			<div class='resultListSimple'>
				".$qr_res->getWithTemplate("<l>".$vs_caption."</l>")."
			</div>
		</div><!-- end col -->";
}else{	
		$vs_result_output = "<div class='col-xs-12 col-sm-12 col-md-6'>".$qr_res->getWithTemplate("<l><div class='bgWhite resultListTile'><div class='row'><div class='col-sm-3 col-md-4 col-lg-3'>".$vs_image."</div><div class='col-sm-9 col-md-8 col-lg-9'><div class='resultListTileText'>".$vs_caption."</div></div></div></div>")."</div>";
}
