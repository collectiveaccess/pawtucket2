	...
	if(is_array($va_sets) && sizeof($va_sets)){
		# --- main area with info about selected set loaded via Ajax				
			...
			foreach($va_sets as $vn_set_id => $va_set){
				...
				print "<div class='col-sm-6'>".caNavLink($this->request, "<div class='bgLightGray imgTile'><div class='row'><div class='col-sm-5 col-md-4 col-lg-3'>".$va_first_item["representation_tag"]."</div><div class='col-sm-7 col-md-8 col-lg-9'><div class='imgTileText'><div class='imgTileTextTitle'>".$va_set["name"]."</div>".$va_set["item_count"]." ".(($va_set["item_count"] == 1) ? _t("item") : _t("items"))."</div></div></div></div>", "", "", "Gallery", $vn_set_id)."</div>";
				$vn_i++;
				if ($vn_i == 2) {
					print "</div><!-- end row -->\n";
					$vn_i = 0;
