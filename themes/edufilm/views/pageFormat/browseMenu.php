...
?>
				<li class="dropdown<?php print (($this->request->getController() == "Browse") && (strToLower($this->request->getAction()) != "collections")) ? ' active' : ''; ?>" style="position:relative;"><a href="#" class="dropdown-toggle mainhead top" data-toggle="dropdown"><?php print $o_config->get("browse_menu_button_text") ? $o_config->get("browse_menu_button_text") : _t("Browse"); ?></a>
					<ul class="dropdown-menu">
<?php
						foreach($va_browse_types as $vs_browse_name => $va_browse_type){
							if(!$va_browse_type["dontShowInBrowseMenu"]){
								print "<li>".caNavLink($this->request, caUcFirstUTF8Safe($va_browse_type['displayName']), '', '', 'Browse', $vs_browse_name, array("sort" => "default"))."</li>";
							}
						}
						print "<li>".caNavLink($this->request, _t("Das Bild"), 'das-bild', '', 'Browse',  'texts', array('facet' => 'related_objects', 'id' => '2028', 'view' => 'list'))."</li>";
?>
					</ul>	
				</li>
<?php				
