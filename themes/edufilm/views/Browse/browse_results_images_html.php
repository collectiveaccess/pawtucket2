			...
			while($qr_res->nextHit()) {
				...
				if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result')){
					...
					if(($vs_table == "ca_objects") && ($vs_typecode == "nonav_manifestation")){
						...
						$vs_image = $qr_res->getWithTemplate("<unit relativeTo='ca_objects.related' restrictToTypes='item' restrictToRelationshipTypes='IsItemOfNonAV' limit='1'>^ca_object_representations.media.medium</unit>", array("checkAccess" => $va_access_values));	
						
						
					}else{
						$vs_image = (in_array($vs_table, array('ca_objects', 'ca_entities'))) ? $qr_res->getMediaTag("ca_object_representations.media", 'medium', array("checkAccess" => $va_access_values)) : $va_images[$vn_id];
					}
					if(!$vs_image){
