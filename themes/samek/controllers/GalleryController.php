	...
	require_once(__CA_LIB_DIR__.'/pawtucket/BasePawtucketController.php');
 	...
 	class GalleryController extends BasePawtucketController {
 		...
 		public function __call($function, $pa_args) {
 			...
 			switch($function){
 				case "index":
 					#--- list out the different object group types
 					$t_list = new ca_lists();
		
					$va_groups = $t_list->getItemsForList("set_object_groups", array("directChildrenOnly" => "1"));
					if(is_array($va_groups) && sizeof($va_groups)){
						$t_list_item = new ca_list_items();
						$va_groups_for_display = array();
						foreach($va_groups as $vn_item_id => $va_group){
							$va_group = array_pop($va_group);
							$t_list_item->load($vn_item_id);
							$va_groups_for_display[$vn_item_id] = array("name" => $va_group["name_singular"], "image" => $t_list_item->get("ca_list_items.icon.large.url"), "description" => $t_list_item->get("ca_list_item_labels.description"));
						}
					}
 					$this->view->setVar("groups", $va_groups_for_display);
 				...
 				# ---------------------------------
 				case "list":
					#limit the list to only show sets of a certain object group ty pe (ca_sets.set_object_group)
					$group = $this->request->getParameter('group', pInteger);
					if($gallery_set_type_id){
						...
						if($group){
							$t_list_item = new ca_list_items($group);
							$this->view->setVar("group_name", $t_list_item->get("ca_list_items.preferred_labels"));
							if($tmp = $t_list_item->get("ca_list_item_labels.description")){
								$this->view->setVar("group_description", $tmp);
							}
							$qr_sets = caMakeSearchResult("ca_sets", array_keys($sets), ['checkAccess' => $this->opa_access_values]);
							if($qr_sets->numHits()){
								while($qr_sets->nextHit()){								
									if($group != $qr_sets->get("ca_sets.set_object_group")){
										unset($sets[$qr_sets->get("ca_sets.set_id")]);
									}
								}
							}
						}
