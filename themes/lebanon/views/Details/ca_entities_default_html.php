...
<?php
				if ($va_lifespan = $t_item->get('ca_entities.lifespan')) {
					print "<div class='unit'><h6>Life dates</h6>".$va_lifespan."</div>";
				}
				if ($va_bio = $t_item->get('ca_entities.biography')) {
					print "<div class='unit'><h6>Biography</h6>".$va_bio."</div>";
				}
				if ($va_bio_source = $t_item->get('ca_entities.biography_source')) {
					print "<div class='unit'><h6>Biography Source</h6>".$va_bio_source."</div>";
				}
?>
				{{{<ifcount min="1" code="ca_entities.lc_names"><ifdef code="ca_entities.lc_names"><div class='unit'><H6>Library of Congress Name Authority File</H6><unit delimiter="<br/>">^ca_entities.lc_names</unit></div></ifdef></ifcount>}}}
...
{{{<ifcount code="ca_objects" min="1">
			...
			<script type="text/javascript">
				jQuery(document).ready(function() {
					jQuery("#browseResultsContainer").load("<?php print caNavUrl($this->request, '', 'Search', 'all', array('search' => 'entity_id:^ca_entities.entity_id'), array('dontURLEncodeParameters' => true)); ?>", function() {
