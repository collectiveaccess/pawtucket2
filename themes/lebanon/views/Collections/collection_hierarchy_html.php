...
<?php
					...
					if($qr_collection_children->numHits()){
						while($qr_collection_children->nextHit()) {
							...
							if ($qr_collection_children->get("ca_collections.unitdate.dacs_date_value")) {
								$vs_collection_date = "<p>(".$qr_collection_children->get("ca_collections.unitdate.dacs_date_value").")</p>";
							} else { 
								$vs_collection_date = null; 
							}
							if($vb_link_sublist){
								print "<a href='#' class='openCollection openCollection".$qr_collection_children->get("ca_collections.collection_id")."'>".$vs_icon." ".$qr_collection_children->get('ca_collections.preferred_labels').$vs_record_count.$vs_collection_date."</a>";
							}else{
