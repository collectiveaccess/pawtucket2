...
function printLevel($po_request, $va_collection_ids, $o_config, $vn_level, $va_options = array()) {
	...
	if($qr_collections->numHits()){
		while($qr_collections->nextHit()) {
			...
			if ($qr_collections->get("ca_collections.unitdate.dacs_date_value")) {
				$vs_collection_date = "<p> (".$qr_collections->get("ca_collections.unitdate.dacs_date_value").")</p>";
			} else { 
				$vs_collection_date = null; 
			}			
			if($vb_link){
				$vs_output .= $vs_icon." ";
				if($vb_collapse_link){
					$vs_output .= "<a href='#' onClick='jQuery(\"#level".$qr_collections->get('ca_collections.collection_id')."\").toggle(); return false;'>".$qr_collections->get('ca_collections.preferred_labels').$vs_collection_date."</a>";
				}else{
					$vs_output .= caDetailLink($po_request, $qr_collections->get('ca_collections.preferred_labels'), '', 'ca_collections',  $qr_collections->get("ca_collections.collection_id")).$vs_collection_date;
				}
				...
				if($vb_collapse_link){
					...
					$vs_output .= $qr_collections->get("ca_collections.preferred_labels").$vs_collection_date;
				}
				$vs_output .= "</span>";
			}
