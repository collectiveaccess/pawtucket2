	...
	require_once(__CA_LIB_DIR__.'/pawtucket/BasePawtucketController.php');
 	...
 	class GalleryController extends BasePawtucketController {
 		...
 		public function __call($function, $pa_args) {
 			...
 			# set type is passed through URL 		
 			$vs_set_type = $this->request->getParameter('set_type', pString);
 			$gallery_set_type_id = $t_list->getItemIDFromList('set_types', $vs_set_type); 		
 			$this->view->setVar('set_type', $vs_set_type);
  			...
  			if($function == "index"){
						...
						if(!$set_first_items[$set_id][$vn_item_id]["representation_tag"]){
							if(Datamodel::getTableName($va_set['table_num']) != "ca_objects"){
								...
								if($vs_thumbnail = $t_instance->getWithTemplate('^ca_object_representations.media.iconlarge', array("checkAccess" => $this->opa_access_values))){
 								...
 								}elseif($vs_thumbnail = $t_instance->getWithTemplate('<unit relativeTo="ca_objects.related" length="1">^ca_object_representations.media.iconlarge</unit>', array("checkAccess" => $this->opa_access_values))){
 									$set_first_items[$set_id][$vn_item_id] = array("representation_tag" => $vs_thumbnail);
 								}
							}
						}
						if(!$set_first_items[$set_id][$vn_item_id]["representation_tag"]){
							$set_first_items[$set_id][$vn_item_id]["representation_tag"] = caGetThemeGraphic($this->request, 'eye_square.png', array("alt" => "No media available"));
						}
					}
 				...
 				if(is_array($sets) && sizeof($sets)){
					switch($vs_set_type){
						case "timelines":
							...
							if(!$set_id){
								$set_id = array_key_first($sets);
							}
							$t_set = $this->_getSet($set_id);
							$this->view->setVar('set_id', $set_id);
							$this->view->setVar('set', $t_set);
							$this->render("Gallery/index_timeline_html.php");
						break;
						# ----
						case "maps":
							AssetLoadManager::register('timeline');
							...
							if($set_id){
								...
								$this->view->setVar('set_id', $set_id);
								$this->view->setVar('set', $t_set);
								$table = Datamodel::getTableName($va_set['table_num']);
							...
							$this->render("Gallery/index_map_html.php");
						break;
						# -----
						case "highlights":
 					...
 					throw new ApplicationException(_t('No sets configured to display'));
 				}	
 			}else{
 				$set_id = (int)$function;
 				$t_set = $this->_getSet($set_id);
				...
				if (!$this->request->isAjax()){
 				...
 				# --- display is based on the type of set it is
 				$vs_set_type = $t_set->get("ca_sets.type_id", array("convertCodesToDisplayText" => true));
 				...
 				switch($vs_set_type) {
					...
					case 'Map':
						...
						break;
					default:
					# highlights and research guides
						$this->render("Gallery/detail_html.php");
						break;
				}
		...
		# -------------------------------------------------------
 		...
 		public function getSetItemRep(){
			...
			$set_items = caExtractValuesByUserLocale($t_set->getItems(array("thumbnailVersions" => array("icon", "iconlarge"), "checkAccess" => $this->opa_access_values)));
 			...
 			if (!($t_instance = Datamodel::getInstanceByTableNum($t_set->get('table_num')))) { throw new ApplicationException(_t('Invalid item')); }
			$t_instance->load($t_set_item->get("ca_set_items.row_id"));		
 			$this->view->setVar("set_item_id", $item_id); 
 			$vn_rep_id = $t_instance->get("ca_object_representations.representation_id", array("checkAccess" => $this->opa_access_values));
 			if(!$vn_rep_id){
 				# --- if there is no rep and this is not an objects table, try to show a related object rep instead
 				$vn_rep_id = $t_instance->getWithTemplate("<unit relativeTo='ca_objects.related' length='1'>^ca_object_representations.representation_id</unit>", array("checkAccess" => $this->opa_access_values));
 			}
 			if($vn_rep_id){
				...
				if(!(is_array($this->opa_access_values) && sizeof($this->opa_access_values) && !in_array($t_rep->get("access"), $this->opa_access_values))){
					...
					$this->view->setVar("representation_id", $vn_rep_id);
				}
			}
