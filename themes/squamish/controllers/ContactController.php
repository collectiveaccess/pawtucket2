 		...
 		public function Send() {
 			...
 			if(sizeof($va_errors) == 0){
 				# --- send email
					...
					$vs_contact_email = $this->config->get("contact_email");
					# --- if this is an item inquiry and the item is a cultural item, send to different email address
					if(strToLower($this->request->getParameter("itemtype", pString)) == "cultural item"){
						$vs_contact_email = $this->config->get("contact_email_cultural_items");
					}
					if(caSendmail($vs_contact_email, $this->request->config->get("ca_admin_email"), $vs_subject_line, $vs_mail_message_text, $vs_mail_message_html)){
