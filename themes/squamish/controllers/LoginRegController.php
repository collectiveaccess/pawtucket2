...
class LoginRegController extends BasePawtucketController {
	...
	function profileSave() {
		if (caValidateCSRFToken($this->request, null, ['notifications' => $this->notification])) {
			...
			$ps_email = html_entity_decode(strip_tags($this->request->getParameter("email", pString)));
			$ps_fname = strip_tags($this->request->getParameter("fname", pString));
			$ps_lname = strip_tags($this->request->getParameter("lname", pString));
			$ps_password = html_entity_decode(strip_tags($this->request->getParameter("password", pString)));
			$ps_password2 = html_entity_decode(strip_tags($this->request->getParameter("password2", pString)));
			...
			$ps_member_code = html_entity_decode(strip_tags($this->request->getParameter("member_code", pString)));
			$ps_band_number = html_entity_decode(strip_tags($this->request->getParameter("pref_user_profile_band_number", pString)));
			
			$ps_staff_code = html_entity_decode(strip_tags($this->request->getParameter("staff_code", pString)));
			$ps_staff_jde_number = html_entity_decode(strip_tags($this->request->getParameter("pref_user_profile_staff_jde_number", pString)));

			$o_appvars = new ApplicationVars();
			$vs_configured_member_code = $o_appvars->getVar("pawtucket_global_register_member_code");
			$vs_configured_staff_code = $o_appvars->getVar("pawtucket_global_register_staff_code");
			...
			if (!caCheckEmailAddress($ps_email)) {
				...
				if(!$va_errors["email"]){
					if($t_user->get('userclass') == 1) { $t_user->set("user_name", $ps_email); }	// only set for public users
			...
			$vb_add_member_role = $vb_add_staff_role = false;
			# --- only check nation codes if do not already have the role
			if(!$t_user->hasRole("nation_member")){
				# --- if member, must have both code and band number
				# member_code pref_user_profile_band_number
				if($ps_member_code || $ps_band_number){
					if(!$ps_band_number){
						$va_errors["user_profile_band_number"] = _t("Please enter your band number");
					}
					if(!$ps_member_code){
						$va_errors["user_profile_band_number"] = _t("Please enter the member code");
					}
					if($ps_member_code && ($ps_member_code != $vs_configured_member_code)){
						$vs_tmp = _t("The member registration code is incorrect");
						if($va_errors["user_profile_band_number"]){
							$vs_tmp = "<br/>".$vs_tmp;
						}
						$va_errors["user_profile_band_number"] = $va_errors["user_profile_band_number"].$vs_tmp;
					}
					if($ps_member_code && $ps_band_number && !$va_errors["user_profile_band_number"]){
						$vb_add_member_role = true;
					}
				}
			}
			# --- only check staff codes if do not already have the role
			if(!$t_user->hasRole("nation_staff")){		
				# --- if staff, must have both code and band number
				if($ps_staff_code || $ps_staff_jde_number){
					if(!$ps_staff_jde_number){
						$va_errors["user_profile_staff_jde_number"] = _t("Please enter your JDE number");
					}
					if(!$ps_staff_code){
						$va_errors["user_profile_staff_jde_number"] = _t("Please enter the staff code");
					}
					if($ps_staff_code && ($ps_staff_code != $vs_configured_staff_code)){
						$vs_tmp = _t("The staff registration code is incorrect");
						if($va_errors["user_profile_staff_jde_number"]){
							$vs_tmp = "<br/>".$vs_tmp;
						}
						$va_errors["user_profile_staff_jde_number"] = $va_errors["user_profile_staff_jde_number"].$vs_tmp;
					}
					if($ps_staff_code && $ps_staff_jde_number && !$va_errors["user_profile_staff_jde_number"]){
						$vb_add_staff_role = true;
					}
				}
			}
			...
			if(sizeof($va_errors) == 0){
				...
				if($t_user->numErrors()) {
					...
					if($vb_add_member_role){
						$t_user->addRoles(array("nation_member"));
					}
					if($vb_add_staff_role){
						$t_user->addRoles(array("nation_staff"));
					}
	...
	function register() {
		...
		$ps_password2 = html_entity_decode(strip_tags($this->request->getParameter("password2", pString)));
		$ps_security = strip_tags($this->request->getParameter("security", pString));
		$ps_captcha = strip_tags($this->request->getParameter("g-recaptcha-response", pString));
		$ps_group_code = strip_tags($this->request->getParameter("group_code", pString));
