		...
		if ($start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit()) {
				...
				if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($cache_key,'browse_result')){
					...
					}elseif($image_format == "list_grid"){
						$result_output = "<div class='col-md-6 col-lg-4 d-flex'>".caDetailLink($this->request, "
								<div id='row{$id}' class='card flex-grow-1 w-100 rounded-0 shadow border-0 mb-4'>
									<div class='card-body'>
										{$caption}
									</div>
								 </div>", "w-100 d-flex", $table, $id)."</div><!-- end col -->";
					}else{
						$result_output = "<div class='col-12'>".caDetailLink($this->request, "
								<div id='row{$id}' class='card w-100 rounded-0 shadow border-0 mb-4'>
