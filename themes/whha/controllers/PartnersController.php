<?php
/* ----------------------------------------------------------------------
 * /controllers/PartnersController.php : 
 * ----------------------------------------------------------------------
 * CollectiveAccess
 ...
 */
	...
	require_once(__CA_LIB_DIR__.'/pawtucket/BasePawtucketController.php');
 	...
 	class PartnersController extends BasePawtucketController {
 		# -------------------------------------------------------
 		public function __construct(&$request, &$response, $view_paths=null) {
            ...
            caSetPageCSSClasses(array("partners"));
			MetaTagManager::setWindowTitle($this->request->config->get("app_display_name").": Partners");
 			
 			$this->config = Configuration::load("partners.conf");
 			
 			$t_list = new ca_lists();
 		 	$this->opn_partner_id = $t_list->getItemIDFromList('entity_types', $this->config->get("entity_type"));
 		...
 		public function __call($function, $args){
			...
			$qr_res = $o_search->search("ca_entities.type_id:".$this->opn_partner_id);
			...
			$qr_res->seek(0);
 		 	$context = new ResultContext($this->request, "ca_entities", "partners");
			$context->setAsLastFind();
			$context->setResultList($qr_res->getPrimaryKeyValues(200));
			$context->saveContext();
 			$this->view->setVar("partner_results", $qr_res);

 			$this->render("Partners/partners_html.php");
 		}
 		# -------------------------------------------------------
		/** 
