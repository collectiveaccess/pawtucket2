...
?>
		<div style="float:left;">
			...
			<div class="maincol" >	
				...
				<p class="searchbox-top-links"><?php print caNavlink($this->request, 'Back to Catalogue', 'searchbox-button', '', '','');?></a></p>
				
					<div id="browse" style="width:578px; margin-left:-30px;"><div id="resultBox"> 
<?php
	}
?>
...
<?php
			if (sizeof($va_criteria)) {
				...
				if (sizeof($va_facets)) { 
?>
					<div id="refineBrowse"><span class='refineHeading'><?php print _t('Refine results by'); ?>:<br/></span>
<?php
						$vn_i = 1;
						$va_available_facets = $this->getVar('available_facets');
						foreach($va_available_facets as $vs_facet_code => $va_facet_info) {
							print "<a href='#' onclick='caUIBrowsePanel.showBrowsePanel(\"{$vs_facet_code}\");' class='facetLink'>".ucwords($va_facet_info['label_plural'])."</a>";
...
<?php
				...
				#print caNavLink($this->request, _t('start new search')." &rsaquo;", '', '', '', '', array());
...
<script type="text/javascript">
	var caUIBrowsePanel = caUI.initBrowsePanel({ facetUrl: '<?php print caNavUrl($this->request, '', 'Browse', 'getFacet'); ?>', useExpose: false });
