...
?> -->
				<?php print caNavLink($this->request, caGetThemeGraphic($this->request, 'HEDDA-STERNE-FOUNDATION_LOGO_A01_RGB.jpg', array("alt" => $this->request->config->get("app_display_name"), "role" => "banner")), "navbar-brand", "", "","");?>
			</div>
				...
				<ul class="nav navbar-nav navbar-right" id="user-navbar" role="list" aria-label="<?= _t("User Navigation"); ?>">
					<li class="dropdown" style="position:relative;">
						...
						<a href="#" class="dropdown-toggle icon" data-toggle="dropdown">
							<span class="" aria-label="<?= _t("User options"); ?>">Account</span>
						</a>
...
?> -->
				...
				<ul class="nav navbar-nav navbar-right menuItems" role="list" aria-label="<?= _t("Primary Navigation"); ?>">
					...
					<?php if($this->request->isLoggedIn() && ($this->request->user->hasRole('admin') || $this->request->user->hasRole('cataloguer'))) { ?>
