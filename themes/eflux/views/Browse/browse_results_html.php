		...
		if (sizeof($va_criteria) > 0) {
			...
			foreach($va_criteria as $va_criterion) {
				...
				if ($va_criterion['facet_name'] != '_search') {
					print '<button type="button" class="btn btn-default btn-sm">'.$va_criterion['value'].caNavLink($this->request, ' <span class="glyphicon glyphicon-remove-circle"></span>', 'browseRemoveFacet', '*', '*', '*', array('removeCriterion' => $va_criterion['facet_name'], 'removeID' => $va_criterion['id'], 'view' => $vs_current_view, 'key' => $vs_browse_key))."</button>";
				}else{
