			...
			while($qr_res->nextHit()) {
				...
				if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result')){
					...
					if ($vs_table == 'ca_collections') {
						$vs_count = $qr_res->getWithTemplate("^ca_objects._count");
						$vs_result_text = $vs_label_detail_link."<br/><small>".$vs_count." item".(($vs_count > 1) ? "s" : "")."</small>";
					}else{
						$vs_result_text = "<small>".$vs_idno_detail_link."</small><br/>".$vs_label_detail_link;
					}
					$vs_result_output = "
