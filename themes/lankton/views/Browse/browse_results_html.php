				...
				foreach($va_criteria as $va_criterion) {
					if ($va_criterion['facet_name'] == '_search') {
						if(strpos($va_criterion['value'], "parent_id") !== false){
							$vs_parent_search = $va_criterion['value'];
						}
					}
				}
				if($vs_parent_search){
					$vn_parent_collection_id = str_replace("ca_collections.parent_id:", "", $vs_parent_search);
					$t_parent_collection = new ca_collections($vn_parent_collection_id);
					print "<div class='detailButton'><span class='glyphicon glyphicon-file' aria-label='"._t("Download")."'></span> ".caDetailLink($this->request, "Download as PDF", "", "ca_collections",  $vn_parent_collection_id, array('view' => 'pdf', 'export_format' => '_pdf_ca_collections_summary'))."</div>";
					
					print "<H1>".$t_parent_collection->get("ca_collections.preferred_labels.name")."</H1>";
...
<?php
			if($vs_parent_search){
				print $vn_result_size." Sub-Series";	
			}else{
