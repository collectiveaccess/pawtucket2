...
<?php
		...
		while($qr_results->nextHit()) {
			if ($vn_i == 0) { print "<div class='{{{block}}}Set authoritySet'>\n"; $vb_div_open = true;}
				print "<div class='{{{block}}}Result authorityResult'>";
				
				
				
				$vn_collection_root_id = $vn_subcollection_id = null;
				$va_collection_path = array();
					
				if($qr_results->get('ca_collections.parent_id')){
					$va_title = $qr_results->get('ca_collections.hierarchy.preferred_labels', array('returnWithStructure' => true));
					foreach ($va_title as $va_collection_key => $va_collection_names) {
						foreach ($va_collection_names as $vn_collection_id => $va_collection_name) {
							foreach ($va_collection_name as $vn_key => $va_name) {
								if(!$vn_collection_root_id){
									$vn_collection_root_id = $vn_collection_id; 
								}elseif($vn_collection_root_id && !$vn_subcollection_id){
									$vn_subcollection_id = $vn_collection_id; 
								}
								if($vn_collection_id == $qr_results->get('ca_collections.collection_id')){
									$va_collection_path[] = "<b>".$va_name['name']."</b>";
								}else{
									$va_collection_path[] = $va_name['name'];
								}
							}
						}
					}
					print caDetailLink($this->request, join($va_collection_path, " > "), '', 'ca_collections',  $vn_collection_root_id, array("subcollection_id" => $vn_subcollection_id, "expandAll" => 1));
				}else{
					print $qr_results->get('ca_collections.preferred_labels.name', array('returnAsLink' => true));
				}
				
				
				print "</div><!-- end Result -->";
			$vn_count++;
			$vn_i++;
