	...
	$vs_lightbox_display_name_plural = $va_lightbox_display_name["plural"];
		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit() && ($vn_c < $vn_hits_per_block)) {
				...
				#	}
				#}
				if (($vs_table === 'ca_objects') && ($qr_res->get('ca_objects.type_id') == $vn_bib_id)) {
					$va_title_trunc = explode(":", $qr_res->get("ca_objects.preferred_labels"));
					$vs_author = $qr_res->get("ca_entities.preferred_labels", array('delimiter' => ', ', 'restrictToRelationshipTypes' => array("author")));
