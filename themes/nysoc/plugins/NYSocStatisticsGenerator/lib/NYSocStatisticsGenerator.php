...
if(true) {			
			...
			while($qr_bibs->nextHit()) {
				...
				while($qr_bibs_with_children->nextHit()) {
					// get readers
					$dates_in = $qr_bibs_with_children->get('ca_objects_x_entities.date_in', ['returnWithStructure' => true, 'rawDate' => true]);
					$dates_out = $qr_bibs_with_children->get('ca_objects_x_entities.date_out', ['returnWithStructure' => true, 'rawDate' => true]);
					
					if (is_array($dates_in)) {
						$day = 24 * 60 * 60;
						foreach($dates_in as $i => $date_in) {
						    $date_in = array_shift(array_shift($date_in));
						    $date_out = array_shift(array_shift($dates_out[$i]));
							if ((int)$date_out['start'] < 1780) { continue; }
							if ((int)$date_out['end'] > 1810) { continue; }
					
							$o_tep->setHistoricTimestamps($date_out['start'], $date_in['end']);
...
if(true) {			
			...
			while($qr_entities->nextHit()) {
				...
				// get readers
				$dates_in = $qr_entities->get('ca_objects_x_entities.date_in', ['restrictToRelationshipTypes' => ['reader'], 'returnWithStructure' => true, 'rawDate' => true]);
				$dates_out = $qr_entities->get('ca_objects_x_entities.date_out', ['restrictToRelationshipTypes' => ['reader'], 'returnWithStructure' => true, 'rawDate' => true]);
				...
				if (is_array($dates_in)) {
					foreach($dates_in as $i => $date_in) {
						...
						if ((int)$date_out['start'] < 1780) { continue; }
						if ((int)$date_out['end'] > 1805) { continue; }
						$o_tep->setHistoricTimestamps($date_out['start'], $date_in['end']);
						$vn_interval = $o_tep->interval();
