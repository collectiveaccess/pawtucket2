...
if ($start < $qr_res->numHits()) {
	...
	while($qr_res->nextHit()) {
		...
		if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($cache_key,'browse_result')){
			...
			$detail_button_link = caDetailLink($this->request, "<i class='bi bi-arrow-right-square'></i> Get Details", 'btn btn-white px-2 ms-1', $table, $id, null, array("title" => _t("View Record"), "aria-label" => _t("View Record")));
			#$detail_button_link = "<button type='button' class='btn btn-white btn-select px-2' title='"._t("View Record")."' aria-label='"._t("View Record")."'><i class='bi bi-arrow-right-square'></i></button>";
			$result_output = "<div class='col-md-6 col-lg-4 d-flex pb-4'>
								<div id='row{$id}' class='card flex-grow-1 w-100 h-100 rounded-0 shadow border-0 mb-4'>".caDetailLink($this->request, "				
									{$image}
								<div class='card-body'>
									{$caption}
								</div>", 'w-100 h-100 d-block', $table, $id)."
	...
	if(($start + $results_output) < $qr_res->numHits()) {
		print "<button class='btn btn-primary w-auto mx-auto mb-4' hx-trigger='click' hx-target='this' hx-swap='outerHTML' hx-get='".caNavUrl($this->request, '*', '*', '*', array('s' => $start + $results_output, 'key' => $browse_key, 'view' => $current_view, 'sort' => $current_sort, '_advanced' => $this->getVar('is_advanced') ? 1  : 0))."'>View More</button>";
	}
}