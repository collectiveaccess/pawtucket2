...
if (sizeof($va_criteria) > 0) {
	...
	if($vb_start_over){
		$vs_criteria .= caNavLink($this->request, _t("Clear All"), 'browseRemoveFacet btn btn-secondary btn-sm w-100 mb-2', '', 'Browse', '*', array('view' => $vs_current_view, 'key' => $vs_browse_key, 'clear' => 1, '_advanced' => $vn_is_advanced ? 1 : 0), array("role" => "button"));
	}
}
...
<?php
			if($table == "ca_objects"){
?>
			<div class="form-text px-3 pt-1 pb-3"><?= caNavLink($this->request, _t("Advanced search"), "", "", "Search", "advanced/objects"); ?></div>	
<?php
			}
?>
		</form>
<?php
	...
	if(is_array($va_sorts = $this->getVar('sortBy')) && sizeof($va_sorts)) {
		print "<H2 class='fs-4 px-3 py-2'>"._t("Sort by")."</H2>";
		print "<div class='px-3'><ul class='list-inline p-0'>\n";
		$i = 0;
