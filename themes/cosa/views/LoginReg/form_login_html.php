...
?>
<form id="LoginForm" action="<?= caNavUrl($this->request, "", "LoginReg", "login"); ?>" class="form-horizontal" method="POST">
...
<?php
	if (!$this->request->config->get(['dontAllowRegistrationAndLogin', 'dont_allow_registration_and_login']) && !$this->request->config->get('dontAllowRegistration')) {
		print '<div class="col-12 mb-2">'.caNavLink($this->request, _t("Create an account"), "", "", "LoginReg", "registerForm", array()).'</div>';
	}
