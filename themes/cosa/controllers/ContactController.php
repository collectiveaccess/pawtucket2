<?php
/* ----------------------------------------------------------------------
 * themes/cosa/controllers/ContactController.php
 * custom controller to support different email addresses for exhibitions inquire form and everything else
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
class ContactController extends BasePawtucketController {
	...
	public function Send() {
		...
		if(sizeof($errors) === 0){
				...
				$table = $this->request->getParameter("table", pString);
				if($table && ($table == "ca_occurrences")){
					$contact_email = $this->config->get("contact_email_exhibitions");
				}else{
					$contact_email = $this->config->get("contact_email");
				}
				if(caSendmail($contact_email, $from, $subject_line, $mail_message_text, $mail_message_html)){
