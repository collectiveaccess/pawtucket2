		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit() && ($vn_c < $vn_hits_per_block)) {
				...
				if($qr_res->get('parent_id')){
					...
					$vn_parent_reel = "<span > "._t("Part Of reel").": ".caNavLink($this->request, $qr_res->get("ca_objects.parent.preferred_labels.name"), '', '', 'Detail', 'objects/'.$vn_parent_id)."</span><br/>";
				}
		...
		<div class='bResultItem' onmouseover='jQuery(\"#bResultItemExpandedInfo{$vn_id}\").show();'  onmouseout='jQuery(\"#bResultItemExpandedInfo{$vn_id}\").hide();'>
			<div class='bResultItemContent'><div class='text-center bResultItemImg'>{$vs_rep_detail_link}</div>
				<div class='bResultItemText'>
					<small>{$vs_idno_detail_link}</small><br/>{$vs_label_detail_link}<br/><br/>{$vn_parent_reel}
