...
<?php 
	if (isset($va_facet_info['groupings']) && is_array($va_facet_info['groupings']) && sizeof($va_facet_info['groupings'] )) {
		print '<strong>'._t('Group by').': </strong>'; 
		$len = count($va_facet_info['groupings']);
		$i = 0;
		foreach($va_facet_info['groupings'] as $vs_grouping => $vs_grouping_label) {
			print "<a href='#' onclick='caUpdateFacetDisplay(\"{$vs_grouping}\");' class='".(($vs_grouping == $vs_grouping_field) ? 'nav-on' : '')."'>{$vs_grouping_label}</a> ";
			if($i < $len - 1) {
				print ' | ';
			}
			$i++;
...
<div id="title"><h1><?php print unicode_ucfirst($va_facet_info['label_singular']); ?></h1></div>
...
?>
	...
	<div id="browseSelectPanelHeader">
		<div id="browseSelectPanelHeaderContent">
<?php 
	$vs_g = null;
