 ...
 */
	...
	$num_all_reps = 					$t_object->getRepresentationCount(array("return_with_access" => array()));
...
<?php
			...
			if($t_object->get('idno')){
				print "<div class='unit identifier'><p>"._t("Identifier").": ".$t_object->get('idno')."</p></div><!-- end unit -->";
			}
			# --- parent hierarchy info
			if($t_object->get('parent_id')){
				print "<div class='unit'><p>"._t("Part Of").": ".caNavLink($this->request, $t_object->get("ca_objects.parent.preferred_labels.name"), '', 'Detail', 'Object', 'Show', array('object_id' => $t_object->get('parent_id')))."</p></div>";
			}
			# --- attributes
			...
			if(is_array($va_attributes) && (sizeof($va_attributes) > 0)){
				foreach($va_attributes as $vs_attribute_code){
					if($vs_value = $t_object->get("ca_objects.{$vs_attribute_code}")){
						print "<div class='unit'><p>".$t_object->getDisplayLabel("ca_objects.{$vs_attribute_code}").":</p> {$vs_value}</div><!-- end unit -->";
					}
				}
			}
			# --- description
			if($this->request->config->get('ca_objects_description_attribute')){
				if($vs_description_text = $t_object->get("ca_objects.".$this->request->config->get('ca_objects_description_attribute'))){
					print "<div class='unit description'><p><strong>".$t_object->getDisplayLabel("ca_objects.".$this->request->config->get('ca_objects_description_attribute'))."</strong><br/> {$vs_description_text}</p></div><!-- end unit -->";				
?>
...
<?php
			...
			if(sizeof($va_terms) > 0){
				...
				foreach($va_terms as $va_term_info){
					print "<div>".caNavLink($this->request, $va_term_info['label'], '', '', 'Search', 'Index', array('search' => urlencode($va_term_info['label'])))."</div>";
				}
				print "</div><!-- end unit -->";
			}
			# --- map
			...
			if (sizeof($va_related_objects)) {
				print "<div class='unit related-objects'><h3>"._t("Related Objects")."</h3>";
				print '<ul class="">';
				foreach($va_related_objects as $vn_rel_id => $va_info){
					...
					print '<li>';
					print caNavLink($this->request, $va_reps['tags']['icon'], 'thumb-link', 'Detail', 'Object', 'Show', array('object_id' => $va_info["object_id"]));
					print '</li>';
				}
				print '</ul>';
				print '<a href="#" class="btn-show-more"></a>';
				print "</div><!-- end unit -->";
...
?>
	...
	<?php
					...
					print "<a href='#' class='zoom-in' onclick='caMediaPanel.showPanel(\"".caNavUrl($this->request, 'Detail', 'Object', 'GetRepresentationInfo', array('object_id' => $t_object->get("object_id"), 'representation_id' => $t_rep->getPrimaryKey()))."\"); return false;' >".(($vn_num_reps > 1) ? _t("Zoom/more media") : _t("Zoom"))."</a>";
?>
...
<?php
			...
			if($num_all_reps > 0) {
				print '<div class="no-image-msg"><h3>Copyright-restricted media</h3></div>';
			}
		}
if (!$this->request->config->get('dont_allow_registration_and_login')) {
