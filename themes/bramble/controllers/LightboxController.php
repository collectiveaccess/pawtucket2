 ...
 */
	...
	require_once(__CA_LIB_DIR__.'/Logging/Downloadlog.php');
 	...
 	class LightboxController extends FindController {
         ...
         *
         */
 		function projects($pa_options = null) {
 			if($this->opb_is_login_redirect) { return; }

			
			$va_lightbox_displayname = caGetLightboxDisplayNameProject();
			$this->view->setVar('lightbox_displayname', $va_lightbox_displayname["singular"]);
			$this->view->setVar('lightbox_displayname_plural', $va_lightbox_displayname["plural"]);

            # Get sets for display
            $t_sets = new ca_sets();
 			#$va_read_sets = $t_sets->getSetsForUser(array("setType" => "project", "table" => "ca_objects", "user_id" => $this->request->getUserID(), "checkAccess" => $this->opa_access_values, "access" => (!is_null($vn_access = $this->request->config->get('lightbox_default_access'))) ? $vn_access : 1, "parents_only" => true));
 			#$va_write_sets = $t_sets->getSetsForUser(array("setType" => "project", "table" => "ca_objects", "user_id" => $this->request->getUserID(), "checkAccess" => $this->opa_access_values, "parents_only" => true));
            ...
            #$va_set_ids = array_merge(array_keys($va_read_sets), array_keys($va_write_sets));
 			...
 			$va_project_sets = $t_sets->getSetsForUser(array("setType" => "project", "table" => "ca_objects", "user_id" => $this->request->getUserID(), "checkAccess" => $this->opa_access_values, "parents_only" => true));
			$this->view->setVar("project_sets", $va_project_sets);
			$va_set_ids = array_keys($va_project_sets);
            ...
            MetaTagManager::setWindowTitle($this->request->config->get("app_display_name").$this->request->config->get("page_title_delimiter").ucfirst($this->ops_lightbox_display_name));
 			...
 			$this->render(caGetOption("view", $pa_options, "Lightbox/project_set_list_html.php"));
 		}
 		# ------------------------------------------------------
        /**
         *
         */
 		function setDetail($pa_options = null) {
			...
			$this->view->setVar("browse_type", "caLightbox");	// this is only used when loading hierarchy facets and is a way to get around needing a browse type to pull the table in FindController		
 			...
 			if($this->request->getParameter("view", pString) == "xlxsOrderChart"){
 				$ps_view = "xlxsOrderChart";
 			}else{
		...
		if ($ps_view !== 'timelineData') {
            ...
            MetaTagManager::setWindowTitle($this->request->config->get("app_display_name").$this->request->config->get("page_title_delimiter").ucfirst($this->ops_lightbox_display_name).$this->request->config->get("page_title_delimiter").$t_set->getLabelForDisplay());
 			switch($ps_view) {
 				...
 				case 'pptx':
 				case 'pdf':
 					$vs_export_format = $this->request->getParameter("export_format", pString);
 					#$this->_genExport($qr_res, $this->request->getParameter("export_format", pString), caGenerateDownloadFileName(caGetOption('pdfExportTitle', $va_browse_info, $vs_label = $t_set->get('ca_sets.preferred_labels.name')), ['t_subject' => $t_set]), array("criteriaSummary" => $vs_label, "set" => $t_set));
 					$this->_genExport($qr_res, $this->request->getParameter("export_format", pString), caGenerateDownloadFileName(str_replace("_pdf_", "", $vs_export_format).".pdf"), array("criteriaSummary" => $vs_label, "set" => $t_set));
 					break;
 				case 'xlxsOrderChart':
 					$this->_genExportOrderChart($qr_res, $t_set, $va_set_item_info);
 					break;
 				case 'timelineData':
         ...
         */
 		function setForm($pa_options = null) {
			...
			$t_list = new ca_lists();
			$vn_set_type_project = $t_list->getItemIDFromList('set_types', $this->request->config->get('project_set_type'));
			$this->view->setVar("project_set_type_id", $vn_set_type_project);
			...
			if($this->request->getParameter('set_id', pInteger)){
				if($t_set = $this->_getSet(__CA_SET_EDIT_ACCESS__)){
					...
					$this->view->setVar("client", $t_set->get("ca_sets.client"));
					$this->view->setVar("address1", $t_set->get("ca_sets.location.address1"));
					$this->view->setVar("city", $t_set->get("ca_sets.location.city"));
					$this->view->setVar("stateprovince", $t_set->get("ca_sets.location.stateprovince"));
					$this->view->setVar("postalcode", $t_set->get("ca_sets.location.postalcode"));
					$this->view->setVar("country", $t_set->get("ca_sets.location.country"));
				}else{
				...
				$t_set = new ca_sets();
				if($ps_mode != "project"){
					# --- pass list of user's project sets
					$va_user_projects = array();
					$va_user_projects = $t_set->getSetsForUser(array("setType" => "project", "table" => "ca_objects", "user_id" => $this->request->getUserID(), "checkAccess" => $this->opa_access_values, "access" => (!is_null($vn_access = $this->request->config->get('lightbox_default_access'))) ? $vn_access : 1, "parents_only" => true));
			
					$this->view->setVar("user_projects", $va_user_projects);
				}
         ...
         */
 		function ajaxSaveSetInfo($pa_options = null) {
            ...
            if (!$this->request->isAjax()) { $this->response->setRedirect(caNavUrl($this->request, '', 'Lightbox', 'Index')); return; }
 			...
 			# --- if mode = project we are making a parent project set
 			$ps_mode = $this->request->getParameter('mode', pString);
			...
			if(($ps_mode != "project") && ($t_set->get("type_id") != $vn_set_type_project)){
				// palette -> needs a project parent
				$pn_parent_id =  $this->request->getParameter('project_id', pInteger);
				if(!$pn_parent_id && !($pn_parent_id = $t_set->get("ca_sets.parent_id"))){
					# --- new project?
#					if($vs_project = $this->purifier->purify($this->request->getParameter("new_project_name", pString))){
# --- insert new project here to link to new set
#					}
				}
			}
 			...
 			if(sizeof($va_errors) == 0){
				...
				if($t_set->get("set_id")){
					...
					if(($ps_mode == "project") || ($t_set->get("type_id") == $vn_set_type_project)){
						$t_set->replaceAttribute(array('client' => $ps_client, 'locale_id' => $g_ui_locale_id), 'client');
						$t_set->replaceAttribute(array('address1' => $ps_address1, 'city' => $ps_city, 'stateprovince' => $ps_stateprovince, 'postalcode' => $ps_postalcode, 'country' => $ps_country, 'locale_id' => $g_ui_locale_id), 'location');
					}
					$t_set->update();
				}else{
					$t_set->set('table_num', $vn_object_table_num);
					$t_set->set('type_id', ($ps_mode == "project") ? $vn_set_type_project : $vn_set_type_user);
					...
					if(($ps_mode == "project") || ($t_set->get("type_id") == $vn_set_type_project)){
						$t_set->addAttribute(array('client' => $ps_client, 'locale_id' => $g_ui_locale_id), 'client');
						$t_set->addAttribute(array('address1' => $ps_address1, 'city' => $ps_city, 'stateprovince' => $ps_stateprovince, 'postalcode' => $ps_postalcode, 'country' => $ps_country, 'locale_id' => $g_ui_locale_id), 'location');
					}
					$t_set->insert();
					$vb_is_insert = true;
				}
				if($t_set->numErrors()) {
					...
					if(($ps_mode == "project") || ($vb_is_insert)){
 		...
 		# ------------------------------------------------------
 		function ajaxSaveSetItemInfo($pa_options = null) {
            if($this->opb_is_login_redirect) { return; }
            if (!$this->request->isAjax()) { $this->response->setRedirect(caNavUrl($this->request, '', 'Lightbox', 'Projects')); return; }
 			...
 			if (!$vn_item_id) { $this->response->setRedirect(caNavUrl($this->request, '', 'Lightbox', 'Projects')); return; }
 			
 			$t_item = new ca_set_items($vn_item_id);
 			
 			# --- get the form data
 			$ps_size =  	$this->purifier->purify($this->request->getParameter("size", pString));
 			$ps_quantity =  $this->purifier->purify($this->request->getParameter("quantity", pString));
 			$ps_notes =  	$this->purifier->purify($this->request->getParameter("notes", pString));
 			
 			$t_item->setMode(ACCESS_WRITE);
 			$t_item->replaceAttribute(array('order_size' => $ps_size, 'order_quantity' => $ps_quantity, 'order_notes' => $ps_notes), 'order_information');
			$this->view->setVar('item_id', $vn_item_id);
			
 			$t_item->update();
 			if($t_item->numErrors()) {
				$va_errors[] = join("; ", $t_item->getErrors());
				$this->view->setVar('errors', $va_errors);
			}else{
				$this->view->setVar("message", _t('Saved %1', $vs_display_name));
			}
 			$this->render("Lightbox/ajax_save_set_item_info_json.php");
 		}
 		# ------------------------------------------------------
        /**
         *
            ...
            $this->view->setVar("display_name_plural", $vs_display_name_plural);
 			...
 			if($this->request->getParameter('set_id', pInteger)){
				...
				$t_set = new ca_sets();
 				...
 				$pn_project_id = $this->purifier->purify($this->request->getParameter('project_id', pInteger));
				if($pn_project_id < 0){
					$t_list = new ca_lists();
					$vn_set_type_project = $t_list->getItemIDFromList('set_types', $this->request->config->get('project_set_type'));
					# --- making a new project - was the name entered
					$ps_project_name = $this->purifier->purify($this->request->getParameter('project_name', pString));
					if(!$ps_project_name){
						$ps_project_name = _t("Your project");
					}
					$t_project = new ca_sets();
					
					$t_project->setMode(ACCESS_WRITE);
					$t_project->set('access', (!is_null($vn_access = $this->request->config->get('lightbox_default_access'))) ? $vn_access : 1);
					$t_project->set('table_num', $vn_object_table_num);
					$t_project->set('type_id', $vn_set_type_project);
					$t_project->set('user_id', $this->request->getUserID());
					$t_project->set('set_code', $this->request->getUserID().'_project_'.time());
					$t_project->insert();
					if($t_project->numErrors()) {
						$va_errors["general"] = join("; ", $t_project->getErrors());
						$this->view->setVar('errors', $va_errors);
						$this->addItemForm();
						return;
					}else{
						// save name - add new label
						$t_project->addLabel(array('name' => $ps_project_name), $g_ui_locale_id, null, true);
						$pn_project_id = $t_project->get("set_id");
					}
				}
				...
				$t_set->set('user_id', $this->request->getUserID());
				$t_set->set('parent_id', $pn_project_id);
				$t_set->set('set_code', $this->request->getUserID().'_'.time());
				// create new attribute
				if($ps_description){
					$t_set->addAttribute(array($this->ops_description_attribute => $ps_description, 'locale_id' => $g_ui_locale_id), $this->ops_description_attribute);
			...
			while($qr_res->nextHit()) {
				...
				foreach($va_reps as $vn_representation_id => $va_rep) {
					...
					switch($this->request->user->getPreference('downloaded_file_naming')) {
						...
						default:
							$va_tmp = array();
							if($vs_tmp = $t_instance->get("genus")){
								$va_tmp[] = $vs_tmp;
							}
							if($vs_tmp = $t_instance->get("species")){
								$va_tmp[] = $vs_tmp;
							}
							if($vs_tmp = $t_instance->get("variety")){
								$va_tmp[] = $vs_tmp;
							}
							$vs_file_name = str_replace(" ", "_", join("_", $va_tmp));
							$vs_file_name = str_replace("'", "", $vs_file_name);
							$vs_file_name = $vs_file_name.(($vn_c > 1) ? "_".$vn_c : "").".".$va_rep_info['EXTENSION'];
						break;
					} 
		 ...
		 * options: criteriaSummary, set
		 */
		protected function _genExport($po_result, $ps_template, $ps_output_filename, $pa_options = array()) {
