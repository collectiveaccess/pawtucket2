 ...
 */
	...
	$va_write_project_sets = $t_set->getSetsForUser(array("table" => "ca_objects", "user_id" => $this->request->getUserID(), "setType" => "project", "access" => 2));
...
<?php
		...
		if(is_array($va_write_project_sets) && sizeof($va_write_project_sets)){
			...
			foreach($va_write_project_sets as $va_write_project){
				$t_write_set->load($va_write_project["set_id"]);
				...
				if($vs_palettes){
					print "<option value='".$va_write_project["set_id"]."' disabled>".$t_write_set->getLabelForDisplay()."</option>\n";
					print $vs_palettes;
				}
				$vs_project_dropdowns .= "<option value='".$va_write_project["set_id"]."'>".$t_write_set->getLabelForDisplay()."</option>\n";
			}
			print "</select>\n";
		...
		print "<div class='form-group'><label for='project' class='col-sm-4 control-label'>"._t("Project")."</label><div class='col-sm-7'><select id='project_id' name='project_id' class='form-control'>".$vs_project_dropdowns."<option value='-1'>Create a New Project</option></select></div><!-- end col-sm-7 --></div><!-- end form-group -->\n";
		print "<div id='newProject' class='form-group' ".(($vs_project_dropdowns) ? "style='display:none;'" : "")."><div class='col-sm-7 col-sm-offset-4'><input type='text' name='project_name' placeholder='"._t("New Project Name")."' class='form-control'></div><!-- end col-sm-7 --></div><!-- end form-group -->\n";
