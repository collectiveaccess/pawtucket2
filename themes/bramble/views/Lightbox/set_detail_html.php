...
?>
							...
							<li><a href='#' data-set_id="<?php print $vn_set_id; ?>" data-set_name="<?php print addslashes($t_set->get("ca_sets.preferred_labels")); ?>" data-toggle='modal' data-target='#confirm-delete'>Delete Palette</a></li>
<?php
						}
?>
					...
					if ($vn_start < $qr_set_item_objects->numHits()) {
						...
						if($qr_set_item_objects->numHits()){
								...
								if(is_array($va_set_item_info[$vn_object_id])) {
									foreach ($va_set_item_info[$vn_object_id] as $va_item_info) {
										...
										$va_items[$va_item_info['item_id']] = array(
											...
											'notes' => $va_set_item_metadata[$vn_object_id]['notes'],
											'quantity' => $va_set_item_metadata[$vn_object_id]['quantity'],
											'size' => $va_set_item_metadata[$vn_object_id]['size']
										);
									}
								}
								$vn_c++;
							...
							foreach($va_item_ids as $vn_i => $vn_item_id) {
								...
								$this->setVar('quantity', $va_items[$vn_item_id]['quantity']);
...
?>
		<hr/>
		<b>Help Tips</b>
		<ul class="paletteHelp">
			<li><b>Drag and Drop</b> plants into your preferred order</li>
			<li>Use the <b>Download</b> links above to export your palette in a variety of formats</li>
			<li>Show as list <span class="glyphicon glyphicon-list"></span> to enter size and quantity for each plant</li>
		</ul>
		</div><!-- end col -->
	</div><!-- end row -->
...
<div id="palettePlaceholder" style="display:none;">
	<br/><br/><?php print caNavLink($this->request, 'Back to Projects List', 'btn btn-default', '', 'Lightbox', 'Projects', array()); ?>
</div>
