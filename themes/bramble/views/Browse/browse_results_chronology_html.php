		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit()) {
				...
				if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result')){
					...
					$vs_label_detail_link 	= caDetailLink($this->request, "<span class='bResultListItemSimpleTitle'><i>".$vs_label.(($vs_variety) ? " ".$vs_variety." " : "")."</i></span>", '', $vs_table, $vn_id);
					$vs_color_chart = caColorChart($qr_res);
					$vs_result_output = "<div class='container'><div class='row'><div class='col-sm-3 btccResultLabel'>".$vs_label_detail_link."</div><div class='col-sm-9'>".$vs_color_chart."</div></div></div>";
