...
<?php
			...
			if(is_array($va_add_to_set_link_info) && sizeof($va_add_to_set_link_info)){
				...
				print "&nbsp;&nbsp;&nbsp;<a href='#' class='bSetsSelectAll' onClick='$(\".bSetsSelectMultiple input\").prop(\"checked\", true); ' return false;>Select All</a><span class='bSetsSelectAll'> / </span><a href='#' class='bSetsSelectAll' onClick='$(\".bSetsSelectMultiple input\").prop(\"checked\", false);' return false;>Deselect All</a>";
			}
?>
		</H1>
		...
		if (sizeof($va_criteria) > 0) {
			...
			foreach($va_criteria as $va_criterion) {
				...
				if ($va_criterion['facet_name'] != '_search') {
					print caNavLink($this->request, '<button type="button" class="btn btn-default btn-med">'.$va_criterion['value'].' <span class="glyphicon glyphicon-remove-circle"></span></button>', 'browseRemoveFacet', '*', '*', '*', array('removeCriterion' => $va_criterion['facet_name'], 'removeID' => urlencode($va_criterion['id']), 'view' => $vs_current_view, 'key' => $vs_browse_key));
				}else{
