<?php
/* ----------------------------------------------------------------------
 * controllers/ExhibitionController.php
 * ----------------------------------------------------------------------
 * CollectiveAccess
 ...
 */
	...
	require_once(__CA_LIB_DIR__.'/pawtucket/BasePawtucketController.php');
 	...
 	class ExhibitionController extends BasePawtucketController {
 		# -------------------------------------------------------
 		public function __construct(&$po_request, &$po_response, $pa_view_paths=null) {
 			AssetLoadManager::register("carousel");
 			parent::__construct($po_request, $po_response, $pa_view_paths);
 			#$this->config = caGetFrontConfig();
 			caSetPageCSSClasses(array("exhibition"));
 		...
 		public function __call($ps_function, $pa_args) {
 			...
 			#
 			# --- displays the set configured to show on the front page, load it now --- IT'S AND ENTITY SET
 			#
 			$va_featured_ids = array();
 			if($vs_set_code = $this->request->config->get("exhibition_set_code")){
 				$t_set = new ca_sets();
				...
				if((sizeof($va_access_values) == 0) || (sizeof($va_access_values) && in_array($t_set->get("access"), $va_access_values))){
					...
					$this->view->setVar('featured_set_items_as_search_result', caMakeSearchResult('ca_entities', $va_featured_ids));
				}
 			}
