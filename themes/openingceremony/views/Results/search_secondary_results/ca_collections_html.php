...
<?php
					if ($vn_num_hits > $vn_num_hits_per_page) {
						print "<div class='nav'>";
						if ($vn_page > 0) {
							print "<a href='#' onclick='jQuery(\"#collectionsSecondaryResults\").load(\"".caNavUrl($this->request, '', 'Search', 'secondarySearch', array('spage' => $vn_page - 1, 'type' => 'ca_collections'))."\"); return false;'>&lsaquo; "._t("Previous")."</a>";
						}
						if (($vn_page > 0) && ($vn_page < (ceil($vn_num_hits/$vn_num_hits_per_page) - 1))) {
							print " | ";
						}
						if ($vn_page < (ceil($vn_num_hits/$vn_num_hits_per_page) - 1)) {
							print "<a href='#' onclick='jQuery(\"#collectionsSecondaryResults\").load(\"".caNavUrl($this->request, '', 'Search', 'secondarySearch', array('spage' => $vn_page + 1, 'type' => 'ca_collections'))."\"); return false;'>"._t("Next")." &rsaquo;</a>";
						}
						print "</div><!-- end nav -->\n";
					}
					print _t("Your search found %1 %2.", $qr_collections->numHits(), ($qr_collections->numHits() == 1) ? _t('result') : _t('results'));
?>
...
<?php
					...
					while($qr_collections->nextHit()) {
						...
						if ($vb_link_to_collection_detail) {
							...
							print caNavLink($this->request, $vs_name, '', '', 'Search', 'Index', array('search' => urlencode($vs_name)));
						}
						print "<br/>\n";
						$vn_c++;
