...
if($vo_result) {
	...
	if ($this->getVar('page') < $this->getVar('num_pages')) {
		print "<div id='moreOccResults".$this->getVar('page')."'><div class='item more'><a href='#' onclick='jQuery(\"#moreOccResults".$this->getVar('page')."\").load(\"".caNavUrl($this->request, '', $this->request->getController(), 'Index', array('page' => $this->getVar('page') + 1))."\"); return false;'>"._t("More Results")."</a></div></div>";
	}
	if (!$this->request->isAjax()) {
		print "</div>\n";
	}
