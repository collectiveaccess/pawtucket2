...
<?php
					...
					if ($vn_page < (ceil($vn_num_hits/$vn_num_hits_per_page) - 1)) {
						print "<div id='moreCollectionsResults".$vn_page."'><div class='item more'><a href='#' onclick='jQuery(\"#moreCollectionsResults".$vn_page."\").load(\"".caNavUrl($this->request, '', 'Search', 'secondarySearch', array('spage' => $vn_page + 1, 'type' => 'ca_collections'))."\"); return false;'>"._t("More Results")."</a></div></div>";
					}
			if (!$this->request->isAjax()) {
?>
					</div><!-- end listItems -->
