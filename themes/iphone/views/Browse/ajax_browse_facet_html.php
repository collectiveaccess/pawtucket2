...
<div id="facetList">
<div id="facetTitle">
	<?php print caNavLink($this->request, _t('start over')." &rsaquo;", '', '', 'Browse', 'clearCriteria', array()); ?>
	<?php print unicode_ucfirst($va_facet_info['label_plural']); ?>
</div>
<?php
...
?>
<?php
				print "<div class='item'>".caNavLink($this->request, $va_item['label'], 'browseSelectPanelLink', $this->request->getModulePath(), $this->request->getController(), ((strlen($vm_modify_id)) ? 'modifyCriteria' : 'addCriteria'), array_merge(array('facet' => $vs_facet_name, 'id' => $va_item['id'], 'mod_id' => $vm_modify_id), $va_other_params))."</div>";
			}
?>
	</div>
<?php
		...
		default:
			...
			# --- only show letter bar if grouping mode is label
			if($vn_show_jump_bar){
?>
				<div id="jumpBar">
<?php 
				foreach($va_groups as $vs_group) {
					...
					print " <a href='#browse_group_{$vs_group_proc}'>{$vs_group}</a><br/>";
				}
?>
				</div>
<?php
...
?>
	<div class="listItems"<?php print ($vn_show_jump_bar) ? ' id="facetsWithJumpBar"' : ''; ?>>
<?php
			foreach($va_grouped_items as $vs_group => $va_items) {
				...
				$vs_group_proc = preg_replace("![^A-Za-z0-9]+!", "_", $vs_group);
				print "<div class='listHeading' id='browse_group_{$vs_group_proc}'><a name='{$vs_group_proc}'>{$vs_group}</a></div>\n";
				foreach($va_items as $va_item) {
					print "<div class='item'>".caNavLink($this->request, $va_item['label'], '', $this->request->getModulePath(), $this->request->getController(), ((strlen($vm_modify_id) > 0) ? 'modifyCriteria' : 'addCriteria'), array_merge(array('facet' => $vs_facet_name, 'id' => $va_item['id'], 'mod_id' => $vm_modify_id), $va_other_params))."</div>";
				}
			}
?>
	</div>
