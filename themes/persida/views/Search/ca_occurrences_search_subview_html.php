...
<?php
		...
		while($qr_results->nextHit()) {
			if ($vn_i == 0) { print "<div class='{{{block}}}Set authoritySet'>\n"; $vb_div_open = true; }
				print "<div class='{{{block}}}Result authorityResult'><div>".$qr_results->get('ca_occurrences.preferred_labels.name', array('returnAsLink' => true))."</div>";
				if ($vs_venue = $qr_results->get('ca_entities.preferred_labels', array('restrictToRelationshipTypes' => array('venue')))) {
					print "<div>".caDetailLink($this->request, $vs_venue, '', 'ca_occurrences', $qr_results->get('ca_occurrences.occurrence_id'))."</div>";
				} else {
					$vs_venue = null;
				}
				if ($vs_author = $qr_results->get('ca_occurrences.author')) {
					print "<div>".caDetailLink($this->request, $vs_author, '', 'ca_occurrences', $qr_results->get('ca_occurrences.occurrence_id'))."</div>";
				} else {
					$vs_author = null;
				}	
				if ($vs_edition = $qr_results->get('ca_occurrences.edition_bib')) {
					print "<div>".caDetailLink($this->request, $vs_edition, '', 'ca_occurrences', $qr_results->get('ca_occurrences.occurrence_id'))."</div>";
				} else {
					$vs_edition = null;
				}											
				if ($vs_date = $qr_results->get('ca_occurrences.occurrence_dates')) {
