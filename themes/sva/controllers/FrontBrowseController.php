<?php
/* ----------------------------------------------------------------------
 * controllers/FrontBrowseController.php
 * ----------------------------------------------------------------------
 * CollectiveAccess
 ...
 */
	...
	require_once(__CA_MODELS_DIR__."/ca_objects.php");
 	...
 	class FrontBrowseController extends FindController {
 		# -------------------------------------------------------
 		/**
 		 *
 		 */
 		 ...
 		 *
 		 */
 		protected $ops_view_prefix = 'FrontBrowse';
 		# -------------------------------------------------------
 		...
 		public function __call($ps_function, $pa_args) {
			...
			$this->view->setVar('row_id', $pn_row_id = $this->request->getParameter('row_id', pInteger));
 			...
 			if (!$this->request->getParameter('getFacet', pString) && !$this->request->getParameter('dontSetFind', pInteger)) {
			...
			$this->view->setVar('paging', in_array(strtolower($va_browse_info['paging']), array('continous', 'nextprevious', 'letter')) ? strtolower($va_browse_info['paging']) : 'continous');
 			...
 			if(!is_array($va_views) || (sizeof($va_views) == 0)){
 				$va_views = array('list' => array(), 'images' => array(), 'chronology' => array(), 'chronology_images' => array(), 'timeline' => array(), 'timelineData' => array(), 'pdf' => array(), 'xlsx' => array(), 'pptx' => array());
 			} else {
			...
			//	
			if ($this->request->getParameter('getFacet', pString)) {
