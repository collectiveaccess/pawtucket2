 ...
 * 
 */	
function caGetCollectionLevelSummarySVA($po_request, $va_collection_ids, $vn_level) {
	$va_access_values = caGetUserAccessValues($po_request);
	# --- get collections configuration
	$o_collections_config = caGetCollectionsConfig();
	$va_object_access_values = $o_collections_config->get("export_object_access_values");
	...
	if($qr_collections->numHits()){
		while($qr_collections->nextHit()) {
			...
			if(sizeof($va_child_ids)) {
				$vs_output .=  caGetCollectionLevelSummarySVA($po_request, $va_child_ids, $vn_level + 1);
			}
		}
	}
	return $vs_output;
