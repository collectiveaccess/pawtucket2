...
?>					
				...
				<div class="row" id="collectionsWrapper">
					<div class='h-100 overflow-y-auto col-12<?php print ($o_collections_config->get("always_link_to_hierarchy_viewer_sublist") || $vb_has_grandchildren) ? " col-md-4" : ""; ?>'>
						<div class='mx-3'>
					...
					if($qr_collection_children->numHits()){
						...
						while($qr_collection_children->nextHit()) {
							...
							if($vn_rel_object_count){
								$vs_record_count = "<br aria-hidden='true' /><small class='ms-2 fw-normal'>(".$vn_rel_object_count." image".(($vn_rel_object_count == 1) ? "" : "s").")</small>";
							}
							if($vb_link_sublist || $o_collections_config->get("always_link_to_hierarchy_viewer_sublist")){
								...
								print "<button data-bs-toggle='pill' type='button' role='tab' aria-selected='".(($c == 1) ? "true" : "false")."' aria-controls='#collectionLoad' class='btn w-100 text-start p-3 d-block bg-body-tertiary text-black fw-medium".(($c == 1) ? " active" : "")."' hx-target='#collectionLoad' hx-get='".caNavUrl($this->request, '', 'Collections', 'childList', array('collection_id' => $qr_collection_children->get("ca_collections.collection_id")))."'>".$vs_icon." ".$qr_collection_children->get('ca_collections.preferred_labels').$vs_record_count."</button>";
							}else{
...
?>
							<div class='h-100 overflow-y-auto col-12 col-md-8 border-start'><div id='collectionLoad' class='mx-3'hx-trigger="load" hx-target="#collectionLoad" hx-get="<?php print caNavUrl($this->request, '', 'Collections', 'childList', array('collection_id' => $first_sub_collection_id)); ?>">
								Loading...
							</div></div>
