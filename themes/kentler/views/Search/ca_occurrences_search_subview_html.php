...
<?php
		...
		while($qr_results->nextHit()) {
			$va_ids[] = $qr_results->get('ca_occurrences.occurrence_id');
		}
		$qr_results->seek($vn_start);
		
		$va_images = caGetDisplayImagesForAuthorityItems('ca_occurrences', $va_ids, array('version' => 'widepreview', 'relationshipTypes' => caGetOption('selectMediaUsingRelationshipTypes', $va_options, null), 'checkAccess' => $va_access_values));
...
<?php
			...
			print $vs_image_tag;
?>
				<br/><?php print $qr_results->getWithTemplate('<l>^ca_occurrences.preferred_labels.name<br/>^ca_occurrences.exhibition_dates</l>'); ?>
			</div>
<?php
			$vn_count++;
