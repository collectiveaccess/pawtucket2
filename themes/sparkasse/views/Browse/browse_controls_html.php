...
<?php
						...
						foreach($va_available_facets as $vs_facet_code => $va_facet_info) {
							print "<a href='#' onclick='caUIBrowsePanel.showBrowsePanel(\"{$vs_facet_code}\");' class='facetLink'>".ucwords($va_facet_info['label_singular'])."</a>";
...
<?php
				...
				if (sizeof($va_facets)) { 
					print "<div id='browseFacetDiv'><span class='startBrowsingBy'>Suche nach:</span>";
					$va_available_facets = $this->getVar('available_facets');
					foreach($va_available_facets as $vs_facet_code => $va_facet_info) {
						print "<a href='#' onclick='caUIBrowsePanel.showBrowsePanel(\"{$vs_facet_code}\");'>".$va_facet_info['label_singular']."</a>\n";
					}
					print "</div><!-- end browseFacetDiv -->";
				}
