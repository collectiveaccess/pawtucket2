...
<?php
		...
		while($qr_results->nextHit()) {
			if ($vn_i == 0) { print "<div class='{{{block}}}Set authoritySet'>\n"; $vb_div_open = true; }
				...
				print caNavLink($this->request, (strlen($qr_results->get('ca_occurrences.preferred_labels.name')) > 100 ? substr($qr_results->get('ca_occurrences.preferred_labels.name'), 0, 97)."..." : $qr_results->get('ca_occurrences.preferred_labels.name')), '', '', 'Detail', 'occurrences/'.$vn_id);
				$vs_type = $qr_results->get('ca_occurrences.type_id', array('convertCodesToDisplayText' => true));
				if ($vs_type == "Exhibition") {
					$vs_date = null;	
					if ($vs_date = $qr_results->get('ca_occurrences.date.display_date')) {
						print "<p>".$vs_date."</p>";
					} elseif($vs_date = $qr_results->get('ca_occurrences.date.parsed_date')) {
						print "<p>".$vs_date."</p>";
					}
				} else {
					$va_pub_info = null;
					if ($vs_author = $qr_results->get('ca_entities.preferred_labels', array('restrictToRelationshipTypes' => array('author'), 'delimiter' => ', '))) {
						$va_pub_info[] = $vs_author;  
					}
					if ($vs_place = $qr_results->get('ca_occurrences.bib_place_published')) {
						$va_pub_info[] = $vs_place;
					}
					if ($vs_date = $qr_results->get('ca_occurrences.bib_year_published')) {
						$va_pub_info[] = $vs_date;
					}
					print "<p>".join(', ', $va_pub_info)."</p>";
				}
