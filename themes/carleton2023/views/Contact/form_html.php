 <?php
	...
	$vs_inquiry_type = $this->getVar("inquiry_type");
?>
...
?>
	<form id="contactForm" action="<?php print caNavUrl($this->request, "", "Contact", "send"); ?>" role="form" method="post">
	    ...
	    <div class="row">
			<div class="col-md-9">
<?php
			if($vs_inquire_intro = $this->getVar("inquire_intro")){
				print "<p>".$vs_inquire_intro."</p>";
			}
?>
			</div>
		</div>
...
?>
		<div class="row">
			<div class="col-md-9">
				...
				<div class="row">
					<div class="col-sm-6">
						<div class="form-group<?php print (($va_errors["type"]) ? " has-error" : ""); ?>">
							<label for="type">Type of inquiry</label>
							<select name="inquiry_type" class="form-control input-sm">
								<option value="Schedule an appointment"<?php print ($vs_inquiry_type == "Schedule an appointment") ? " selected" : ""; ?>>Schedule an appointment</option>	
								<option value="Request further information"<?php print ($vs_inquiry_type == "Request further information") ? " selected" : ""; ?>>Request further information</option>
								<option value="Request high-resolution image or video"<?php print ($vs_inquiry_type == "Request high-resolution image or video") ? " selected" : ""; ?>>Request high-resolution image or video</option>
								<option value="Other"<?php print ($vs_inquiry_type == "Other") ? " selected" : ""; ?>>Other</option>
							</select>
						</div>
					</div><!-- end col -->
					<div class="col-sm-6">
