						...
						while($qr_collection_children->nextHit()) {
							...
							if($vb_link_sublist){
								...
								if($vb_link_to_detail){
									print caDetailLink($this->request, $vs_icon." ".$qr_collection_children->getWithTemplate($vs_label_template)." ".(($o_collections_config->get("link_out_icon")) ? $o_collections_config->get("link_out_icon") : ""), '', 'ca_collections',  $qr_collection_children->get("ca_collections.collection_id")).$vs_record_count;
								}else{
									print "<div class='listItem'>".$vs_icon." ".$qr_collection_children->getWithTemplate($vs_label_template).$vs_record_count."</div>";
								}
							}
							print "</div>";	
							if($vb_link_sublist){
