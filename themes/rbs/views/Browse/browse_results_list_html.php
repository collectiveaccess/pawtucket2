		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit()) {
				...
				if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result')){
					...
					$vs_info = "";
					if ($vs_author_name = $qr_res->get('ca_entities.preferred_labels', array('restrictToRelationshipTypes' => array('author'), 'delimiter' => ', '))) {
						$vs_author = "<p>".$vs_author_name."</p>";
					}
					if ($vs_place_name = $qr_res->get('ca_objects.place')) {
						$vs_info.= "<p>".$vs_place_name."<p>";
					}
					if ($vs_publisher_name = $qr_res->get('ca_entities.preferred_labels', array('restrictToRelationshipTypes' => array('publisher'), 'delimiter' => ', '))) {
						$vs_info.= "<p>".$vs_publisher_name."</p>";
					}
					if ($vs_date = $qr_res->get('ca_objects.date')) {
						$vs_info.= "<p>".$vs_date."<p>";
					}	
					if ($vs_call_no = $qr_res->get('ca_objects.call_number')) {
						$vs_info.= "<p>".$vs_call_no."<p>";
					}
					if ($vs_collection = $qr_res->get('ca_collections.preferred_labels', array('delimiter' => ', '))) {
						$vs_info.= "<p>".$vs_collection."</p>";
					}	
					if ($vs_storage = $qr_res->get('ca_storage_locations.preferred_labels', array('delimiter' => ', '))) {
						$vs_info.= "<p>".$vs_storage."</p>";
					}
					if ($vs_notes = $qr_res->get('ca_objects.notes', array('delimiter' => '<br/>'))) {
						$vs_info.= "<p>".$vs_notes."</p>";
					}					
