...
<?php 
				...
				$vs_info = array();
				if ($vs_author = $qr_results->get('ca_entities.preferred_labels', array('restrictToRelationshipTypes' => array('author')))) {
					$vs_info[] = $vs_author; 
				}
				$vs_info[] = $qr_results->get('ca_objects.date');
				print "<p>".join(', ', $vs_info)."</p>";
?>
			</div><!-- end blockResult -->
<?php
			$vn_count++;
