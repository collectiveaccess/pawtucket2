...
// 						print "<div class='unit'><label>Nationality</label>".$vs_nationality."</unit></div>";
// 					}
					if ($t_item->get('ca_entities.vital_dates.vital_date_value')) {
						...
						print "<div class='unit'><label>Vital dates</label>".$vs_vital."</unit></div>";
					}
...
// 						print "<div class='unit'><label>Gender</label>".$vs_gender."</unit></div>";
// 					}
					if ($vs_role = $t_item->get('ca_entities.roleCreator', array('delimiter' => '<br/>'))) {
						print "<div class='unit'><label>Role</label>".$vs_role."</unit></div>";
					}	
					if ($vs_bio = $t_item->get('ca_entities.biography')) {
						print "<div class='unit'><label>Biography</label>".$vs_bio."</unit></div>";
					}	
					if ($vs_sourcebio = $t_item->get('ca_entities.biography_source')) {
						print "<div class='unit'><label>Source of Biography</label>".$vs_sourcebio."</unit></div>";
					}
...
?>
					<div id="detailTools">
						<div class="detailTool"><a href='#' onclick='jQuery("#detailComments").slideToggle(); return false;'><span class="glyphicon glyphicon-comment"></span>Comments (<?php print print is_array($va_comments) ? sizeof($va_comments) : 0; ?>)</a></div><!-- end detailTool -->
