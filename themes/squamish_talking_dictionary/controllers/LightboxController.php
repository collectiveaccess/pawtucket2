...
class LightboxController extends FindController {
	...
	function index($pa_options = null) {
		...
		$va_read_sets = $t_sets->getSetsForUser(array("setType" => $vn_set_type_user, "table" => "ca_objects", "user_id" => $this->request->getUserID(), "checkAccess" => $this->opa_access_values, "access" => (!is_null($vn_access = $this->request->config->get('lightbox_default_access'))) ? $vn_access : 1, "parents_only" => true, 'sort' => $sort, 'sortDirection' => $sort_direction));
		$va_write_sets = $t_sets->getSetsForUser(array("setType" => $vn_set_type_user, "table" => "ca_objects", "user_id" => $this->request->getUserID(), "checkAccess" => $this->opa_access_values, "parents_only" => true, 'sort' => $sort, 'sortDirection' => $sort_direction));
	...
	function ajaxSaveSetInfo($pa_options = null) {
		...
		caValidateCSRFToken($this->request, null, ['exceptions' => true]);
