...
function printLevelArchivalCollectionExport($po_request, $va_collection_ids, $o_config, $vn_level) {
	...
	if($qr_collections->numHits()){
		while($qr_collections->nextHit()) {			
			# --- related objects?
			$vn_rel_object_count = sizeof($qr_collections->get("ca_objects.object_id", array('excludeTypes' => array('archival_file'), "returnAsArray" => true, 'checkAccess' => $va_access_values)));
				...
				if(sizeof($va_child_file_ids)){
						...
						if($qr_collection_children_file->numHits()){
							while($qr_collection_children_file->nextHit()){
								...
								$vn_rel_object_count_file = sizeof($qr_collection_children_file->get("ca_objects.related.object_id", array('excludeTypes' => array('archival_file'), 'returnAsArray' => true, 'checkAccess' => $va_access_values)));
								$vs_output .= $qr_collection_children_file->get("ca_objects.type_id", array("convertCodesToDisplayText" => true)).": ".$qr_collection_children_file->get("ca_objects.preferred_labels.name");
