...
$acl_info			= $qr_res->prefetchAclAccess();
$rep_acl_info 		= $qr_res->prefetchAclAccessForRelated('ca_object_representations');
...
$vs_default_placeholder_tag = "<div class='bResultItemImgPlaceholder'>".$vs_default_placeholder."</div>";
	...
	if ($vn_start < $qr_res->numHits()) {
		...
		while($qr_res->nextHit()) {
			...
			if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result')){
				...
				if(
					($acl_info[$vn_id] ?? null) && (in_array($acl_info[$vn_id]['source'], ['USER_EXCEPTION', 'GROUP_EXCEPTION'], true))
					||
					(sizeof(array_filter($rep_ids, function($rep_id) use($rep_acl_info) {
						return (isset($rep_acl_info[$rep_id]) && in_array($rep_acl_info[$rep_id]['source'], ['USER_EXCEPTION', 'GROUP_EXCEPTION']));
					})) > 0)
				) {
	...
	<div class='bResultItemCol col-xs-{$vn_col_span_xs} col-sm-{$vn_col_span_sm} col-md-{$vn_col_span}'>
		<div class='bResultItem{$restricted_class}' id='row{$vn_id}' onmouseover='jQuery(\"#bResultItemExpandedInfo{$vn_id}\").show();'  onmouseout='jQuery(\"#bResultItemExpandedInfo{$vn_id}\").hide();'>
