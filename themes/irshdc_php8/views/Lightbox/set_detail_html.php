...
<?php
            ...
            if ($vn_num_comments > 0) {
                ...
                while ($qr_comments->nextHit()) {
                    $va_comments[] = array("comment_id" => $qr_comments->get('ca_item_comments.comment_id'), 'comment' => $qr_comments->get('ca_item_comments.comment'), 'author' => $qr_comments->get('ca_users.fname') . ' ' . $qr_comments->get('ca_users.lname') . ' ' . $qr_comments->get('ca_item_comments.created_on'), 'is_author' => $this->getVar('user_id') == $this->request->user->get("user_id"));
                }
                $va_comments = array_reverse($va_comments);
            	foreach($va_comments as $va_comment){
                    $this->setVar('comment_id', $va_comment['comment_id']);
                    $this->setVar('comment', $va_comment['comment']);
                    $this->setVar('author', $va_comment['author']);
                    $this->setVar('is_author', $va_comment['is_author']);
        ...
        jQuery("#addComment").on('submit', function(e) {
            jQuery.getJSON('<?php print caNavUrl($this->request, '', 'Lightbox', 'AjaxAddComment'); ?>', {'id': '<?php print $t_set->get("set_id"); ?>', 'type': 'ca_sets', 'comment': jQuery("#addCommentTextArea").val(), 'csrfToken': <?= json_encode(caGenerateCSRFToken($this->request)); ?> } , function(data) {
                if(data.status == 'ok') {
                    ...
                    jQuery('.lbComments').prepend(data.comment).show();
