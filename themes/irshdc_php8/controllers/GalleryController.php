	...
	require_once(__CA_LIB_DIR__.'/pawtucket/BasePawtucketController.php');
 	...
 	class GalleryController extends BasePawtucketController {
 			...
 			if($ps_function == "index"){
 				if($vn_gallery_set_type_id){
					...
					if($this->config->get('gallery_sort_by') !== 'name') {
						$va_sets = caSortArrayByKeyInValue($va_sets, ['set_id'], 'DESC');
					}
				...
				if (!$this->request->isAjax()){
 				...
 				switch($vs_display) {
					case 'timeline':
						...
						break;
					case 'storymap':
						$this->render('Gallery/set_detail_storymap_html.php');
						break;
					case 'map':
		...
		public function getSetInfoAsJSON() {
			...
			switch($ps_mode) {
				case 'storymap':
					$this->getView()->setVar('view', 'storymap');
					$this->render('Gallery/set_detail_storymap_json.php');
				break;
				# --------------------------------------------------------
				case 'timeline':
				default:
