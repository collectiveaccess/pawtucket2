...
?>	
		<div class="col-sm-<?php print ($ps_description) ? "8" : "12"; ?>">
			<div id="galleryDetailImageGrid" class="row">		
<?php
		$vn_i = 0;
		foreach($pa_set_items as $pa_set_item){
			...
			if($pa_set_item["representation_tag_".$vs_icon]){
				...
				print "<a href='#' id='galleryIcon".$pa_set_item["item_id"]."' onclick='jQuery(\"#galleryDetailImageArea\").load(\"".caNavUrl($this->request, '', 'Gallery', 'getSetItemRep', array('item_id' => $pa_set_item["item_id"], 'set_id' => $pn_set_id))."\"); jQuery(\"#galleryDetailObjectInfo\").load(\"".caNavUrl($this->request, '', 'Gallery', 'getSetItemInfo', array('item_id' => $pa_set_item["item_id"], 'set_id' => $pn_set_id))."\"); galleryHighlightThumbnail(\"galleryIcon".$pa_set_item["item_id"]."\"); window.scrollTo(0, 108); return false;'>".$vs_rep."</a>";
				print "</div>\n";
...
<?php
                if($vn_i > 12){
					print "<div class='row'><div class='col-sm-12 text-center' id='moreLink'>
								<button class='btn btn-default btn-lg' onclick='$(\".galleryIconHidden\").removeClass(\"galleryIconHidden\"); $(\"#moreLink\").hide(); return false;'><span class='glyphicon glyphicon-arrow-down'></span> ".(sizeof($pa_set_items) - 12)." "._t("more")." <span class='glyphicon glyphicon-arrow-down'></span></button>
							</div></div>";
				}
?>
		</div><!-- end col -->
