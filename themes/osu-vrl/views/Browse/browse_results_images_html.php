		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit()) {
				...
				if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result')){
					...
					$vs_creationDate = $qr_res->get("{$vs_table}.dateSet.displayDate", ["delimiter" => ', ']);
					$vs_related_entities = $qr_res->get("ca_entities.preferred_labels", ["delimiter" => ', ', "restrictToTypes" => "artist"]);
					$vs_workDetail = "";
					if($vs_related_entities){ 
					    $vs_workDetail .= "<br/><strong>Artist</strong><br/>{$vs_related_entities}";
					}
					if($vs_creationDate){
					    $vs_workDetail .= "<br/><strong>Date(s)</strong><br/>{$vs_creationDate}";
					}
					$vs_thumbnail = "";
					$vs_type_placeholder = "";
					$vs_typecode = "";
			...
			<div class='bResultItem' id='row{$vn_id}' onmouseover='jQuery(\"#bResultItemExpandedInfo{$vn_id}\").show();'  onmouseout='jQuery(\"#bResultItemExpandedInfo{$vn_id}\").hide();'>
				...
				<div class='bResultItemContent'><div class='text-center bResultItemImg'>{$vs_rep_detail_link}</div>
					<div class='bResultItemText'>
						<strong>Title</strong><br/>{$vs_label_detail_link}{$vs_workDetail}
