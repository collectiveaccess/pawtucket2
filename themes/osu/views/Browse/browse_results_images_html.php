	...
	$vs_default_placeholder_tag = "<div class='bResultItemImgPlaceholder'>".$vs_default_placeholder."</div>";
		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit() && ($vn_c < $vn_hits_per_block)) {
				...
				$vs_label_detail_link 	= caDetailLink($this->request, "<i>".$qr_res->get("{$vs_table}.preferred_labels.name")."</i>", '', $vs_table, $vn_id);
				if ($qr_res->get('ca_objects.date')) {
					$vs_label_detail_link.= "<br/>".$qr_res->get('ca_objects.date');
				}
				if ($qr_res->get('ca_objects.type')) {
					$vs_label_detail_link.= "<br/>".$qr_res->get('ca_objects.type', array('delimiter' => ', ', 'convertCodesToDisplayText' => true));
				}				
				if ($qr_res->get('ca_objects.material')) {
					$vs_label_detail_link.= "<br/>".$qr_res->get('ca_objects.material', array('delimiter' => ', ', 'convertCodesToDisplayText' => true));
				}				
				$vs_thumbnail = "";
				$vs_type_placeholder = "";
				$vs_typecode = "";
