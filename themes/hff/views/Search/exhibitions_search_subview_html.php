...
<?php
		...
		while($qr_results->nextHit()) {
			if ($vn_i == 0) { print "<div class='{{{block}}}Set authoritySetLong'>\n"; $vb_div_open = true;}
				...
				$vs_date = $qr_results->get("ca_occurrences.exhibition_dates_display", array("delimiter" => ", "));
				if(!$vs_date){
					$vs_date = $qr_results->get("ca_occurrences.common_date");
				}
				print caDetailLink($this->request, (($vs_originating_venue) ? $vs_originating_venue.", " : "").$vs_title.(($vs_date) ? ", ".$vs_date : ""), '', 'ca_occurrences', $qr_results->get("ca_occurrences.occurrence_id"));
