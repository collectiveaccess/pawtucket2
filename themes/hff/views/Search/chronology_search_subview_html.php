<?php
/* ----------------------------------------------------------------------
 * themes/default/views/Search/chronology_search_subview_html.php : 
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
<?php
		...
		while($qr_results->nextHit()) {
			$va_occurrence_ids[] = $qr_results->get('ca_occurrences.occurrence_id');
		}
		$qr_results->seek($vn_start);
		
		$va_images = caGetDisplayImagesForAuthorityItems('ca_occurrences', $va_occurrence_ids, array('version' => 'widepreview', 'relationshipTypes' => caGetOption('selectMediaUsingRelationshipTypes', $va_options, null), 'objectTypes' => caGetOption('selectMediaUsingTypes', $va_options, null), 'checkAccess' => $va_access_values));
...
<?php
			...
			if (sizeof($va_images) > 0){
				$vs_image = $va_images[$qr_results->get('ca_occurrences.occurrence_id')];
				if($vs_image){
			...
			print $vs_image_tag;
?>
				<br/><?php print $qr_results->get('ca_occurrences.preferred_labels.name', array('returnAsLink' => true)); ?>
			</div>
<?php
			$vn_count++;
