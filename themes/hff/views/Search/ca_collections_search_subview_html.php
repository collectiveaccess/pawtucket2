...
<?php
		...
		while($qr_results->nextHit()) {
			if ($vn_i == 0) { print "<div class='{{{block}}}Set authoritySetLong'>\n"; $vb_div_open = true;}
				...
				#print caDetailLink($this->request, (($vs_tmp = $qr_results->get("ca_collections.fa_title")) ? $vs_tmp : $qr_results->get("ca_collections.preferred_labels")), '', 'ca_collections', $qr_results->get("ca_collections.collection_id"));
				print caDetailLink($this->request, $qr_results->getWithTemplate("<unit relativeTo='ca_collections.hierarchy' delimiter=' &gt; '>^ca_collections.preferred_labels.name</unit><ifdef code='ca_collections.unitdate.dacs_date_text'>, ^ca_collections.unitdate.dacs_date_text</ifdef>"), '', 'ca_collections', $qr_results->get("ca_collections.collection_id"));
				print "</div>";
			$vn_count++;
			$vn_i++;
