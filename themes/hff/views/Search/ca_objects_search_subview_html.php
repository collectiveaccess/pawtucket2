...
<?php
				...
				switch($vs_block){
					case "artworks":
						print "<small>".$qr_results->get("ca_objects.idno", array('returnAsLink' => true))."</small><br/>";
						print italicizeTitle($qr_results->get("ca_objects.preferred_labels", array('returnAsLink' => true))).(($qr_results->get("ca_objects.common_date")) ? ", ".$qr_results->get("ca_objects.common_date") : "");
					break;
					# -------------------------
					case "digital_items":
						print "<small>".$qr_results->get("ca_objects.idno", array('returnAsLink' => true))."</small><br/>";
						print $qr_results->get("ca_objects.preferred_labels", array('returnAsLink' => true)).(($qr_results->get("ca_objects.unitdate.dacs_date_text")) ? ", ".$qr_results->get("ca_objects.unitdate.dacs_date_text") : "");
						#$vs_tmp = $qr_results->getWithTemplate("<unit relativeTo='ca_collections' delimiter=', '><l>^ca_collections.preferred_labels</l></unit>", array("checkAccess" => $va_access_values));				
						#if($vs_tmp){
					...
					case "library":
						$va_tmp = array();
						if($vs_tmp = $qr_results->get("ca_objects.preferred_labels")){
							$va_tmp[] = $vs_tmp;
						}
						if($vs_tmp = $qr_results->get("ca_objects.author.author_name", array("delimiter" => ", "))){
							$va_tmp[] = $vs_tmp;
						}
						if($vs_tmp = $qr_results->getWithTemplate("<unit relativeTo='ca_entities' restrictToRelationshipTypes='publisher' delimiter=', '>^ca_entities.preferred_labels</unit>", array("checkAccess" => $va_access_values))){
							$va_tmp[] = $vs_tmp;
						}elseif($vs_tmp = $qr_results->get("ca_objects.publisher", array("delimiter" => ", "))){
							$va_tmp[] = $vs_tmp;
						}
						if($vs_tmp = $qr_results->get("ca_objects.common_date", array("delimiter" => ", "))){
							$va_tmp[] = $vs_tmp;
						}
						if($vs_tmp = $qr_results->get("ca_objects.library", array("delimiter" => ", ", "convertCodesToDisplayText" => true))){
							$va_tmp[] = $vs_tmp;
						}
						if($vs_tmp = $qr_results->get("ca_objects.call_number", array("delimiter" => ", "))){
							$va_tmp[] = $vs_tmp;
						}
						if($vs_tmp = $qr_results->get("ca_objects.artwork_status", array("delimiter" => ", ", "convertCodesToDisplayText" => true))){
							$va_tmp[] = $vs_tmp;
						}
						if($vs_tmp = $qr_results->get("ca_objects.remarks", array("delimiter" => ", "))){
							$va_tmp[] = $vs_tmp;
						}
						print caDetailLink($this->request, join(", ", $va_tmp), '', 'ca_objects', $qr_results->get("ca_objects.object_id"));
