...
<?php
				...
				switch($va_facet_info["group_mode"]){
					...
					case "list":
					default:
						if($va_facet_info["sort_by_result_count"]){
							$va_tmp = array();
							$va_tmp_grouped = array();
							$va_tmp_group = array();
							foreach($va_facet_info['content'] as $va_item_tmp){
								$va_tmp_grouped[$va_item_tmp['content_count']][] = $va_item_tmp;
							}
							krsort($va_tmp_grouped);
							foreach($va_tmp_grouped as $va_tmp_group){
								foreach($va_tmp_group as $va_item_tmp){
									$va_tmp[] = $va_item_tmp;
								}
							}
							$va_facet_info['content'] = $va_tmp;
						}
						...
						foreach($va_facet_info['content'] as $va_item) {
							...
							if ((!$va_facet_info["dont_use_show_more"]) && ($vn_c == $vn_facet_display_length_initial) && ($vn_facet_size > $vn_facet_display_length_initial) && ($vn_facet_size <= $vn_facet_display_length_maximum)) {
								print "<span id='{$vs_facet_name}_more' style='display: none;'>";
							} else {
								if((!$va_facet_info["dont_use_show_more"]) && ($vn_c == $vn_facet_display_length_initial) && ($vn_facet_size > $vn_facet_display_length_maximum))  {
									break;
								}
							}
						}
						if ((!$va_facet_info["dont_use_show_more"]) && ($vn_facet_size > $vn_facet_display_length_initial) && ($vn_facet_size <= $vn_facet_display_length_maximum)) {
							print "</span>\n";
						...
						} elseif ((!$va_facet_info["dont_use_show_more"]) && ($vn_facet_size > $vn_facet_display_length_initial) && ($vn_facet_size > $vn_facet_display_length_maximum)) {
