...
<?php
				...
				if($va_exhibitions = $t_object->get("ca_occurrences", array("checkAccess" => $va_access_value, "returnWithStructure" => true, "restrictToTypes" => array("exhibition")))){
					...
					foreach($va_exhibitions as $va_exhibition){
						...
						print caDetailLink($this->request, (($vs_originating_venue) ? $vs_originating_venue.", " : "").$vs_title.(($vs_date) ? ", ".$vs_date : ""), '', 'ca_occurrences', $va_exhibition["occurrence_id"])."<br/>";
					}
					print "</div>";
				}
?>
				{{{<ifcount code="ca_occurrences" min="1" restrictToTypes="literature"><div class='unit'><H6>Literature References</H6><unit relativeTo="ca_occurrences" restrictToTypes="literature" delimiter="<br/>"><ifdef code="lit_citation"><l>^ca_occurrences.lit_citation</l></ifdef><ifnotdef code="lit_citation">^ca_occurrences.preferred_labels</ifnotdef></unit></div></ifcount>}}}
