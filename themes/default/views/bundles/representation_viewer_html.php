...
$params = ['context' => $vs_context, 'id' => $vn_subject_id, 'representation_id' => ''];
...
<script type='text/javascript'>
	jQuery(document).ready(function() {
		var caSliderepresentation_ids = <?=  json_encode($va_representation_ids); ?>;
		/* width of li */
		...
		$('.jcarousel').on('jcarousel:animate', function (event, carousel) {
			...
			if (event.type == 'jcarousel:animateend') {
				if (!jQuery('#slide' + caSliderepresentation_ids[i] + ' #slideContent' + current_rep_id).html()) {
					...
					jQuery('#slide' + caSliderepresentation_ids[i] + ' #slideContent' + current_rep_id).load('<?=  caNavUrl($this->request, '*', '*', 'GetMediaInline', array('context' => $vs_context, 'id' => $vn_subject_id, 'representation_id' => '')); ?>' + caSliderepresentation_ids[i] + '/display/detail', function(e) {
				...
				if (jQuery('#detailAnnotations').length) {
					jQuery('#detailAnnotations').html('<?=  addslashes(caBusyIndicatorIcon($this->request)." Loading..."); ?>');
					jQuery('#detailAnnotations').load('<?=  caNavUrl($this->request, '*', '*', 'GetTimebasedRepresentationAnnotationList', $params); ?>' + current_rep_id);
				}
			}else{
<?php
...
?>
			...
			if (jQuery('#detailAnnotations').length) { jQuery('#detailAnnotations').load('<?=  caNavUrl($this->request, '*', '*', 'GetTimebasedRepresentationAnnotationList', $params); ?>' + caSliderepresentation_ids[0]); }
<?php
	}
?>
			}
...
<?php
		if ($vs_show_annotations_mode == 'div') {
			$params['representation_id'] = $va_representation_ids[0];
?>	
<script type='text/javascript'>
	jQuery(document).ready(function() {
