...
?>
			...
			<H3><?php print caNavLink($va_block_info['displayName']."&nbsp;&nbsp;<span class='highlight'>(".$qr_results->numHits().")</span>", '', '', 'Search', '{{{block}}}', array('search' => $vs_search)); ?></H3>
...
<?php
		...
		while($qr_results->nextHit()) {
				...
				$vs_label_detail_link 	= caDetailLink($qr_results->get("ca_objects.preferred_labels.name"), '', 'ca_objects', $vn_id);
				...
				if(!$this->request->getParameter("openResultsInOverlay", pInteger)){
					$vs_rep_detail_link 	= caDetailLink($vs_thumbnail, '', 'ca_objects', $vn_id);
				}else{
					$vs_rep_detail_link = "<a href='#' onclick='caMediaPanel.showPanel(\"".caNavUrl('Detail', 'objects', $vn_id, array('overlay' => 1))."\"); return false;'>".$vs_thumbnail."</a>";
				}
				$vs_add_to_set_link = "<div class='addTo'><a href='#' onclick='caMediaPanel.showPanel(\"".caNavUrl('', 'Lightbox', 'addItemForm', array('object_id' => $vn_id))."\"); return false;' title='Add to lightbox'><i class='fa fa-suitcase'></i></a></div>";
				
				$vs_rep_id = $qr_results->get("ca_object_representations.representation_id");
				$vs_obj_id = $qr_results->get("ca_objects.object_id");
				$vs_download_link = "";
				if ($vs_rep_id) {
					$vs_download_link = caNavLink('<i style="padding-left:10px;" class="fa fa-download"></i>', 'multiDl', '', 'Detail', 'DownloadRepresentation', array('representation_id' => $vs_rep_id, 'object_id' => $vs_obj_id, 'download' => 1, 'version' => 'original'));
				}
				$vs_expanded_info = "";
	...
	<div class='bResultItemCol col-xs-12 col-sm-3 col-lg-2'>
		...
		print "<div style='clear:both'></div>".caNavLink(_t('Next %1', $vn_hits_per_block), 'jscroll-next', '*', '*', '*', array('s' => $vn_start + $vn_hits_per_block, 'key' => $this->getVar("cacheKey"), 'block' => $vs_block, 'search'=> $vs_search));
