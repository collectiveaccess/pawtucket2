 ...
 */
	...
	if ($qr_results->numHits() > 0) {
		$c = 0;
		print "<div class='row'><div class='col-12 mt-3 mb-2 text-capitalize fw-bold'>".$qr_results->numHits()." ".(($qr_results->numHits() == 1) ? $block_info["labelSingular"] : $block_info["labelPlural"])."</div></div>";
		print "<div class='row'>";
		if ($table != 'ca_objects') {
			$ids = array();
		...
		while($qr_results->nextHit()) {
			...
			$image = $qr_results->get('ca_object_representations.media.medium', array("checkAccess" => $access_values, "class" => $image_class));
			if(!$image){
				if($images[$id]){
...
?>
				<div class='row justify-content-center'>
					<div class='col-12 col-sm-6 col-md-4 col-lg-3 mb-3 text-center'>
						<?= caNavLink($this->request, _t("Full Results")."  <i class='ps-2 bi bi-box-arrow-up-right' aria-label='link out'></i>", "pt-3 pb-4 px-2 d-flex align-items-center justify-content-center bg-dark h-100 w-100 text-white", "", "Search", $block, ["search" => $search], [], ['encodeSlashesInParams'=> true]); ?>
					</div>
				</div>
<?php
			}
