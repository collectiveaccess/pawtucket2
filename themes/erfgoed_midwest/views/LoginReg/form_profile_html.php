...
<div class="row"><div class="col-sm-4"><H1<?php print (!$this->request->isAjax()) ? ' class="text-right"' : ''; ?>>Gebruikersprofiel</H1></div></div>
...
<?php
		$va_labels_dutch = array("fname" => "Voornaam", "lname" => _t("Achternaam"), "email" => _t("E-mail"));
		foreach(array("fname", "lname", "email") as $vs_field){
			...
			print $t_user->htmlFormElement($vs_field,"<div class='form-group".(($va_errors[$vs_field]) ? " has-error" : "")."'><label for='".$vs_field."' class='col-sm-4 control-label'>".$va_labels_dutch[$vs_field]."</label><div class='col-sm-7'>^ELEMENT</div><!-- end col-sm-7 --></div><!-- end form-group -->\n", array("classname" => "form-control"));
		}
...
?>
	...
	<!--<div class="row"><div class="col-sm-4"><H2<?php print (!$this->request->isAjax()) ? ' class="text-right"' : ''; ?>><?php print _t("Groups"); ?></H2></div></div>
		<div class="form-group<?php print (($va_errors["group_code"]) ? " has-error" : ""); ?>">
			...
			<div class="col-sm-7"><p class="help-block"><?php print _t("If you have been provided with a group code enter it here to join the group."); ?></p><input type="text" name="group_code" id="group_code" size="40" class="form-control"  autocomplete="off" /></div>
		</div>-->
<?php
