...
<?php
		...
		foreach($va_facet_content as $vn_id => $va_item) {
			$va_simple_label = explode('[', $va_item['label']);
			print "<div class='browseFacetItem col-sm-4 col-md-3'>".caNavLink($this->request, $va_simple_label[0], '', '*', '*', '*', array('facet' => $vs_facet_name, 'id' => $va_item['id'], 'view' => $vs_view, 'key' => $vs_key))."</div>";
			$vn_c++;
