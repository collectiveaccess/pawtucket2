...
<?php
					if($qr_collection_children->numHits()){
						while($qr_collection_children->nextHit()) {
							...
							$vs_dates = $qr_collection_children->get('ca_collections.unitdate.dacs_date_value', array("delimiter" => ", "));
							$vs_collection = $qr_collection_children->get('ca_collections.preferred_labels').(($vs_dates) ? " (".$vs_dates.")" : "");
							...
							if($vb_link_sublist){
								print "<a href='#' class='openCollection openCollection".$qr_collection_children->get("ca_collections.collection_id")."'>".$vs_icon." ".$vs_collection.$vs_record_count."</a>";
							}else{
								...
								if($vb_link_to_detail){
									print caDetailLink($this->request, $vs_icon." ".$vs_collection." ".(($o_collections_config->get("link_out_icon")) ? $o_collections_config->get("link_out_icon") : ""), '', 'ca_collections',  $qr_collection_children->get("ca_collections.collection_id")).$vs_record_count;
								}else{
									print "<div class='listItem'>".$vs_icon." ".$vs_collection.$vs_record_count."</div>";
								}
							}
							print "</div>";	
							if($vb_link_sublist){
