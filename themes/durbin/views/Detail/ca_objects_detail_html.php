...
<?php
			...
			print "<div class='unit'><b>Category: </b>".$t_object->get("ca_objects.category", array('convertCodesToDisplayText' => true))."</div><!-- end unit -->";
			if (($va_genre = $t_object->get("ca_objects.genre", array('convertCodesToDisplayText' => true))) && ($va_genre != 'Not Applicable')){
				print "<div class='unit'><b>Photographic genre: </b>".$va_genre."</div><!-- end unit -->";
			}
			print "<div class='unit'><b>Date: </b>".$t_object->get("ca_objects.date")."</div><!-- end unit -->";
			if ($va_synopsis = $t_object->get("ca_objects.synopsis")){
				print "<div class='unit'><b>Synopsis: </b>".$va_synopsis."</div><!-- end unit -->";
			}
			if ($va_publisher = $t_object->get("ca_objects.periodical.publisher")){
				print "<div class='unit'><b>Publisher: </b>";
				if ($va_per_link = $t_object->get("ca_objects.periodical.per_link")) {
					print "<a href='".$va_per_link."'>".$va_publisher."</a>";
				} else {
					print $va_publisher;
				}	
				if ($va_per_date = $t_object->get("ca_objects.periodical.per_date")) {
					print ", ".$va_per_date;
				}
				if ($va_per_page = $t_object->get("ca_objects.periodical.page_no")) {
					print ", page: ".$va_per_page;
				}				
				print "</div>";
			}
			if ($va_char = $t_object->get("ca_objects.char_synopsis")){
				print "<div class='unit'><b>Character Synopsis: </b>".$va_char."</div><!-- end unit -->";
			}
			if ($va_link = $t_object->get("ca_objects.link.url_entry")){
				print "<div class='unit'><b>Link to Source: </b><a href='".$va_link."'>".$t_object->get("ca_objects.link.url_source")."</a></div><!-- end unit -->";
			}			
			# --- description
