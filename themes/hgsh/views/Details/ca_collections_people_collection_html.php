<?php
	...
	#$va_featured_items = $t_item->get("ca_entities.entity_id", array("returnWithStructure" => true, "restrictToRelationshipTypes" => array("featured"), "checkAccess" => $va_access_values));
	$va_featured_items = $t_item->get("ca_entities.entity_id", array("returnWithStructure" => true, "checkAccess" => $va_access_values));
...
<?php
		if(is_array($va_featured_items) && sizeof($va_featured_items)){
			$q_featured_people = caMakeSearchResult('ca_entities', $va_featured_items);
			if($q_featured_people->numHits()){
				print "<div class='row'><div class='col-sm-12'><div class='btn btn-default'>"._t("Featured People")."</div></div></div><!-- end row -->\n";
				$i = 0;
				while($q_featured_people->nextHit()){
					if($i == 0){
						print "<div class='row'>";
					}
					if(!($vs_media = $q_featured_people->getWithTemplate("<l>^ca_object_representations.media.resultcrop</l>", array("checkAccess" => $va_access_values)))){
						$vs_media = caGetThemeGraphic($this->request, 'placeholder.jpg');
					}
					$vs_caption = $q_featured_people->getWithTemplate('<l>^ca_entities.preferred_labels.displayname</l>');
					if($vs_caption){
