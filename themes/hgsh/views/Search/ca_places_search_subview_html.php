...
<?php
		...
		while($qr_results->nextHit()) {
			$va_place_ids[] = $qr_results->get('ca_places.place_id');
		}
		$qr_results->seek($vn_start);
		
		$va_images = caGetDisplayImagesForAuthorityItems('ca_places', $va_place_ids, array('version' => 'resultcrop', 'relationshipTypes' => caGetOption('selectMediaUsingRelationshipTypes', $va_options, null), 'checkAccess' => $va_access_values));
...
<?php
			...
			if (sizeof($va_images) > 0){
				$vs_image = $va_images[$qr_results->get('ca_places.place_id')];
				if($vs_image){
			...
			print $vs_image_tag;
?>
				<div class="multisearchResultText"><?php print $qr_results->get('ca_places.preferred_labels.name', array('returnAsLink' => true)); ?></div>
			</div>
<?php
			$vn_count++;
