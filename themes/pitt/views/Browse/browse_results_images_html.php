	...
	$vs_default_placeholder_tag = "<div class='bResultItemImgPlaceholder'>".$vs_default_placeholder."</div>";
		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit() && ($vn_c < $vn_hits_per_block)) {
				...
				$vs_label_detail_link 	= "<p class='objTitle'>".caDetailLink($this->request, $qr_res->get("{$vs_table}.preferred_labels.name"), '', $vs_table, $vn_id)."</p>";
				$vs_thumbnail = "";
				$vs_type_placeholder = "";
				$vs_typecode = "";
				if ($vs_table == 'ca_objects') {
					...
					if ($va_artists = "<p>".$qr_res->get('ca_entities.preferred_labels.displayname', array('restrictToRelationshipTypes' => array('creator'), 'delimiter' => ', '))."</p>") {
						
					} else {
						$va_artists = null;
					}
					$va_dates = "<p class='dateValue'>".$qr_res->getWithTemplate('<unit delimiter=", "><if rule="^ca_objects.date.date_types =~ /Date created/">^ca_objects.date.date_value</if></unit>')."</p>";					
		...
		<div class='bResultItem' onmouseover='jQuery(\"#bResultItemExpandedInfo{$vn_id}\").show();'  onmouseout='jQuery(\"#bResultItemExpandedInfo{$vn_id}\").hide();'>
			...
			<div class='bResultItemContent'><div class='text-center bResultItemImg'>{$vs_rep_detail_link}</div>
				<div class='bResultItemText'>
					<small>{$vs_idno_detail_link}</small><br/>{$vs_label_detail_link}{$va_artists}{$va_dates}
