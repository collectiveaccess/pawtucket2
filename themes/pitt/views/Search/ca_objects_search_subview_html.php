...
<?php 
				...
				$vn_object_id = $qr_results->get('ca_objects.object_id');
				print $qr_results->getWithTemplate('<l>'.$vs_image.'</l>', array("checkAccess" => $va_access_values));
				
				print "<p class='objTitle'>";
				if ($va_obj_title = $qr_results->get('ca_objects.preferred_labels.name')) {
					if (strlen($va_obj_title) > 80) {
						print caNavLink($this->request, substr($va_obj_title, 0, 77)."...", '', '', 'Detail', 'objects/'.$vn_object_id);
					} else {
						print caNavLink($this->request, $va_obj_title, '', '', 'Detail', 'objects/'.$vn_object_id);
					}
				}
				print "</p>"; 
				
				if ($va_artists = $qr_results->get('ca_entities.preferred_labels.displayname', array('restrictToRelationshipTypes' => array('creator'), 'delimiter' => ', '))) {
					print "<p>".$va_artists."</p>"; 
				} else {
					$va_artists = null;
				}
				print "<p class='dateValue'>".$qr_results->getWithTemplate('<unit delimiter=", "><if rule="^ca_objects.date.date_types =~ /Date created/">^ca_objects.date.date_value</if></unit>')."</p>";
?>
