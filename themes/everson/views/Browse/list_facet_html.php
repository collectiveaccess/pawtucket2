...
<?php
		...
		foreach($va_facet_content as $vn_id => $va_item) {
			print "<div class='browseFacetItem col-sm-4 col-md-4'>".caNavLink($this->request, $va_item['label'], '', '*', '*', '*', array('facet' => $vs_facet_name, 'id' => $va_item['id'], 'view' => $vs_view, 'key' => $vs_key))."</div>";
			$vn_c++;
			...
			<script type="text/javascript">
				jQuery(document).ready(function() {
					jQuery("#panel_<?php print $vs_facet_name; ?>").jscroll({
						loadingHtml: "<div class='browseFacetItem col-sm-4 col-md-4'><a href='#'><?php print addslashes(caBusyIndicatorIcon($this->request).' '._t('Loading...')); ?></a></div>",
						padding: 1000,
