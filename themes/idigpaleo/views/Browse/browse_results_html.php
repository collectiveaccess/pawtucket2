 ...
 */
	...
	$vs_refine_position = $o_config->get('refine_position');
...
<?php
	# Display Browse menu here if set to left or top
	if($vs_refine_position == 'top'){
	# TODO: Create new full width Facet menu display option
?>
		<div class="col-sm-12">
		
		</div>	
<?php
	} else if($vs_refine_position == 'left'){
?>
		<div class="<?php print ($vs_refine_col_class) ? $vs_refine_col_class : "col-sm-4 col-md-3 col-md-offset-1 col-lg-3 col-lg-offset-1"; ?>">
			<div id="bViewButtonsLeft">
	<?php
			if(is_array($va_views) && (sizeof($va_views) > 1)){
				foreach($va_views as $vs_view => $va_view_info) {
					if ($vs_current_view === $vs_view) {
						print '<a href="#" class="active"><span class="glyphicon '.$va_view_icons[$vs_view]['icon'].'"  aria-label="selected view style"></span></a> ';
					} else {
						print caNavLink($this->request, '<span class="glyphicon '.$va_view_icons[$vs_view]['icon'].'"  aria-label="other view style options"></span>', 'disabled', '*', '*', '*', array('view' => $vs_view, 'key' => $vs_browse_key)).' ';
					}
				}
			}
	?>
...
<?php
		...
		if (sizeof($va_criteria) > 0) {
			...
			foreach($va_criteria as $va_criterion) {
				...
				$vs_last_facet = $va_criterion['facet'];
