 ...
 */
	...
	$vb_multiple_selection = caGetOption('multiple', $va_facet_info, false, ['castTo' => 'boolean']);
...
<?php
		...
		foreach($va_facet_content as $vn_id => $va_item) {
			if($va_facet_info["group_mode"]== "alphabetical"){
				...
				$vs_facet_list .= "<div class='facetItem' data-facet='{$vs_facet_name}' data-facet_item_id='{$va_item['id']}'>".caNavLink($this->request, $va_item['label'], '', '*', '*', '*', array('facet' => $vs_facet_name, 'id' => $va_item['id'], 'view' => $vs_view, 'key' => $vs_key))."</div>\n";
			$vn_c++;
		}
...
<?php			
		...
		print "<div id='bScrollList' class='{$vs_facet_name}_facet_container'>".$vs_facet_list."</div><!-- end bScrollList -->";
