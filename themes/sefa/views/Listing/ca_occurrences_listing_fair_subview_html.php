 ...
 */
	...
	foreach($va_lists as $vn_type_id => $qr_list) {
		...
		while($qr_list->nextHit()) {
			...
			print "<div class='col-sm-4 fairListing'>".caDetailLink($this->request, ($vs_image) ? $vs_image : "<h2>".$qr_list->getWithTemplate('<l>^ca_occurrences.preferred_labels.name</l>')."</h2>", '', 'ca_occurrences', $qr_list->get("occurrence_id"), null, null, array("type_id" => $qr_list->get("type_id"))).$qr_list->get('ca_occurrences.opening_closing')."</div>\n";	
			if($vn_col == 3){
				print "</div>";
				$vn_col = 1;
			}else{
