...
<?php
				...
				if ($va_related_ex_ids = $t_object->get('ca_occurrences.occurrence_id', array('returnAsArray' => true, 'restrictToTypes' => array('exhibition', 'public_program')))) {
					print '<div class="col-sm-12">	
								<hr>
								<h6>Related Exhibitions & Programs</h6>
							</div>
						  </div>';
					foreach ($va_related_ex_ids as $va_id => $va_related_ex_id) {
						$t_rel_ex = new ca_occurrences($va_related_ex_id);
						print "<div class='col-sm-12'>";
						print "<p>".$t_rel_ex->get('ca_occurrences.preferred_labels')."</p>";
						print "<p>".$t_rel_ex->get('ca_occurrences.exhibition_dates')."</p>";
						print "</div>";
					}
				}
				print "</div><!-- end row -->";	
