<?php
/* ----------------------------------------------------------------------
 * themes/stormking/views/Search/ca_collections_search_subview_html.php : 
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
<?php
		...
		while($qr_results->nextHit()) {
			if ($vn_i == 0) { print "<div class='{{{block}}}Set authoritySet'>\n"; $vb_div_open = true;}
				print "<div class='collectionsResult authorityResult'>".strip_tags($qr_results->get('ca_collections.preferred_labels.name', array('returnAsLink' => true)), "<a>")."</div>";
			$vn_count++;
			$vn_i++;
