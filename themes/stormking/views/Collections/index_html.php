<?php
	...
	if($qr_collections && $qr_collections->numHits()) {
		while($qr_collections->nextHit()) {
			$va_collections[$qr_collections->get('ca_collections.collection_id')] = array("rank" => $qr_collections->get('ca_collections.collection_rank'), "id" => $qr_collections->get("ca_collections.collection_id"), "media" => $qr_collections->get("ca_object_representations.media.widepreview", array('primaryOnly' => true)), "title" => $qr_collections->get("ca_collections.preferred_labels")); 
...
<?php						
	...
	if(is_array($va_collections) && sizeof($va_collections)) {
		foreach($va_collections as $va_collection) {
			...
			if ($va_collection["description"]) {
				print "<div class='collectionDetail'>".$va_collection["description"]."</div>";
			}
