...
#	$vs_default_placeholder_tag = "<div class='bResultItemImgPlaceholder'>".$vs_default_placeholder."</div>";
	...
	$va_add_to_set_link_info = caGetAddToSetInfo($this->request);
		...
		$vn_col_span_xs = 12;
		$vb_refine = false;
		// if (($vs_table == 'ca_entities') | ($vs_table == 'ca_occurrences')) {
// 			$vn_col_span = 6;
// 			$vn_col_span_sm = 6;
// 			$vn_col_span_xs = 6;		
// 		}
		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit()) {
				...
				if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result')){
					...
					$vs_artist = $vs_artist_text = $vs_art_date = $vs_date_text = $vs_entity_date = $vs_entity_date_text = $vs_ex_date = $vs_ex_date_text = "";
					if ($vs_table == 'ca_objects') {
					...
					if ($vs_table == 'ca_entities') {
						if ($vs_entity_date = $qr_res->get('ca_entities.entity_display_date')) {
							$vs_entity_date_text = ", ".$vs_entity_date;
						} else {
