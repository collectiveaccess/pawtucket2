...
$options = $this->getVar("detail_options");
...
<?php
	...
	$media_review_element = $options['media_review_element'];
...
?>
	<div class="col-4 text-center">
		<button class='btn btn-sm btn-outline-danger mt-2' id="mediaviewer-remove" hx-on:click='window.mediaViewerManagers["mediaviewer"].removeMedia();'><i class="bi bi-x-circle"></i> Remove Media</button>
	</div>
<?php
	}
?>
...
<?php
	if(sizeof($media_list) > 1) {
		...
		foreach($media_list as $i => $m) {
			$media_icons[] = "<div class='col-2 img-fluid mb-3' id='mediaviewer-icon-{$i}'><button class='mediaviewer-selector-control mediaIcon' hx-on:click='window.mediaViewerManagers[\"mediaviewer\"].render({$i});'>".$m['icon_tag'].'</button></div>';
		}
?>
