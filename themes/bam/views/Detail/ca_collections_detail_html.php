 ...
 */
	...
	$vn_occurrence_id 	= $t_collection->getPrimaryKey();
...
?>
	<div id="detailBody">
		<div id="pageNav">
<?php
			print ResultContext::getResultsLinkForLastFind($this->request, 'ca_collections', "<img src='".$this->request->getThemeUrlPath()."/graphics/arrow_up_grey.gif' width='11' height='10' border='0'> "._t("BACK"), '');

			if (($this->getVar('next_id')) || ($this->getVar('previous_id'))) {	
				print "&nbsp;&nbsp;&nbsp;";
			}
			if ($this->getVar('previous_id')) {
				print caNavLink($this->request, "<img src='".$this->request->getThemeUrlPath()."/graphics/arrow_grey_left.gif' width='10' height='10' border='0'> "._t("PREVIOUS"), '', 'Detail', 'Collection', 'Show', array('collection_id' => $this->getVar('previous_id')), array('id' => 'previous'));
			}
			if (($this->getVar('next_id')) && ($this->getVar('previous_id'))) {	
				print "&nbsp;&nbsp;|&nbsp;&nbsp;";
			}
			if ($this->getVar('next_id') > 0) {
				print caNavLink($this->request, _t("NEXT")." <img src='".$this->request->getThemeUrlPath()."/graphics/arrow_grey_right.gif' width='10' height='10' border='0'>", '', 'Detail', 'Collection', 'Show', array('collection_id' => $this->getVar('next_id')), array('id' => 'next'));
			}
?>
		</div><!-- end nav -->
...
<?php
				$va_entity_rel_types = $t_rel_types->getRelationshipInfo('ca_entities_x_collections');
...
<?php
			...
			if(sizeof($this->getVar('occurrences'))){
				...
				$va_occ_rel_types = $t_rel_types->getRelationshipInfo('ca_occurrences_x_collections');
...
<?php					
			...
			if($this->getVar('places')){
				...
				$va_place_rel_types = $t_rel_types->getRelationshipInfo('ca_places_x_collections');
			...
			if($this->getVar('collections')){
				...
				$va_collection_rel_types = $t_rel_types->getRelationshipInfo('ca_collections_x_collections');
