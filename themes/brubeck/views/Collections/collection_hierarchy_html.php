...
<?php
					if($qr_collection_children->numHits()){
						while($qr_collection_children->nextHit()) {
							...
							$vs_date = $qr_collection_children->getWithTemplate('<ifdef code="ca_collections.date_container.date"><unit relativeTo="ca_collections.date_container" delimiter=", "><if rule="^ca_collections.date_container.date_type =~ /Created/">, ^ca_collections.date_container.date</if></unit></ifdef>');
							if($vb_link_sublist){
