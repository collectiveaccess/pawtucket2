...
<script type='text/javascript'>
	...
	let representation_ids = <?= json_encode($representation_ids); ?>;
	...
	function setItem(i) {
		if((i >= 0) && (i < slide_list.length)) {
			let rep_id = representation_ids[index];
			let players = caUI.mediaPlayerManager.getPlayers();
			let p = players['caMediaOverlayTimebased_' + rep_id + '_detail'];
			if(p) {
				p.destroy();
			}
			...
			rep_id = representation_ids[index];
			
			setTimeout(function() {
				caUI.mediaPlayerManager.play('caMediaOverlayTimebased_' + rep_id + '_detail');
			},
			1000);
		}
		return false;
