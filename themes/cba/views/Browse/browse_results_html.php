...
?>
		<H1>
<?php
			print _t('%1 %2', (($vb_is_search) ? "Search" : "Browse"), $va_browse_info["labelPlural"]);	
...
<?php
		print "<div class='resultCount'>("._t('%1 %2', $vn_result_size, ($vn_result_size == 1) ? _t("Result") : _t("Results")).")</div>";
...
<?php
			...
			foreach($va_criteria as $va_criterion) {
				...
				if ($va_criterion['facet_name'] != '_search') {
					if($va_criterion['facet_name'] == "has_cba_highlight_facet"){
...
?>
			...
			<div class='row' id='searchOptions'>	
				<div class='col-sm-12 col-md-4 col-lg-4 btn-group'>
				...
				if(is_array($va_views) && (sizeof($va_views) > 1)){
					...
					foreach($va_views as $vs_view => $va_view_info) {
						if ($vs_current_view == $vs_view) {
							$vs_views[] = '<li><a href="#" class="active"><em>'.$vs_view.'</em></a></li>';
						} else {
							$vs_views[] = "<li>".caNavLink($this->request, $vs_view, '', '*', '*', '*', array('view' => $vs_view, 'key' => $vs_browse_key)).'</li>';
						}
					}
...
?>
	...
	<div class="<?php print ($vs_refine_col_class) ? $vs_refine_col_class : "col-sm-3 col-md-3"; ?>">
<?php
