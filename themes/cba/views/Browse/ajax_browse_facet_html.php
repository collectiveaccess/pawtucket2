...
?>
	<div id="browseSelectPanelHeader">
		<div id="browseSelectPanelHeaderContent"><nobr><span id="jumpTo"><?php print _t("Jump to:"); ?></span>
<?php 
	foreach($va_groups as $vs_group) {
		$vs_group_proc = preg_replace("![^A-Za-z0-9]+!", "_", $vs_group);
		if($vs_extra_padding){
			$vs_style = " style='margin-right:25px;'";
		}else{	
			$vs_style = "";
		}
		print " <a href='#' onclick='jQuery(\".browseSelectPanelList\").scrollTop(jQuery(\".browseSelectPanelList\").scrollTop() + jQuery(\"#browse_group_{$vs_group_proc}\").offset().top - 193); return false;' ".$vs_style.">{$vs_group}</a> ";
	}
?>
		</nobr></div>
	</div>
