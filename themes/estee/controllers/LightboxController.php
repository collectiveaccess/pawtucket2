...
class LightboxController extends FindController {
	...
	public function getLightboxMedia() {
		# --- to download media, set access must be "acessible on site with download" and the user must have access to the set
		...
		$vn_download_access = ($this->request->config->get('lightbox_download_access')) ? $this->request->config->get('lightbox_download_access') : 2;
		if(!$t_set->get("ca_sets.access") == $vn_download_access){
			$this->notification->addNotification(_t('You do not have access to download media'), __NOTIFICATION_TYPE_ERROR__);
			$this->opo_response->setRedirect(caNavUrl($this->request, '', 'Lightbox', 'Index'));
			return false;
		}
		...
		while($qr_res->nextHit()) {
			...
			foreach($va_reps as $vn_representation_id => $va_rep) {
				...
				$va_download_display_info = caGetMediaDisplayInfo('download_high_res', $va_rep["mimetype"]);
