				...
				if($va_options["show_object"] && sizeof($va_object_ids)){
					...
					if($qr_objects->numHits()){
						...
						while($qr_objects->nextHit()){
							...
							$vs_output .= "<div class='row'><div class='col-xs-9'>".$vs_eye.caDetailLink($po_request, $qr_objects->get("ca_objects.preferred_labels"), '', 'ca_objects', $qr_objects->get("ca_objects.object_id"))."</div><div class='col-xs-3'>".$qr_objects->get("ca_objects.box_folder")."</div></div>";
						}
						$vs_output .= "</div>";
					}
				}
