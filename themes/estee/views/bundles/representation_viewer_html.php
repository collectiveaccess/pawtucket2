		...
		$('.jcarousel').on('jcarousel:animate', function (event, carousel) {
			...
			if (event.type == 'jcarousel:animateend') {
				if (!jQuery('#slide' + caSliderepresentation_ids[i] + ' #slideContent' + current_rep_id).html()) {
					...
					jQuery('#slide' + caSliderepresentation_ids[i] + ' #slideContent' + current_rep_id).load('<?php print caNavUrl($this->request, '*', '*', 'GetMediaInline', array('context' => $vs_context, 'id' => $vn_subject_id, 'representation_id' => '')); ?>' + caSliderepresentation_ids[i] + '/display/detail<?php print (($t_subject->get("type_id", array("convertCodesToDisplayText" => true)) == "Product") ? "_watermark" : ""); ?>', function(e) {
