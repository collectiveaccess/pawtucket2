...
<?php
				if($x && in_array($vs_block, $va_browse_types)){
?>
...
?>
				<span class='multisearchFullResults'><?php print caNavLink($this->request, _t('Full results'), 'btn btn-default btn-small', '', 'Search', '{{{block}}}', array('search' => str_replace("/", "", $vs_search))); ?></span> 
...
<?php 
				...
				print $qr_results->getWithTemplate('<l>'.$vs_image.'</l>', array("checkAccess" => $va_access_values));
						...
						$vs_caption .= $qr_results->get('ca_objects.type_id', array('convertCodesToDisplayText' => true))." &rsaquo; ";
						$vs_brand = $qr_results->get("ca_objects.brand", array("convertCodesToDisplayText" => true, "delimiter" => ", "));
						$vs_subbrand = $qr_results->get("ca_objects.sub_brand", array("convertCodesToDisplayText" => true, "delimiter" => ", "));
						if($vs_brand || $vs_subbrand){
							$vs_caption .= $vs_brand.(($vs_brand && $vs_subbrand) ? " &rsaquo; " : "").$vs_subbrand;
						}
						$vs_caption .= "</div>";
						$vs_caption .= trim($qr_results->get('ca_objects.preferred_labels'));
						$vs_tmp = $qr_results->getWithTemplate('^ca_objects.manufacture_date');
						if(!$qr_results->get("ca_objects.manufacture_date")){
							$vs_tmp .= "undated";
						}
						if(trim($vs_tmp)){
							$vs_caption .= ", ".$vs_tmp;
						}
						if($vs_tmp = $qr_results->get("ca_objects.codes.product_code")){
							$vs_caption .= " (".$vs_tmp.")";
						}
						print "<div class='bResultItemText'>".caDetailLink($this->request, $vs_caption, '', 'ca_objects', $qr_results->get("ca_objects.object_id"))."</div>";
