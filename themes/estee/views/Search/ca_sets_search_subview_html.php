<?php
/* ----------------------------------------------------------------------
 * themes/default/views/Search/ca_sets_search_subview_html.php : 
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
<?php
		...
		while($qr_results->nextHit()) {
			if ($vn_i == 0) { print "<div class='{{{block}}}Set authoritySet'>\n"; $vb_div_open = true; }
				print "<div class='{{{block}}}Result authorityResult'>".caNavLink($this->request, $qr_results->get('ca_sets.preferred_labels.name'), '', '', 'Gallery', $qr_results->get("ca_sets.set_id"))."</div>";
			$vn_count++;
			$vn_i++;
