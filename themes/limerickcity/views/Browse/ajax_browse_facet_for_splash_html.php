...
<script type="text/javascript">
	function caUpdateFacetDisplay(grouping) {
		...
		jQuery("#splashBrowseTabPanel").load('<?php print caNavUrl($this->request, '', 'Browse', 'getFacet', array()); ?>', { facet: '<?php print $vs_facet_name;?>', modify: <?php print ($vm_modify_id ? $vm_modify_id : '0'); ?>, id: <?php print $vm_modify_id; ?>, grouping: grouping, view: 'for_splash' });
	}
...
<?php 
	if (isset($va_facet_info['groupings']) && is_array($va_facet_info['groupings']) && sizeof($va_facet_info['groupings'] )) {
		...
		foreach($va_facet_info['groupings'] as $vs_grouping => $vs_grouping_label) {
			print "<a href='#' onclick='caUpdateFacetDisplay(\"{$vs_grouping}\"); return false;' class='".(($vs_grouping == $vs_grouping_field) ? 'selected' : '')."'>{$vs_grouping_label}</a>";
...
?>
	<div id="browseSelectPanelHeaderScrollButtons">
		<div id="browseFacetNextPrevControls"><a href="#" id="browseFacetPrevControl" onclick="return false;"><img src="<?php print $this->request->getThemeUrlPath(); ?>/graphics/arrow_left_gray.gif" width="10" height="12" border="0"></a> <a href="#" id="browseFacetNextControl" onclick="return false;"><img src="<?php print $this->request->getThemeUrlPath(); ?>/graphics/arrow_right_gray.gif" width="10" height="12" border="0"></a></div>
	</div>
<?php
...
<?php 
	foreach($va_groups as $vs_group) {
		...
		print " <a href='#' onclick='jQuery(\".browseSelectPanelList\").scrollTop(jQuery(\".browseSelectPanelList\").scrollTop() + jQuery(\"#browse_group_{$vs_group_proc}\").offset().top - 700); return false;'>{$vs_group}</a> ";
	}
?>
		</nobr></div>
	</div>
