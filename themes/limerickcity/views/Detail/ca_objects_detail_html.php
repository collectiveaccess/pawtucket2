...
<?php
			if (($this->getVar('is_in_result_list')) && ($vs_back_link = ResultContext::getResultsLinkForLastFind($this->request, 'ca_objects', _t("Back to Results"), ''))) {
				...
				print "<div class='searchbox-button' style='float:right; margin: 10px 15px 20px 0px; '>".caNavLink($this->request, _t("Back to Catalogue"), '', '', '', '')."</div>";
			}
?>
...
<?php
			...
			print "<div id='objDetailImage'><img src='http://museum.limerick.ie/themes/limerickcity/graphics/city/absent.jpg' border='0'></div>";
		}
?>
...
<?php
			...
			# --- attributes
			$va_attributes = $this->request->config->get('ca_objects_detail_display_attributes_date');
			...
			# --- attributes
			$va_attributes = $this->request->config->get('ca_objects_detail_display_attributes_location');
			if(is_array($va_attributes) && (sizeof($va_attributes) > 0)){
				foreach($va_attributes as $vs_attribute_code){
					if($t_object->get("ca_objects.{$vs_attribute_code}")){
						print "<div class='unit'><b>".$t_object->getAttributeLabel($vs_attribute_code).":</b> ".$t_object->get("ca_objects.{$vs_attribute_code}")."</div><!-- end unit -->";
			...
			# --- entities
			$va_entities = $t_object->get("ca_entities", array("returnAsArray" => 1, 'checkAccess' => $va_access_values, 'exclude_relationship_types' => array('donor')));
