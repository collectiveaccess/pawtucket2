<?php
	...
	if($va_collection_children = $t_item->get('ca_collections.children.collection_id', array('returnAsArray' => true, 'checkAccess' => $va_access_values, 'sort' => 'ca_collections.preferred_labels.name_sort'))){
		...
		if($qr_collection_children->numHits()){
			while($qr_collection_children->nextHit()){
				if($qr_collection_children->get("ca_collections.children.collection_id", array('returnAsArray' => true, 'checkAccess' => $va_access_values, 'sort' => 'ca_collections.preferred_labels.name_sort'))){
	...
	if($vb_has_children){
			# --- checkbox on collection level record indicates if the viewer should be open or collapsed by default
			# --- load top level collection and check setting
