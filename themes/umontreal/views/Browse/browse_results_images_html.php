		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit()) {
				...
				if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result')){
					...
					$vs_label_detail_link 	= caDetailLink($this->request, $qr_res->getWithTemplate("<l>^ca_objects.preferred_labels.name</l><br/><ifdef='ca_objects.culture'>^ca_objects.culture<br></ifdef><unit relativeTo='ca_places' restrictToTypes='region' restrictToRelationshipTypes='origine)' start='0' length='1'>^ca_places.preferred_labels.name%maxLevelsFromBottom=4&delimiter=_➜_</unit>^ca_objects.places"), '', $vs_table, $vn_id);
					$vs_thumbnail = "";
					$vs_type_placeholder = "";
					$vs_typecode = "";
					if ($vs_table == 'ca_objects') {
						if(!($vs_thumbnail = $qr_res->get('ca_object_representations.media.medium', array("checkAccess" => $va_access_values)))){
							...
							if($vs_type_placeholder = caGetPlaceholder($vs_typecode, "placeholder_media_icon")){
								$vs_thumbnail = "<div class='bResultItemImgPlaceholder' style='background-image:url(\"/themes/umontreal/assets/pawtucket/graphics/Petroglyphe_ANT.png\");background-size:contain;background-repeat:no-repeat;background-position:center center;'></div>";
							}else{
