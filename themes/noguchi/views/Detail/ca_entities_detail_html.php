...
<?php
			...
			if(is_array($va_attributes) && (sizeof($va_attributes) > 0)){
				foreach($va_attributes as $vs_attribute_code){
					if($t_entity->get("ca_entities.".$vs_attribute_code)){
						print "<div class='unit'><b>".$t_entity->getAttributeLabel($vs_attribute_code).":</b> ".$t_entity->get("ca_entities.".$vs_attribute_code)."</div><!-- end unit -->";
					}
				}
			}
			# --- description
			if($this->request->config->get('ca_entities_description_attribute')){
				if($vs_description_text = $t_entity->get("ca_entities.".$this->request->config->get('ca_entities_description_attribute'))){
					print "<div class='unit'><div id='description'><b>".$t_entity->getAttributeLabel($this->request->config->get('ca_entities_description_attribute')).":</b> ".$vs_description_text."</div></div><!-- end unit -->";				
?>
