			...
			for (let i in this.state.facetContentSort) {
				...
				options.push((
					<li key={this.props.facetName + 'facetItem' + i}>
						<NoguchiArchiveBrowseFacetPanelItem id={this.props.facetName + 'facetItem' + i} data={item} callback={this.clickFilterItem} selected={this.state.selectedFacetItems[item.id]}/>
					</li>
				));
			}
		}
	...
	render() {
		...
		if((this.context.state.resultSize === null) && !this.context.state.loadingMore) {
			...
			for (let i in this.context.state.resultList) {
				...
				resultList.push(<NoguchiArchiveBrowseResultItem key={'result_' + r.id} view={this.props.view} data={r} scrollToRef={ref}/>)
			}
