 ...
 */
  ...
  # TG update 5/2021 to get correct lightbox text and icon (did have default text/icon hard-coded)
  $va_add_to_set_link_info = caGetAddToSetInfo($this->request);
  if(is_array($va_add_to_set_link_info) && sizeof($va_add_to_set_link_info)){
    $vs_lb_text = $va_add_to_set_link_info["link_text"];
    $vs_lb_icon = $va_add_to_set_link_info["icon"];
  }
  else{
    $vs_lb_text = "Add to lightbox";
    $vs_lb_icon = "<i class='fa fa-suitcase'></i>";
  }
...
<?php
		...
		while($qr_results->nextHit()) {
				...
				$vs_add_to_set_link = "<div class='addTo'><a href='#' onclick='caMediaPanel.showPanel(\"".caNavUrl($this->request, '', 'Lightbox', 'addItemForm', array('object_id' => $vn_id))."\"); return false;' title='".$vs_lb_text."'>".$vs_lb_icon."</a></div>";
