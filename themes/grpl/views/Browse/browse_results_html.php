		...
		if (sizeof($va_criteria) > 0) {
			...
			foreach($va_criteria as $va_criterion) {
				...
				if (($va_criterion['facet_name'] != '_search') || (($va_criterion['facet_name'] == '_search')) && ($i > 0)) {
					...
					print caNavLink($this->request, '<button type="button" class="btn btn-default-br btn-sm">'.$va_criterion['value'].' <span class="glyphicon glyphicon-remove" aria-label="Remove filter"></span></button>', 'browseRemoveFacet', '*', '*', '*', array('removeCriterion' => $va_criterion['facet_name'], 'removeID' => urlencode($va_criterion['id']), 'view' => $vs_current_view, 'key' => $vs_browse_key));
				}else{
