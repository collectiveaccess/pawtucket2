...
?>
<?php
		# TG added 05/21 to display collection info (to implement collection listing that is browsable)
		if (sizeof($va_criteria) > 0) {
			foreach($va_criteria as $va_criterion) {
				if ($va_criterion['facet_name'] == 'collection_facet') {
					$vo_collection = new ca_collections($va_criterion['id']);
					print "<H1>".$vo_collection->get('ca_collections.preferred_labels.name')."</H1>";
					print "<div class='row'><div class='col-md-12 col-lg-12'><p>".$vo_collection->get('ca_collections.description')."</p></div></div>";
				}
			}
		}
			print ($vo_collection) ? "<H4>" : "<H1>";
	?>
<?php
			# TG removed item type from Heading (object, occurance, entity, etc.)
		...
		if (sizeof($va_criteria) > 0) {
			...
			foreach($va_criteria as $va_criterion) {
				...
				if ($va_criterion['facet_name'] != '_search') {
					...
					print caNavLink($this->request, '<button type="button" class="btn btn-default-br btn-sm">'.$va_criterion['value'].' <span class="glyphicon glyphicon-remove-circle" aria-label="Remove filter"></span></button>', 'browseRemoveFacet', '*', '*', '*', array('removeCriterion' => $va_criterion['facet_name'], 'removeID' => urlencode($va_criterion['id']), 'view' => $vs_current_view, 'key' => $vs_browse_key));
				}else{
...
<?php
	if ($vs_current_view == "map"){
		print "<div class='map-warning'><p>This view only includes items that have been mapped. Change ".caNavLink($this->request, 'the view', 'disabled', '*', '*', '*', array('view' => 'image', 'key' => $vs_browse_key))." to see all results.</p></div>";
	}
?>
