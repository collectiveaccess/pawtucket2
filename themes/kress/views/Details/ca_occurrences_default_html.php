	...
	<?php
						...
						if(is_array($media_value_ids = $t_item->get("ca_occurrences.media.media_media.value_id", ["returnAsArray" => true])) && sizeof($media_value_ids)) {
							foreach($media_value_ids as $value_id) {
								...
								$vs_compare_link = "<div class='detailTool'><i class='fas fa-clone' aria-hidden='true' aria-label='Compare Images'></i></span><a href='#' class='compare_link' data-id='attribute:{$media_value_ids[0]}' title='Compare Images'>Compare Image</a></div>";
							}
						}
						if($vn_pdf_enabled) {
