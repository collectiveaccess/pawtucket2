<?php
/* ----------------------------------------------------------------------
 * themes/default/views/Search/ca_loans_search_subview_html.php : 
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
<?php
		...
		while($qr_results->nextHit()) {
			if ($vn_i == 0) { print "<div class='{{{block}}}Set authoritySet'>\n"; $vb_div_open = true;}
				print "<div class='collectionsResult authorityResult'>".$qr_results->getWithTemplate('<l>^ca_loans.idno ^ca_loans.preferred_labels.name</l>')."</div>";
			$vn_count++;
			$vn_i++;
