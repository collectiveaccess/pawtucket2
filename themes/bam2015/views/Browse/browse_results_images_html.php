 ...
 */
	...
	if(($vs_table == 'ca_occurrences') && ($this->request->getParameter("openResultsInOverlay", pInteger))){
		...
		<div class='col-xs-12 col-sm-".(($this->request->getParameter("openResultsInOverlay", pInteger) || $this->request->getParameter("homePage", pInteger)) ? "3" : "4")."'>
			...
			$sort = ($s = $this->request->getParameter('sort', pString)) ? $s : '_relevance';
			print "<div style='clear:both;'></div>".caNavLink($this->request, _t('Next %1', $vn_hits_per_block), 'jscroll-next', '*', '*', '*', array('s' => $vn_start + $vn_hits_per_block, 'key' => $vs_browse_key, 'sort' => $sort, 'view' => $vs_current_view, 'openResultsInOverlay' => (int)$this->request->getParameter("openResultsInOverlay", pInteger), 'homePage' => (int)$this->request->getParameter("homePage", pInteger)));
		}
?>
