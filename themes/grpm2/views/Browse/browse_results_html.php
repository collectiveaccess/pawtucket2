...
<?php
		if (sizeof($va_criteria) > 0) {
			...
			foreach($va_criteria as $va_criterion) {
				...
				if(($vs_table == "ca_collections") && (sizeof($va_criteria) == 1) && !$vb_is_search && $va_current_facet["show_description_when_first_facet"] && ($va_criterion['facet_name'] == "type_facet")){
					# --- show the desc from the list item for collection type
					$t_list_item = new ca_list_items($va_criterion["id"]);
					$vs_facet_description = $t_list_item->get("ca_list_item_labels.description");
				}
			}
		}
?>		
