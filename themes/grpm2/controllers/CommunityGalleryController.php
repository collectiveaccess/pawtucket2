 ...
 */
	...
	require_once(__CA_LIB_DIR__.'/pawtucket/BasePawtucketController.php');
 	...
 	class CommunityGalleryController extends BasePawtucketController {
 		# -------------------------------------------------------
 		public function __construct(&$po_request, &$po_response, $pa_view_paths=null) {
            ...
            if (($this->request->config->get('deploy_bristol'))&&($this->request->isLoggedIn())) {
 			...
 			if(!$vs_section_name = $this->config->get('community_gallery_section_name')){
 				$vs_section_name = _t("Featured Galleries");
 			}
 			$this->view->setVar("section_name", $vs_section_name);
 			if(!$vs_section_item_name = $this->config->get('community_gallery_section_item_name')){
 		...
 		public function __call($ps_function, $pa_args) {
 			...
 			$t_list = new ca_lists();
 			$vn_gallery_set_type_id = $t_list->getItemIDFromList('set_types', $this->config->get('community_gallery_set_type')); 			
 			$t_set = new ca_sets();
 			if($ps_function == "index"){
				...
				MetaTagManager::setWindowTitle($this->request->config->get("app_display_name").$this->request->config->get("page_title_delimiter").(($this->config->get('gallery_section_name')) ? $this->config->get('gallery_section_name') : _t("Gallery")));
 				$this->render("CommunityGallery/index_html.php");
 			}
 		}
 		# -------------------------------------------------------
		/** 
