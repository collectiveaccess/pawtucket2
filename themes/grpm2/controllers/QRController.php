 ...
 */
	...
	require_once(__CA_LIB_DIR__.'/pawtucket/BasePawtucketController.php');
 	...
 	class QRController extends BasePawtucketController {
 		# -------------------------------------------------------
 		public function __construct(&$po_request, &$po_response, $pa_view_paths=null) {
 			...
 			if(!$vs_section_name = $this->config->get('qr_section_name')){
 				$vs_section_name = _t("QR Sets");
 			}
 			$this->view->setVar("section_name", $vs_section_name);
 			if(!$vs_section_item_name = $this->config->get('qr_section_item_name')){
 		...
 		public function __call($ps_function, $pa_args) {
 			...
 			$t_list = new ca_lists();
 			$vn_gallery_set_type_id = $t_list->getItemIDFromList('set_types', $this->config->get('qr_set_type')); 			
 			$t_set = new ca_sets();
 			if($ps_function == "index"){
 				if($vn_gallery_set_type_id){
					...
					$this->view->setVar('qr_intro', $t_set->get('ca_sets.set_description'));
				}
