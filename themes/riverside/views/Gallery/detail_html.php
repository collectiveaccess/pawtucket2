...
<?php
		...
		foreach($pa_set_items as $pa_set_item){
			...
			if(!$vs_rep && ($vs_table != "ca_objects")){
				$t_instance->load($pa_set_item["row_id"]);
				$vs_rep = $t_instance->getWithTemplate('^ca_object_representations.media.iconlarge');
				if(!$vs_rep){
					$vs_rep = $t_instance->getWithTemplate('<unit relativeTo="ca_objects.related" length="1">^ca_object_representations.media.icon_large</unit>');
				}
			}
			if($vs_rep){
				$vn_i++;
