		...
		# -------------------------------------------------------
 		...
 		public function getSetItemRep(){
			...
			$set_items = caExtractValuesByUserLocale($t_set->getItems(array("thumbnailVersions" => array("icon", "iconlarge"), "checkAccess" => $this->opa_access_values)));
 			...
 			$rep_id = $set_items[$item_id]["representation_id"];
 			# --- it there isn't a rep and this is not a set of objects, try to get a related object to show something
			if(!$rep_id){
				if(Datamodel::getTableName($t_set->get('table_num')) != "ca_objects"){
					if (!($t_instance = Datamodel::getInstanceByTableNum($t_set->get('table_num')))) { throw new ApplicationException(_t('Invalid item')); }
					$t_instance->load($set_items[$item_id]["row_id"]);
					$rep_id = $t_instance->getWithTemplate('^ca_object_representations.representation_id', array("checkAccess" => $this->opa_access_values));
					if(!$rep_id){
						$rep_id = $t_instance->getWithTemplate('<unit relativeTo="ca_objects.related" length="1">^ca_object_representations.representation_id</unit>', array("checkAccess" => $this->opa_access_values));
					}
				}
			}
