...
<?php
						if ($va_item['representation_tag']) {
							print caNavLink($this->request, $va_item['representation_tag'], '', '', 'ObjectDetail', 'Show', array('object_id' => $va_item['row_id']));
						}
						
						$va_title = array();
						if ($va_item['caption']) {
							if (unicode_strlen($va_item['caption']) > 30) {
								$va_title[] = unicode_substr($va_item['caption'], 0, 27).'...';
							} else {
								$va_title[] = $va_item['caption'];
							}
						}
						if ($va_item['name']) {
							if (unicode_strlen($va_item['name']) > 30) {
								$va_title[] = '<i>'.unicode_substr($va_item['name'], 0, 27).'...</i>';
							} else {
								$va_title[] = '<i>'.$va_item['name'].'</i>';
							}
						}
						
						$vs_title = join('; ', $va_title);
						//if ($va_item['idno']) {
