...
<?php
			...
			if (sizeof($va_row) > 0) {
				if (sizeof($va_row) < $vn_cols) {
					for($vn_i = sizeof($va_row); $vn_i <= $vn_cols; $vn_i++) {
						$va_row[] = '<td> </td>';
					}
				}
				print "<tr valign='top'>".join('', $va_row)."</tr>\n";
		...
		default:
			...
			foreach($va_facet as $vn_i => $va_item) {
				...
				foreach($va_groups as $vs_group) {
					...
					if ($vn_grouping_attribute_id && ($vn_element_datatype == 2)) {	// if grouping by date, sort facet listing by start date
						$vs_alpha_key = $va_item['ca_attribute_'.$vn_grouping_attribute_id][0]['value_decimal1'].$vn_c;
					} else {
			...
			foreach($va_grouped_items as $vs_key => $va_list) {
				ksort($va_list);
				unset($va_grouped_items[$vs_key]);
				$va_grouped_items[mb_strtoupper($vs_key)] = $va_list;
			}
			ksort($va_grouped_items);
			$va_groups = array_keys($va_grouped_items);
			sort($va_groups, SORT_STRING);
?>
...
<?php
			foreach($va_groups as $vs_group) {
				$va_items =& $va_grouped_items[$vs_group];
				$va_row = array();
				if ($vs_group === '~') {
					$vs_group = '~';
				}
