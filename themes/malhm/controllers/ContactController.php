 	...
 	class ContactController extends BasePawtucketController {
 		...
 		public function Send() {
 			...
 			if(sizeof($va_errors) == 0){
 				# --- if this is an ask a curator form, try to send to the contact person for the item's institution
 				$vs_email = "";
 				$pn_object_id = $this->request->getParameter("object_id", pInteger);
 				
 				if($pn_object_id){
 					$t_object = new ca_objects($pn_object_id);
 					if($vn_source_id = $t_object->get("source_id")){
 						$vn_entity_id = ca_entities::getIDForIdno(caGetListItemIdno($vn_source_id));
 						$t_entity = new ca_entities($vn_entity_id);
 						$vs_email = $t_entity->get("email");
 					}
 				}
 				if(!$vs_email){
 					$vs_email = $this->config->get("contact_email");
 				}
 				# --- send email
