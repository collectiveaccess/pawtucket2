...
<?php
		...
		while($qr_results->nextHit()) {
				...
				if ($vs_idno_detail = $qr_results->get('ca_objects.idno')) {
					$vs_idno_detail_link = $vs_idno_detail;
				} else {
					$vs_idno_detail = null;
				}
				$vs_info = null;
				$vs_info.= $qr_results->getWithTemplate('<p>^ca_objects.type_id</p>');
				if ($vs_date = $qr_results->get('ca_objects.date_created', array('delimiter' => ', '))) {
					$vs_info.= "<p>".$vs_date."</p>";
				} else {
					$vs_date = null;
				}
	...
	<div class='bResultItemCol col-xs-12 col-sm-3'>
		<div class='bResultItem' onmouseover='jQuery(\"#bResultItemExpandedInfo{$vn_id}\").show();'  onmouseout='jQuery(\"#bResultItemExpandedInfo{$vn_id}\").hide();'>
			<div class='bResultItemContent'><div class='text-center bResultItemImg'><div class='imgContainer'>{$vs_rep_detail_link}</div></div>
				<div class='bResultItemText'>
					<small>{$vs_idno_detail_link}</small><p>{$vs_label_detail_link}</p>{$vs_info}
