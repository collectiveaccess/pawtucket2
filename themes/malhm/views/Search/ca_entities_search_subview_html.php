...
<?php
		...
		while($qr_results->nextHit()) {
			$va_entity_ids[] = $qr_results->get('ca_entities.entity_id');
		}
		$qr_results->seek($vn_start);
		$va_images = null;
		$va_images = caGetDisplayImagesForAuthorityItems('ca_entities', $va_entity_ids, array('version' => 'widepreview', 'relationshipTypes' => caGetOption('selectMediaUsingRelationshipTypes', $va_options, null), 'objectTypes' => caGetOption('selectMediaUsingTypes', $va_options, null), 'checkAccess' => $va_access_values));
...
<?php
			...
			if (sizeof($va_images) > 0){
				$vs_image = $va_images[$qr_results->get('ca_entities.entity_id')];
				if($vs_image){
					$vs_image_tag = $qr_results->getWithTemplate("<l>{$vs_image}</l>");
				} 
			}
			if(!$vs_image_tag){
				$vs_image_tag = "<div class='multisearchImgPlaceholder'>".$vs_default_placeholder."</div>";
			}
			print $vs_image_tag;
?>
				<?php print $qr_results->get('ca_entities.preferred_labels', array('returnAsLink' => true)); ?>
			</div>
<?php
			$vn_count++;
