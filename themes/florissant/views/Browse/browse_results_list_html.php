	...
	$va_add_to_set_link_info = caGetAddToSetInfo($this->request);
		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit() && ($vn_c < $vn_hits_per_block)) {
					...
					foreach($va_container as $va_hierarchy){
						foreach(['scientific_name', 'genus', 'subgenus', 'subfamily', 'family', 'superfamily', 'suborder', 'order', 'subclass', 'class', 'subphylum', 'phylum', 'kingdom'] as $vs_name){
							if($va_hierarchy[$vs_name] && $va_hierarchy[$vs_name] != ' '){
								...
								$vs_label_detail_link 	= caDetailLink($this->request, $vs_displayName, '', $vs_table, $vn_id);
								break;
							}
						}
					}
                ...
                # Grade Level for Occurrences
                $vs_gradeLevel = $qr_res->get("ca_occurrences.gradeLevel", ['convertCodesToDisplayText' => true]);
                $vs_grade_detail = '<small>Grade Level: '.$vs_gradeLevel.'</small><br/>';
		...
		<div class='bResultListItem' onmouseover='jQuery(\"#bResultListItemExpandedInfo{$vn_id}\").show();'  onmouseout='jQuery(\"#bResultListItemExpandedInfo{$vn_id}\").hide();'>
			<div class='bResultListItemContent'><div class='text-center bResultListItemImg'>{$vs_rep_detail_link}</div>
				<div class='bResultListItemText'>
					<small>{$vs_pref_detail_link}</small><br/>{$vs_label_detail_link}<br/>{$vs_formation_name}{$vs_common_name}{$vs_grade_detail}
