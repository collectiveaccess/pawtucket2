...
<?php
					...
					while($qr_res->nextHit()){
						if($vs_media = $qr_res->getWithTemplate('<l>^ca_object_representations.media.large</l>', array("checkAccess" => $va_access_values))){
							...
							$va_taxonomy = $qr_res->get("ca_objects.taxonomy_specimen", ["returnWithStructure" => true]);
							...
							$vs_caption = $qr_res->getWithTemplate("<l>{$vs_displayName} (^ca_objects.idno)</l>").(($vs_set_item_caption) ? ", ".$vs_set_item_caption : "");
							if($vs_caption){
...
?>
				<!-- Prev/next controls -->
				<a href="#" class="jcarousel-control-prev"><i class="fa fa-angle-left" aria-label="previous"></i></a>
				<a href="#" class="jcarousel-control-next"><i class="fa fa-angle-right" aria-label="next"></i></a>
<?php
				...
				foreach($va_thumbnails as $vn_slide_id => $vs_thumbnail){
					print "<div class='col-xs-1 smallpadding frontCarouselThumbnail'><a href='#' onClick='$(\".jcarousel\").jcarousel(\"scroll\", $(\"#".$vn_slide_id."\"), false); return false;'>".$vs_thumbnail."</a></div>";
				}
				print "</div>";
			}
?>
