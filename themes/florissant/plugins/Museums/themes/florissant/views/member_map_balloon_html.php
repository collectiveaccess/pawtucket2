<?php
/* ----------------------------------------------------------------------
 * app/plugins/Museums/member_map_balloon_html.php : 
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
<?php
	foreach($va_entity_ids as $vn_entity_id){
		...
		print "<H1>".caNavLink($this->request, $t_entity->getLabelForDisplay(), '', '', 'Detail', 'entities/'.$t_entity->get("ca_entities.idno"))."</H1>";

		print "<p class='location'>".$t_entity->getWithTemplate("^ca_entities.address.city, ^ca_entities.address.stateprovince")."</p>"; 
