			...
			while($qr_res->nextHit()) {
				...
				if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result')){
					...
					if($vs_table != 'ca_collections'){
						$vs_label_coll_info = $qr_res->get("ca_collections.preferred_labels.name");
					};
					if($vs_typecode != 'manuscript' and $vs_table != 'ca_collections'){
						$vs_materials = $qr_res->get('ca_objects.materials', array('convertCodesToDisplayText' => true, 'delimiter' => ', '));
						$vs_result_output = "
			...
			<div class='bResultListItemCol col-xs-{$vn_col_span_xs} col-sm-{$vn_col_span_sm} col-md-{$vn_col_span}'>
				<div class='bResultListItem' id='row{$vn_id}'>
					...
					<div class='bResultListItemContent'>
						<div class='bResultListItemText'>
							{$vs_label_detail_link}<br/>{$vs_label_coll_info}{$vs_label_date_see}<br/>{$vs_label_see_link}
						</div><!-- end bResultListItemText -->
					</div><!-- end bResultListItemContent -->
				</div><!-- end bResultListItem -->
			</div><!-- end col -->";
