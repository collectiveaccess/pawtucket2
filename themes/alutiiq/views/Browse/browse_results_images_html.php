 ...
 */
	...
	$vs_default_placeholder_tag = "<div class='bResultItemImgPlaceholder'>".$vs_default_placeholder."</div>";
		...
		$vn_col_span_xs = 6;
		$vb_refine = false;
		if(!$vb_ajax && (is_array($va_facets) && sizeof($va_facets))){
			$vb_refine = true;
			$vn_col_span = 4;
			$vn_col_span_sm = 6;
			$vn_col_span_xs = 6;
		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit()) {
				...
				if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result')){
					...
					if($vs_table == "ca_objects"){
						$vs_idno_detail_link 	= "";
						$vs_institution_link = $qr_res->getWithTemplate('<ifcount code="ca_collections" min="1"><unit relativeTo="ca_collections" delimiter=", "><ifcount code="ca_entities" min="1"><unit relativeTo="ca_entities"><l>^ca_entities.preferred_labels.displayname</l></unit></ifcount></unit></ifcount>');
						$vs_label_detail_link 	= caDetailLink($this->request, $qr_res->getWithTemplate("^ca_objects.preferred_labels<ifdef code='ca_objects.alutiiq_title'> - ^ca_objects.alutiiq_title</ifdef>"), '', $vs_table, $vn_id);
					}else{
			...
			<div class='bResultItem' id='row{$vn_id}'>
				...
				<div class='bResultItemContent'><div class='text-center bResultItemImg'>{$vs_rep_detail_link}</div>
					<div class='bResultItemText'>
						<small>{$vs_idno_detail_link}{$vs_institution_link}</small><br/>{$vs_label_detail_link}
