...
<?php
		print caNavLink($this->request, $va_block_info['displayName'].' <span class="resultCount"> / '.$num_results_display.' of '.$qr_results->numHits().' project'.(($num_results_display > 1) ? "s" : "").'</span>', '', '', 'Search', '{{{block}}}', array('search' => $vs_search));
?>
		</H2>
...
<?php 
						...
						print "<div class='slide'>".caDetailLink($this->request, $vs_thumbnail, "", "ca_objects", $qr_results->get("ca_objects.object_id"))."<div class='slideCaption'>".caDetailLink($this->request, $qr_results->get("ca_objects.preferred_labels.name"), "", "ca_objects", $qr_results->get("ca_objects.object_id"))."</div></div>";
