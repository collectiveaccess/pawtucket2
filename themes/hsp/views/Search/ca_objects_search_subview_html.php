...
?>
			...
			<script>
				$(function () {
					$('#popoverView_<?php print $qr_results->get("ca_objects.idno"); ?>').popover({
						html: true,
						trigger: 'hover',
						placement: 'top',
						content: function () {
							return $('#popoverContent_<?php print $qr_results->get("ca_objects.idno"); ?>').html();
						}
					});
				});
			</script>
<?php 
				...
				print '<div id="popoverView_'.$qr_results->get("ca_objects.idno").'">'.$qr_results->getWithTemplate('<l>'.$vs_image.'</l>', array("checkAccess" => $va_access_values))."</div>";
?>
				...
				<div id="popoverContent_<?php print $qr_results->get("ca_objects.idno"); ?>" class="obj_popover" style="display: none">
					<unit relativeTo='ca_object_representations.related' limit='1'><?php print $qr_results->get("ca_object_representations.media.medium"); ?></unit><br/><strong>Title: </strong><?php print $qr_results->get("ca_objects.preferred_labels"); ?><br/><strong>ID: </strong><?php print $qr_results->get("ca_objects.idno"); ?>
				</div>
			</div><!-- end blockResult -->
<?php
			$vn_count++;
