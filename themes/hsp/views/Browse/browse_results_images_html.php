 ...
 */
	...
	<div class='bResultItemCol col-xs-{$vn_col_span_xs} col-sm-{$vn_col_span_sm} col-md-{$vn_col_span}'>
		<script>
			$(function () {
				$('#popoverView_{$vn_id}').popover({
					html: true,
					trigger: 'hover',
					placement: 'top',
					content: function () {
						return $('#popoverContent_{$vn_id}').html();
					}
				});
			});
		</script>
		<div class='bResultItem' onmouseover='jQuery(\"#bResultItemExpandedInfo{$vn_id}\").show();'  onmouseout='jQuery(\"#bResultItemExpandedInfo{$vn_id}\").hide();'>
			...
			<div class='bResultItemContent'><div id='popoverView_{$vn_id}' class='text-center bResultItemImg'>{$vs_rep_detail_link}</div>
				<div class='bResultItemText'>
					{$vs_label_detail_link}<br/>{$vs_idno_detail_link}
		...
		<div id='popoverContent_{$vn_id}' class='obj_popover' style='display: none'>
			{$vs_thumbnail}<br/><strong>Title: </strong>{$vs_label}<br/><strong>ID: </strong>{$vs_idno}
				</div>
	</div><!-- end col -->";
