...
<?php
		...
		foreach($pa_set_items as $pa_set_item){
			...
			if($pa_set_item["representation_tag_".$vs_icon]){
				$vn_i++;
				print "<div class='smallpadding col-xs-3 col-sm-2 col-md-".(($ps_description) ? "2" : "1").(($vn_i >60) ? " galleryIconHidden" : "")."'>";
				...
				if($vn_i == 60){
					print "<div class='col-sm-3' id='moreLink'>
								<a href='#' onclick='$(\".galleryIconHidden\").removeClass(\"galleryIconHidden\"); $(\"#moreLink\").hide(); return false;'>".(sizeof($pa_set_items) - 60)." "._t("more")."</a>
							</div>";
				}
			}
		}
