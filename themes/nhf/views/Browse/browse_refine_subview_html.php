...
<?php
				...
				print "<H5>".caGetThemeGraphic($this->request, 'cross.gif', array('style' => 'margin: 0px 5px 0px 0px;'))."<a href='#' class='more' onclick='jQuery(\"#bMorePanel\").load(\"".caNavUrl($this->request, '*', '*', '*', array('getFacet' => 1, 'facet' => $vs_facet_name, 'view' => $vs_view, 'key' => $vs_key))."\", function(){jQuery(\"#bMorePanel\").show(); jQuery(\"#bMorePanel\").mouseleave(function(){jQuery(\"#bMorePanel\").hide();});}); return false;'>".$va_facet_info['label_singular']."</a></H5>"; 
				#print "<div class='bMorePanels' id='bMorePanel{$vs_facet_name}'></div>";
				#print "<H5><a href='#' onclick='jQuery(\".facetMoreLists\").hide(); jQuery(\"#{$vs_facet_name}_more\").slideToggle(250); return false;'>".$va_facet_info['label_singular']."</a></H5>"; 
if($inlineFacets){
				print "<div id='{$vs_facet_name}_more' class='facetMoreLists' style='display: none;'>";
				switch($va_facet_info["group_mode"]){
					...
					default:
						...
						foreach($va_facet_info['content'] as $va_item) {
							$vn_c++;
							if($vs_facet_name != "decade_facet"){
								print "<div>";
							}
							print caNavLink($this->request, $va_item['label'], 'facetLink', '*', '*','*', array('key' => $vs_key, 'facet' => $vs_facet_name, 'id' => $va_item['id'], 'view' => $vs_view));
							if($vs_facet_name != "decade_facet"){
								print "</div>";
							}else{
								if($vn_c < sizeof($va_facet_info['content'])){
									print ", ";
								}else{
									print "<br/>";
								}
				...
				print "<a href='#' onclick='jQuery(\"#{$vs_facet_name}_more\").slideToggle(250); return false;'>"._t("close")."</a>"; 
				print "</div><!-- end more -->\n";
}
			}
		}
