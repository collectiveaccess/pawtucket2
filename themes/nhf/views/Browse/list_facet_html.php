...
<?php
		...
		foreach($va_facet_content as $vn_id => $va_item) {
			if($va_facet_info["group_mode"]== "alphabetical"){
				...
				if(!in_array($vs_first_letter, $va_letter_bar) && ($vs_first_letter != "[")){
					$va_letter_bar[$vs_first_letter] = $vs_first_letter;
					$vs_facet_list .= "<div id='facetList".$vs_first_letter."' class='moreFacetListHeading'>".$vs_first_letter."</div>";
				}
			}			
				$vs_label = $va_item['label'];
				if($vs_facet_name == "geoloc_facet"){
					$vs_label = substr($vs_label, 0, mb_strpos($vs_label, "["));
				}
				if($vs_label){
					$vs_facet_list .= "<div>".caNavLink($this->request, $vs_label, '', '*', '*', '*', array('facet' => $vs_facet_name, 'id' => $va_item['id'], 'view' => $vs_view, 'key' => $vs_key))."</div>\n";
				}
			$vn_c++;
		}
		...
		print "<H1 id='bScrollListLabel'>".$va_facet_info["label_plural"]."</H1>";
		if($va_facet_info["group_mode"]== "alphabetical"){
			print "<div id='bLetterBar'><b>Jump To:</b> ";
...
?>
		<script type="text/javascript">		
			...
			jQuery(document).ready(function() {
				jQuery("#bScrollList").height((jQuery("#bMorePanel").height() - 130) + "px");
			});
		</script>
<?php
		}
