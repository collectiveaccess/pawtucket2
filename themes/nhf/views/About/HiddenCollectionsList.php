...
<?php
	...
	if(is_array($va_items) && (sizeof($va_items) > 0)){
		...
		foreach($va_items as $vn_i => $va_set_item_info){
			...
			print caDetailLink($this->request, $t_collection->getAttributesForDisplay("collection_still", null, array('version' => 'thumbnail', 'showMediaInfo' => false)), '', 'ca_collections', $vn_collection_id);
			print caDetailLink($this->request, $t_collection->getLabelForDisplay(), '', 'ca_collections', $vn_collection_id);
...
<?php
	...
	if($qr_collections->numHits() > 0){
		...
		while($qr_collections->nextHit()) {
			print "<div class='item'>".caDetailLink($this->request, $qr_collections->get("ca_collections.preferred_labels.name"), '', 'ca_collections', $qr_collections->get("collection_id"))."</div>\n";
			$c++;
			if($c == $colLength){
