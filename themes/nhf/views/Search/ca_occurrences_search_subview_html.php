...
<?php
		...
		while($qr_results->nextHit()) {
			$vs_has_video = "";
			if(in_array($qr_results->get("occurrence_id"), $va_ids_with_video)){
				$vs_has_video = "<span class='glyphicon glyphicon-facetime-video'></span>";
			}			
			if ($vn_i == 0) { print "<div class='{{{block}}}Set authoritySet'>\n"; $vb_div_open = true; }
				print "<div class='{{{block}}}Result authorityResult'>".$qr_results->get('ca_occurrences.preferred_labels.name', array('returnAsLink' => true))." ".$vs_has_video."</div>";
			$vn_count++;
			$vn_i++;
