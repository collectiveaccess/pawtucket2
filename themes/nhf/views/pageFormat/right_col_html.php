...
<?php
					foreach($va_most_popular_collections as $vn_pop_coll_id => $vs_pop_coll){
						print "<li>".caDetailLink($this->request, $vs_pop_coll, '', 'ca_collections', $vn_pop_coll_id)."</li>";
					}
?>			
				</ul>
			</div><!-- end unit -->
...
<?php
				...
				foreach($va_set_items as $vn_item_id => $va_item_info){
					if($va_item_info["access"] == 1){
						print "<li>".caDetailLink($this->request, $va_item_info["name"], '', 'ca_collections', $va_item_info["row_id"])."</li>";
					}
				}
?>
				</ul>
