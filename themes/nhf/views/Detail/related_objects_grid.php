...
<?php
	while(($vn_itemc < $this->getVar('items_per_page')) && ($qr_hits->nextHit())) {
		...
		$va_media_info = $qr_hits->getPrimaryRepresentationInfo('thumbnail');
		$vn_padding_top = 0;
		...
		if($this->request->config->get('allow_detail_for_ca_objects')){
			print caNavLink($this->request, $qr_hits->getPrimaryRepresentationTag('thumbnail'), '', 'Detail', 'Object', 'Show', array('object_id' => $qr_hits->get('ca_objects.object_id')));
		}else{
			print $qr_hits->getPrimaryRepresentationTag('thumbnail');
		}
		...
		$this->setVar('tooltip_representation', $qr_hits->getPrimaryRepresentationTag('small'));
