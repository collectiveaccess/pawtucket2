 ...
 */
	...
	$va_add_to_set_link_info = caGetAddToSetInfo($this->request);
		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit()) {
				...
				if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result')){
					...
					$vs_type_placeholder = "";
					$vs_typecode = "";
					$vs_image = ($vs_table === 'ca_objects') ? $qr_res->getMediaTag("ca_object_representations.media", 'small', array("class" => "browseImage", "checkAccess" => $va_access_values)) : $va_images[$vn_id];
		...
		<div class='bResultListItemCol col-xs-{$vn_col_span_xs} col-sm-{$vn_col_span_sm} col-md-{$vn_col_span}'>
			...
			$prev_page = $vn_start - $vn_results_output;
			$next_page = $vn_start + $vn_results_output;
			
			print "<div style='clear:both'></div>";
			if ($prev_page >= 0) {
			    print "<div class='bResultListItemCol col-xs-{$vn_col_span_xs} col-sm-{$vn_col_span_sm} col-md-{$vn_col_span}'>".caNavLink($this->request, _t('Previous %1 books', $vn_hits_per_block), 'jscroll-next', '*', '*', '*', array('s' => $prev_page, 'key' => $vs_browse_key, 'view' => $vs_current_view, 'sort' => $vs_current_sort, '_advanced' => $this->getVar('is_advanced') ? 1  : 0))."</div>";
			}
			if ($next_page < $qr_res->numHits()) {
			    print "<div class='bResultListItemCol col-xs-{$vn_col_span_xs} col-sm-{$vn_col_span_sm} col-md-{$vn_col_span}' style='text-align: right;'>".caNavLink($this->request, _t('Next %1 books', $vn_hits_per_block), 'jscroll-next', '*', '*', '*', array('s' => $next_page, 'key' => $vs_browse_key, 'view' => $vs_current_view, 'sort' => $vs_current_sort, '_advanced' => $this->getVar('is_advanced') ? 1  : 0))."</div>";
		    }
		}
?>
