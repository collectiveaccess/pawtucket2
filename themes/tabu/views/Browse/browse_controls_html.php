...
<?php
				...
				$vn_x = 0;
				print "<div id='browseCriteria'>"._t("You browsed for: ")." ";
				...
				print "</div><!-- end browseCriteria --></div><!-- end browseControls -->";
				...
				if (sizeof($va_facets)) { 
					...
					$va_available_facets = $this->getVar('available_facets');
					foreach($va_available_facets as $vs_facet_code => $va_facet_info) {
						print "<div><a href='#' onclick='jQuery(\"#browse\").load(\"".caNavUrl($this->request, '', 'Browse', 'getFacet', array('facet' => $vs_facet_code))."\"); return false;'>".$va_facet_info['label_plural']."</a></div>\n";
					}
...
<?php
	if (sizeof($va_criteria) > 0) {
		...
		print $this->render('Results/'.$vs_browse_target.'_results_full_html.php');
		print "</div>";
	}
	if (!$this->request->isAjax()) {
?>
