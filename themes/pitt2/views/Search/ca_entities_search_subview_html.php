...
<?php
		...
		while($qr_results->nextHit()) {
			if ($vn_i == 0) { print "<div class='row'>\n"; $vb_div_open = true;}
				print "<div class='col-sm-6 col-md-3'><div class='entitiesResult authorityResult'>".$qr_results->getWithTemplate($vs_caption_template, array("checkAccess" => $va_access_values))."</div></div>";
			$vn_count++;
			$vn_i++;
			if ($vn_i >= 4) {
				print "</div><!-- end row -->\n";
		...
		if ($vb_div_open) {print "</div><!-- end row -->";}		// closing div if we stop short of a full row
