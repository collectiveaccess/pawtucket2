 ...
 */
	...
	$vb_only_show_reps_in_order	= $this->getVar('only_show_reps_in_order');	// if set then only display representations selected in the current order
		...
		if (
			...
			if ($t_set_item && ($vb_set_is_loaded = $t_set_item->getPrimaryKey())) {
				...
				if ($t_order_item && ($vb_set_is_loaded = $t_order_item->getPrimaryKey())) {
					...
					if ($vb_only_show_reps_in_order) {
						foreach($va_reps as $vn_id => $va_rep) {
							if (!isset($va_selected_reps[$va_rep['representation_id']])) {
								unset($va_reps[$vn_id]);
							}
						}
						$va_selected_reps = null;
					} 
				}
			}
			...
			if (
				...
				while($o_children->nextHit()) {
					...
					$va_sections[$vn_object_id] = array(
						...
						'isSelected' => 0,
						'editUrl' => caNavUrl($this->request, 'Detail', 'Object', 'Index', array('object_id' => $vn_object_id)), 
						'downloadUrl' => caObjectsDisplayDownloadLink($this->request) ? caNavUrl($this->request, 'Detail', 'Object', 'DownloadRepresentation', array('object_id' => $vn_object_id, 'representation_id' => $vn_representation_id, 'version' => 'original')) : "");
...
?>
	<!-- Controls -->
	<div class="<?php print ($vs_display_type == 'media_overlay') ? 'caMediaOverlayControls' : 'caMediaDisplayControls'; ?>">
			<table width="95%">
				<tr valign="middle">
					<td align="left">
...
<?php
	if ($vn_id = $this->getVar('previous_representation_id')) {
		print "<a href='#' onClick='jQuery(\"#{$vs_container_id}\").load(\"".caNavUrl($this->request, 'Detail', 'Object', 'GetRepresentationInfo', array('representation_id' => (int)$vn_id, 'object_id' => (int)$t_object->getPrimaryKey(), 'display_type' => $vs_display_type, 'containerID' => $vs_container_id))."\");'>←</a>";
	}
	if (sizeof($va_reps) > 1) {
