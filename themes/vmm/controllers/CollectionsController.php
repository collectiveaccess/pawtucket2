 ...
 */
 	...
 	class CollectionsController extends ActionController {
 		...
 		public function Index() {
 			...
 			$t_list = new ca_lists();
 			$va_collection_type_ids = array();
 			foreach($this->opo_config->get("landing_page_collection_type") as $vs_collection_type){
 				$va_collection_type_ids[] = $t_list->getItemIDFromList("collection_types", $vs_collection_type);
 			}
 			$vs_sort = ($this->opo_config->get("landing_page_sort")) ? $this->opo_config->get("landing_page_sort") : "ca_collections.preferred_labels.name";
			$qr_collections = ca_collections::find(['type_id' => ['IN', $va_collection_type_ids]], ['returnAs' => 'searchResult', 'checkAccess' => $this->opa_access_values, 'sort' => $vs_sort]);
