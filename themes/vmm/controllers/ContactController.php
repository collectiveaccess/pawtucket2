 		...
 		public function Send() {
 			...
 			if(sizeof($va_errors) == 0){
 				# --- send email
					...
					$va_to_email = $this->config->get("contact_email");
					...
					if($pn_object_id = $this->request->getParameter("object_id", pInteger)){
						...
						if($vs_typecode == "collection_object"){
							$va_to_email = $this->config->get("contact_email_curator");
						}else{
							$va_to_email = $this->config->get("contact_email_archives");
						}
					}
					if(caSendmail($va_to_email, $this->request->config->get("ca_admin_email"), $vs_subject_line, $vs_mail_message_text, $vs_mail_message_html)){
