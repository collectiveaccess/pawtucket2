...
<?php
		...
		while($qr_results->nextHit()) {
			if ($vn_i == 0) { print "<div class='{{{block}}}Set authoritySet'>\n"; $vb_div_open = true; }
				print "<div class='{{{block}}}Result authorityResult'>".caDetailLink($this->request, trim($qr_results->get("ca_occurrences.vesprefix", array("convertCodesToDisplayText" => true))." <i>".$qr_results->get("ca_occurrences.preferred_labels")."</i> ".$qr_results->get("ca_occurrences.vessuffix", array("convertCodesToDisplayText" => true))), '', 'ca_occurrences', $qr_results->get("ca_occurrences.occurrence_id"))."</div>";
			$vn_count++;
			$vn_i++;
