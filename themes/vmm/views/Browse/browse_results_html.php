		...
		if (sizeof($va_criteria) > 0) {
			...
			foreach($va_criteria as $va_criterion) {
				...
				if ($va_criterion['facet_name'] != '_search') {
					...
					$va_sub_values = $va_all_facets[$va_criterion['facet_name']]['substitute_values'];
					if($va_sub_values && is_array($va_sub_values) && sizeof($va_sub_values) && $va_sub_values[$va_criterion['id']]){
						$vs_value = $va_sub_values[$va_criterion['id']];
					}
					print caNavLink($this->request, '<button type="button" class="btn btn-default btn-sm">'.$vs_value.' <span class="glyphicon glyphicon-remove-circle"></span></button>', 'browseRemoveFacet', '*', '*', '*', array('removeCriterion' => $va_criterion['facet_name'], 'removeID' => urlencode($va_criterion['id']), 'view' => $vs_current_view, 'key' => $vs_browse_key));
				}else{
