...
<?php	
	...
	if($qr_collections && $qr_collections->numHits()) {
		while($qr_collections->nextHit()) {
			...
			print "<div class='text-right'>".caDetailLink($this->request, "View", "btn btn-default btn-small", "ca_collections",  $qr_collections->get("ca_collections.collection_id"))."</div>";
			print "</div></div>";
			$vn_i++;
			if ($vn_i == 2) {
				print "</div><!-- end row -->\n";
