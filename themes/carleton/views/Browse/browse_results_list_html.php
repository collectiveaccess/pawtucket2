					...
					switch($vs_table){
						case "ca_collections":
							if(strToLower($qr_res->getWithTemplate("^ca_collections.type_id")) == "collection"){
								...
								$vs_label_detail_link 	= caDetailLink($this->request, $qr_res->getWithTemplate("^ca_collections.type_id ^ca_collections.idno ^ca_collections.id_number<if rule='^ca_collections.preferred_labels.name !~ /BLANK/'>: ^ca_collections.preferred_labels</if><ifdef code='ca_collections.inclusive_dates'>, ^ca_collections.inclusive_dates%delimiter=,_</ifdef>"), '', $vs_table, $vn_id);
							}
