...
class CollectionsController extends ActionController {
	...
	# -------------------------------------------------------
	public function CollectionHierarchyList(){
		$collection_id = $this->request->getParameter('collection_id', pInteger);
		if($collection_id){
			$t_item = new ca_collections($collection_id);
			if(!$t_item->isLoaded() || !in_array($t_item->get('ca_collections.access'), caGetUserAccessValues($this->request))) {
				throw new ApplicationException(_t('Not available'));	
			}
			$this->view->setVar("item", $t_item);
			
			$this->view->setVar('content', caGetCollectionHierarchyList($this->request, [$collection_id], 1, ['start' => 0]));
		}else{
			throw new ApplicationException("Invalid collection_id");
		}
		$this->render("Collections/collection_hierarchy_list_html.php");
	}
	# -------------------------------------------------------
	public function CollectionHierarchyListPage(){
		$o_collections_config = caGetCollectionsConfig();
		
		$collection_id = $this->request->getParameter('collection_id', pInteger);
		$start = $this->request->getParameter('start', pInteger);
		...
		if($collection_id){
			...
			$collection_ids = $t_item->get('ca_collections.children.collection_id', ['returnAsArray' => true, 'checkAccess' => caGetUserAccessValues($this->request), 'sort' => $o_collections_config->get("sub_collection_sort_list")]);
			
			$this->view->setVar('content', caGetCollectionHierarchyList($this->request, $collection_ids, $level, ['start' => $start, 'partial' => true]));
		}else{
			throw new ApplicationException("Invalid collection_id");
		}
		$this->render("Collections/collection_hierarchy_list_page_html.php");
	}
	# -------------------------------------------------------
