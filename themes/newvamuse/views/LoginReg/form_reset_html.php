...
<?php
	if($this->getVar("message")){
		print "<div class='alert alert-danger'>".(($this->getVar("message") == "E-mail address is not valid") ? "User name is not valid" : $this->getVar("message"))."</div>";
	}
...
?>
		...
		<?php
				...
				default:
?>
					<p><?php print _t("To reset your password enter the user name you used to register below. A message will be emailed with instructions on how to reset your password."); ?></p>
					<form id="ResetForm" action="<?php print caNavUrl($this->request, '', 'LoginReg', 'resetSend'); ?>" method="post" class="form-horizontal" role="form">
						<div class="form-group">
							<label for="reset_email" class="col-sm-4 control-label"><?php print _t("User Name"); ?></label>
							<div class="col-sm-7">
