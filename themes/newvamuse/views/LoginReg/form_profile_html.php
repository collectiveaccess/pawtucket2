...
<?php
		foreach(array("user_name", "fname", "lname", "email") as $vs_field){
			...
			print $t_user->htmlFormElement($vs_field,"<div class='form-group".(($va_errors[$vs_field]) ? " has-error" : "")."'><label for='".$vs_field."' class='col-sm-4 control-label'>^LABEL</label><div class='col-sm-7'>^ELEMENT".(($vs_field == "user_name") ? "<small>Used for site login</small>" : "")."</div><!-- end col-sm-7 --></div><!-- end form-group -->\n", array("classname" => "form-control"));
		}
