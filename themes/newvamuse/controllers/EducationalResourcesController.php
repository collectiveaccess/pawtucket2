...
class EducationalResourcesController extends DetailController {
	...
	function Resource($options = null) {
		...
		$type = $occ->getTypeCode();
		
		if(($v = caGetOption("view", $options, null)) && $this->viewExists($v)) {
			$this->render($v);
		} elseif($this->viewExists("EducationalResources/{$type}_html.php")) {
			$this->render("EducationalResources/{$type}_html.php");
		} else {
			$this->render("EducationalResources/item_html.php");
		}
	}
	# -------------------------------------------------------
	/**
