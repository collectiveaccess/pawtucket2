 	...
 	class LightboxController extends FindController {
         ...
         */
 		public function __construct(&$po_request, &$po_response, $pa_view_paths=null) {
			...
			$this->view->setVar('lightbox_displayname', $va_lightboxDisplayName["singular"]);
			$this->view->setVar('lightbox_displayname_plural', $va_lightboxDisplayName["plural"]);
         ...
         *
         */
 		function parent_list($pa_options = null) {
 			if($this->opb_is_login_redirect) { return; }

			
			$va_lightbox_displayname = caGetLightboxDisplayNameParent();
			$this->view->setVar('lightbox_displayname', $va_lightbox_displayname["singular"]);
			$this->view->setVar('lightbox_displayname_plural', $va_lightbox_displayname["plural"]);
 		...
 		function setDetail($pa_options = null) {
			...
			$this->view->setVar("browse_type", "caLightbox");	// this is only used when loading hierarchy facets and is a way to get around needing a browse type to pull the table in FindController		
 			...
 			if($this->request->getParameter("view", pString) == "xlxsOrderChart"){
 				...
 				$this->view->setVar('view', $ps_view = caCheckLightboxView(array('default' => 'list', 'request' => $this->request)));
			}
         ...
         */
 		function ajaxSaveSetInfo($pa_options = null) {
            ...
            if (!$this->request->isAjax()) { $this->response->setRedirect(caNavUrl($this->request, '', 'Lightbox', 'Index')); return; }
 			...
 			# --- if mode = parent we are making a parent parent set
 			$ps_mode = $this->request->getParameter('mode', pString);
...
# --- insert new parent here to link to new set
#					}
				}
			}
 			...
 			if(sizeof($va_errors) == 0){
				...
				if($t_set->numErrors()) {
					...
					if(($ps_mode == "parent") || ($vb_is_insert)){
						$this->view->setVar("message", _t('Saved gallery'));
 		...
 		function ajaxSaveSetItemInfo($pa_options = null) {
            if($this->opb_is_login_redirect) { return; }
            if (!$this->request->isAjax()) { $this->response->setRedirect(caNavUrl($this->request, '', 'Lightbox', 'parent_list')); return; }
 			...
 			if (!$vn_item_id) { $this->response->setRedirect(caNavUrl($this->request, '', 'Lightbox', 'parent_list')); return; }
            ...
            $this->view->setVar("display_name_plural", $vs_display_name_plural);
 			...
 			if($this->request->getParameter('set_id', pInteger)){
				...
				if($pn_parent_id < 0){
					...
					if(!$ps_parent_name){
						$ps_parent_name = _t("Your galleries");
					}
					...
					$t_parent->set('set_code', $this->request->getUserID().'_parent_'.time());
					$t_parent->insert();
					if($t_parent->numErrors()) {
						$va_errors["general"] = join("; ", $t_parent->getErrors());
						$this->view->setVar('errors', $va_errors);
						$this->addItemForm();
						return;
					}else{
