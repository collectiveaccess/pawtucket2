		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit()) {
				...
				if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result')){
					...
					$vs_medium = $vs_date = $vs_entity_detail_link = $vs_caption = "";
					if($vs_table === 'ca_objects'){
						$vs_rep_detail_link 	= caDetailLink($this->request, $vs_thumbnail, '', $vs_table, $vn_id);	
						$vs_medium 	= $qr_res->get("ca_objects.medium_container.display_medium_support");
						$vs_entity_detail_link 	= $qr_res->get("ca_entities.preferred_labels.displayname", array('restrictToRelationshipTypes' => array('artist', 'painter', 'school', 'attributed'), 'delimiter' => ', '));
						$vs_date = $qr_res->get('ca_objects.creation_date_display');
						$vs_caption = "";
						if($vs_medium){
							$vs_caption .= "<br/>".$vs_medium;
						}
						if($vs_entity_detail_link){
							$vs_caption .= "<br/>".$vs_entity_detail_link;
						}
						if($vs_date){
							$vs_caption .= "<br/>".$vs_date;
						}
					}
			...
			<div class='bResultListItem' id='row{$vn_id}' onmouseover='jQuery(\"#bResultListItemExpandedInfo{$vn_id}\").show();'  onmouseout='jQuery(\"#bResultListItemExpandedInfo{$vn_id}\").hide();'>
				...
				<div class='bResultListItemContent'><div class='text-center bResultListItemImg'>{$vs_rep_detail_link}</div>
					<div class='bResultListItemText'>
						<b>{$vs_label_detail_link}</b>{$vs_caption}
