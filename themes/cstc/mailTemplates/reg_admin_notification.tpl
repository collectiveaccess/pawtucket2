 ...
 */
	...
	print _t("A new user, %1, has registered for \"%2\".", trim($t_user->get("fname")." ".$t_user->get("lname")).", ".$t_user->get("email"), $this->request->config->get("app_display_name"));
			...
			if($t_user->getPreference($profile_pref)){
				switch($profile_pref){
					case "user_profile_type":
						print "\n\nAre you a community member or researcher?: ".str_replace("_", " ", $t_user->getPreference($profile_pref));
					break;
					# ------
					case "user_profile_community":
						print "\n\nCommunity: ".$t_user->getPreference($profile_pref);
					break;
				}
			}
		}
	...
	if($this->request->config->get("dont_approve_logins_on_registration")){
		print _t("\n\nPlease login and navigate to Manage > Access Control to approve the registration.");
	}
