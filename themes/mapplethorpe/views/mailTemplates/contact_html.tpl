...
foreach($va_fields as $vs_element => $va_options){
	if(($vs_element != "sendTo") && ($this->request->getParameter($vs_element, pString))){
		print "<p>";
		if($vs_element != "imageTag"){
			print "<b>".$va_options["label"].":</b> ";
		}
		print $this->request->getParameter($vs_element, pString)."</p>";
	}
}
