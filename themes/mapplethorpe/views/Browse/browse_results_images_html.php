					...
					$vs_result_output = "
						<div class='resultItemColImg col-xs-{$vn_col_span_xs} col-sm-{$vn_col_span_sm} col-md-{$vn_col_span}' id='row".$vn_id."'>".$vs_multiple_reps."
							<div class='resultContentImg'>
								...
								<div class='text-right'>{$vs_add_to_set_link}</div><div class='bSetsSelectMultiple'><input type='checkbox' name='object_ids' value='{$vn_id}'></div>
							</div>
						</div><!-- end col -->\n";
...
#		</div><!-- end col -->";
			...
			print "<div style='clear:both'></div>".caNavLink($this->request, _t('Next %1', $vn_hits_per_block), 'jscroll-next', '*', '*', '*', array('s' => $vn_start + $vn_results_output, 'key' => $vs_browse_key, 'view' => $vs_current_view, 'dontSetFind' => 1, 'sort' => $vs_current_sort, '_advanced' => $this->getVar('is_advanced') ? 1  : 0));
		}
?>
