...
?>
				<div class="btn-group">
					<span class="glyphicon glyphicon-cog bGear" data-toggle="dropdown" title="<?php print $vs_lightbox_displayname; ?> tools"></span>
						...
						if($qr_set_items->numHits()){
							...
							foreach($va_item_ids as $vn_i => $vn_item_id) {
								...
								if ($vs_tag = $va_representations[$vn_object_id]['tags'][$vs_media_version]) {
									...
									$vs_representation = caDetailLink($this->request, "<div class='lbItemImg'>".$vs_tag."</div>", '', 'ca_objects', $vn_object_id, "", array("title" => _t("View Item Detail")));
								} else {
