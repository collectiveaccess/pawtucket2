				...
				if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result')){
					...
					if($vs_table == 'ca_objects'){
						...
						if($vs_formation = $qr_res->get('ca_objects.lithostratigraphy.formation')){
							$vs_object_details .= '<strong>Formation: </strong>'.$vs_formation."<br>";
						}
						if($vs_period = $qr_res->get('ca_objects.chronostratigraphy.earliestPeriod')){
							$vs_object_details .= '<strong>Period(Early): </strong>'.$vs_period."<br>";
						}
					}
