...
<div class="row"><div class="col-sm-12 col-md-12 col-lg-8 col-lg-offset-2">
	<H1>Ontdek onze expo's<br/><hr/></H1><br/>
<?php
	if(is_array($va_sets) && sizeof($va_sets)){
		# --- main area with info about selected set loaded via Ajax				
			...
			foreach($va_sets as $vn_set_id => $va_set){
				...
				print "<div class='galleryList'>".caNavLink($this->request, $va_first_item["representation_tag"], '', '', 'Gallery', $vn_set_id).
							...
							<div><small>".$va_set["item_count"]." ".(($va_set["item_count"] == 1) ? _t("item") : _t("items"))."</small></div>
						</div>\n";
				print "</div><!-- end col -->";
				if($i == 4){
					print "</div><!-- end row -->";
