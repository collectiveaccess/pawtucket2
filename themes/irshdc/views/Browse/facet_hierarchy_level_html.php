...
#						$vs_buf .= "<a href='#' data-item_id='{$vn_id}' class='caSubItems caSubItem{$vs_facet_name}' title='".addslashes(_t('View sub-items'))."'>{$vs_name}</a><div id='children{$vn_id}' style='padding-left:25px;'></div>";
#					}else{
...
#				}else{
					...
					if((int)$va_children["children"] > 0){
						...
						$vs_buf .= ' <a href="#" title="'._t('View sub-items').'" onClick=\'jQuery("#children'.$vn_id.'").toggle(); jQuery("#children'.$vn_id.'").load("'.caNavUrl($this->request, '*', '*', 'getFacetHierarchyLevel', array('facet' => $vs_facet_name, 'key' => $vs_key, 'browseType' => $vs_browse_type, 'id' => $vn_id, 'isNav' => $vb_is_nav ? 1 : 0)).'"); return false;\'><span class="glyphicon glyphicon-chevron-down"></span></a>';
						$vs_buf .= '<div id="children'.$vn_id.'" style="padding-left:10px; display:none;"></div>';
					}
#				}
				$vs_buf .= "</div>";
...
<script type="text/javascript">
	jQuery(document).ready(function() {
		jQuery(".caSubItem<?php print $vs_facet_name; ?>").on('click', function(e) {
			jQuery('#children' + jQuery(this).data('item_id')).load('<?php print caNavUrl($this->request, '*', '*', $vs_browse_type); ?>', { getFacet: 1, facet: '<?php print $vs_facet_name; ?>', view: '<?php print $vs_view; ?>', key: '<?php print $vs_key; ?>', browseType: '<?php print $vs_browse_type; ?>', id: jQuery(this).data('item_id'), isNav: <?php print $vb_is_nav ? 1 : 0; ?>}, 
				function(){
				/*jQuery("#bMorePanel").show(); 
