<?php
/* ----------------------------------------------------------------------
 * pawtucket2/themes/default/views/ca_entities_search_html.php 
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
<?php
	if($vo_result) {
?>
		<h1><?php print ("Entities from the Digital Archive"); ?></h1>
<?php
...
<?php
		...
		if ((!$vs_view) || ($vo_result->numHits() == 0) || (!in_array($vs_view, array_keys($this->getVar('result_views'))))) {
			print $this->render('Results/ca_entities_search_no_results_html.php');
		}else{
			print $this->render('Results/ca_entities_results_'.$vs_view.'_html.php');
		}
?>		
	</div><!-- end sectionbox -->
<?php
