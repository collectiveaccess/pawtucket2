...
<?php
			if (!$this->request->config->get('dont_allow_registration_and_login')) {
				...
				}else if($this->request->getController() != 'LoginReg'){
...
<?php		
	if ($this->request->config->get('show_bristol_only')) {
		print "<div id='header' style='text-align:center'>";
	} else {
		print "<div id='header'>";
	}
				print caNavLink($this->request, "<img src='".$this->request->getThemeUrlPath()."/graphics/haring.jpg' border='0'>", "", "", "", "");
?>				
			</div><!-- end header -->
<?php
		if (!$this->request->config->get('show_bristol_only')) {
