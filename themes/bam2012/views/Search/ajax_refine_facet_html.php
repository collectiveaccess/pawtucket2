...
<script type="text/javascript">
	...
	jQuery(document).ready(function() {
		if (jQuery('.browseSelectPanelHeaderContent').width() > jQuery('.browseSelectPanelHeader').width()) {
			...
			jQuery('.browseSelectPanelHeaderContent');
...
<?php
	...
	switch($va_facet_info['group_mode']) {
		...
		case 'none':
?>
	<div class="browseSelectPanelListNone">
		<table class='browseSelectPanelListTableNone'>
<?php
			$va_row = array();
			foreach($va_facet as $vn_i => $va_item) {
?>
<?php
				$va_row[] = "<td class='browseSelectPanelListCellNone'>".caNavLink($this->request, $va_item['label'], 'browseSelectPanelLink', $this->request->getModulePath(), $this->request->getController(), ((strlen($vm_modify_id)) ? 'modifyCriteria' : 'addCriteria'), array_merge(array('facet' => $vs_facet_name, 'id' => $va_item['id'], 'mod_id' => $vm_modify_id), $va_other_params))."</td>";
...
?>
	<div class="browseSelectPanelHeader">
		<div class="browseSelectPanelHeaderContent"><nobr>
<?php 
	foreach($va_groups as $vs_group) {
