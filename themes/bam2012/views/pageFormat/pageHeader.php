...
<?php
		...
		if((!$this->request->config->get('dont_allow_registration_and_login')) || (is_array($va_ui_locales = $this->request->config->getList('ui_locales')) && (sizeof($va_ui_locales) > 1))){
?>
			<div id="topBar">
<?php
...
<?php
				print caNavLink($this->request, _t("Leon Levy BAM Digital Archive"), "", "", "", "");
?>				
			</div><!-- end headerText --></div><!-- end header -->
<?php
...
?>
			<div id="navArea">
				<div id="search">
						<a href="#" name="searchButtonSubmit" onclick="document.forms.header_search.submit(); return false;"><?php print "<img src='".$this->request->getThemeUrlPath()."/graphics/bam/magGlass.jpg' border='0'>"; ?></a>
						Search<form name="header_search" action="<?php print caNavUrl($this->request, '', 'Search', 'Index'); ?>" method="get">
				...
				<div id="nav">
<?php				
					print caNavLink($this->request, _t("Productions"), "", "", "Browse", "clearCriteria", array("target" => "ca_occurrences"))." ".caNavLink($this->request, _t("Objects"), "", "", "Browse", "clearCriteria", array("target" => "ca_objects"))." ".caNavLink($this->request, _t("People"), "", "", "Browse", "clearCriteria", array("target" => "ca_entities"));
?>
				</div><!-- end nav -->
				<div id="pageTitle">
<?php
				switch($this->request->getController()){
					...
					# --------------------------
					case "Browse":
						# --- need to add code here to figure out what target is when it isn't being passed
						if($this->request->session->getVar('pawtucket2_browse_target') == "ca_occurrences"){
							print _t("Productions");
						}elseif($this->request->session->getVar('pawtucket2_browse_target') == "ca_objects"){
							print _t("Objects");
						}elseif($this->request->session->getVar('pawtucket2_browse_target') == "ca_entities"){
							print _t("People");
						}
					break;
					# --------------------------
