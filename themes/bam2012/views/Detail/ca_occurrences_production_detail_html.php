...
?>
	...
	<?php
				...
				# --- production dates
				if($va_production_date = $t_occurrence->get("productionDate", array('delimiter' => '<br/>'))){
					print "<div id='productionDate'>";
					print _t("Performance dates");
					print "<div id='productionDateText'>".$va_production_date."</div>";
					print "</div><!-- end productionDate -->";
				}

				if ($va_idno = $t_occurrence->get('ca_occurrences.idno')) {
					print "<div class='mdItem'><div class='mdItemHeading'>"._t('Identifier')."</div>{$va_idno}</div><!-- end mdItemHeading -->";
				}
				
				if ($t_occurrence->get('ca_occurrences.venue') != "382"){
					if ($va_venue = $t_occurrence->get('ca_occurrences.venue', array('convertCodesToDisplayText' => true))) {
						print "<div class='mdItem'><div class='mdItemHeading'>"._t('Venue')."</div><div class='pieces'>{$va_venue}</div><div style='clear:both;width:100%'></div></div><!-- end mdItemHeading -->";
					}
				}
				if ($t_occurrence->get('ca_occurrences.premiere') != "119") {
					if ($va_premiere = $t_occurrence->get('ca_occurrences.premiere', array('convertCodesToDisplayText' => true))) {
				...
				if ($t_occurrence->get('ca_occurrences.productionLanguage') != "408"){
					if ($va_production = $t_occurrence->get('ca_occurrences.productionLanguage', array('convertCodesToDisplayText' => true))) {
						print "<div class='mdItem'><div class='mdItemHeading'>"._t('Language')."</div>{$va_production}</div><!-- end mdItemHeading -->";
					}
				}		
				# --- description
...
<?php
		...
		if(sizeof($va_genres) > 0){
			...
			foreach($va_genres as $va_genre){
				print "<div class='relatedItem'><span class='capsText'>".caNavLink($this->request, $va_genre['genre'], '', '', 'Search', 'Index', array('search' => "ca_occurrences.genre:{$va_genre['genre']}"))."</span></div>";
			}
