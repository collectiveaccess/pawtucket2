 ...
 */
	...
	if($this->getVar('num_pages') > 1){
		...
		if ($this->getVar('page') > 1) {
			print "<div id='relatedObjectsPrevious'><a href='#' onclick='jQuery(\"#resultBox\").load(\"".caNavUrl($this->request, $this->request->getModulePath(), $this->request->getController(), $this->request->getAction(), array_merge(array('page' => $this->getVar('page') - 1), $va_other_paging_parameters))."\"); return false;'><img src='".$this->request->getThemeUrlPath()."/graphics/bam/relatedObjectsPrevious.png' border='0'></a></div>";
		}else{
			print "<div id='relatedObjectsPreviousSpacer'><img src='".$this->request->getThemeUrlPath()."/graphics/bam/relatedObjectsPrevious.png' border='0'></div>";
		}
	}
	...
	while(($vn_itemc < $this->getVar('items_per_page')) && ($qr_hits->nextHit())) {
		...
		$va_media_info = $qr_hits->getMediaInfo('ca_object_representations.media', 'relatedthumb', null, array('checkAccess' => $va_access_values));
		$vn_padding_top = 0;
		$vn_padding_top_bottom =  ((130 - $va_media_info["HEIGHT"]) / 2);
		print "<div class='imgBox'>";
		print caNavLink($this->request, $qr_hits->getMediaTag('ca_object_representations.media', 'relatedthumb', array('checkAccess' => $va_access_values)), '', 'Detail', 'Object', 'Show', array('object_id' => $qr_hits->get('ca_objects.object_id')));
	...
	if($this->getVar('num_pages') > 1){
		if ($this->getVar('page') < $this->getVar('num_pages')) {
			print "<div id='relatedObjectsNext'><a href='#' onclick='jQuery(\"#resultBox\").load(\"".caNavUrl($this->request, $this->request->getModulePath(), $this->request->getController(), $this->request->getAction(), array_merge(array('page' => $this->getVar('page') + 1), $va_other_paging_parameters))."\"); return false;'><img src='".$this->request->getThemeUrlPath()."/graphics/bam/relatedObjectsNext.png' border='0'></a></div>";
		}else{
			print "<div id='relatedObjectsPreviousSpacer'><img src='".$this->request->getThemeUrlPath()."/graphics/bam/relatedObjectsPrevious.png' border='0'></div>";
		}
	}
