...
<?php
		...
		if ($vs_facet_name == "manu_label") {
			foreach($va_facet_content as $vn_id => $va_item) {
				print "<div class='browseFacetItem col-sm-4 col-md-3'>".caNavLink($this->request, $va_item['label'], '', '', 'Detail', 'entities/'.$va_item['id'])."</div>";
				$vn_c++;
		...
		if ($vn_facet_size >= ((int)$vn_start + (int)$vn_items_per_page)) {
