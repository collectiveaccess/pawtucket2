					...
					if ($va_top_level) {
						if ($qr_top_level = caMakeSearchResult('ca_collections', $va_top_level, array('sort' => ['ca_collections.preferred_labels']))) {
							...
							while($qr_top_level->nextHit()) {
								...
								$vs_buf.= "<div>".((is_array($va_series_level) && (sizeof($va_series_level) > 0)) ? "<a href='#' onclick='$(\".seriesLevel".$va_top_level_id."\").toggle(200);return false;'><i class='fa fa-plus-square-o'></i> </a>" : "<span class='colspacer'></span>");
...
{{{<ifcount code="ca_objects" min="1">
			...
			<script type="text/javascript">
				jQuery(document).ready(function() {
					jQuery("#browseResultsContainer").load("<?php print caNavUrl($this->request, '', 'Search', 'objects', array('search' => 'ca_collections.collection_id:^ca_collections.collection_id'), array('dontURLEncodeParameters' => true)); ?>", function() {
