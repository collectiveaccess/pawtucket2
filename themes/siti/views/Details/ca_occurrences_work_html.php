...
<?php					
					...
					$va_events = $t_item->get('ca_occurrences.related', array('restrictToTypes' => array('production','training','special_event'), 'returnWithStructure' => true, 'checkAccess' => $va_access_values));
					if (sizeof($va_events)) {
						...
						print "</div></div><!-- end unit -->";
						if($vb_show_view_all){
							print "<div class='unit text-center'>".caNavLink($this->request, "View All Productions, Trainings & Events", "btn btn-default", "", "Browse", "productions", array("facet" => "work_general_facet", "id" => $t_item->get("ca_occurrences.occurrence_id")))."</div>";
						}
					}
?>					
