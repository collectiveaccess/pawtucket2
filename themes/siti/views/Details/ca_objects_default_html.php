...
<?php					
					...
					$va_all = $t_object->get('ca_occurrences', array('restrictToTypes' => array('production','training','special_event'), 'returnWithStructure' => true, 'checkAccess' => $va_access_values));
					$va_productions = $t_object->get('ca_occurrences', array('restrictToTypes' => array('production'), 'returnWithStructure' => true, 'checkAccess' => $va_access_values));
					...
					$va_events = $t_object->get('ca_occurrences', array('restrictToTypes' => array('training','special_event'), 'returnWithStructure' => true, 'checkAccess' => $va_access_values));
					if (sizeof($va_events)) {
						$va_related_list = array();
					...
					if(sizeof($va_all) > 18){
						print "<div class='unit text-center'>".caNavLink($this->request, "View All Productions, Trainings & Events", "btn btn-default", "", "Browse", "productions", array("facet" => "object_general_facet", "id" => $t_object->get("ca_objects.object_id")))."</div>";
					}
?>					
