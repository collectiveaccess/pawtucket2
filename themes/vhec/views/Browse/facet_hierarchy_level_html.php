 ...
 */
	...
	foreach($va_facet_list as $vn_key => $va_facet){
		foreach($va_facet as $vn_id => $va_children){
			...
			$vs_parent_id = $va_children['parent_id'];
			...
			if(isset($vs_name)){
				...
				if($vs_link_to == "morePanel"){
					...
					$t_list_item = new ca_list_items($va_children['parent_id']);
					$vs_parent_link = caNavLink($this->request, $t_list_item->get('ca_list_items.preferred_labels'), '', '*', '*', $vs_browse_type, array('key' => $vs_key, 'facet' => $vs_facet_name, 'id' => $vs_parent_id, 'isNav' => $vb_is_nav ? 1 : 0));
	...
	if ($vs_link_to != "morePanel") {
		print "<h5 style='padding-bottom:15px;'>".$vs_parent_link."</h5>";
	}
	if(sizeof($va_links)){
		ksort($va_links);
		print join($va_links, "\n");
