...
?>					
	...
	<?php
					...
					if ($vs_type == "Library Item") {
						...
						if ($va_classification = $t_object->get('ca_objects.classification')) {
							print "<div class='unit'><h8>Classification</h8>".$va_classification."</div>";
						}
						if ($va_collection = $t_object->get('ca_objects.related.preferred_labels', array('restrictToRelationshipTypes' => array('part_of'), 'delimiter' => '<br/>', 'returnAsLink' => true))) {
							print "<div class='unit'><h8>Part of</h8>".$va_collection."</div>";
						}
						if ($va_assoc_materials = $t_object->get('ca_objects.associated_url')) {
							print "<div class='unit'><h8>Associated Materials</h8>".$va_assoc_materials."</div>";
						}
...
<?php
			...
			if ($va_related_museum_objects = $t_object->get('ca_objects.related.object_id', array('returnWithStructure' => true, 'restrictToTypes' => array('work'), 'checkAccess' => $va_access_values))) {
				foreach ($va_related_museum_objects as $va_key => $va_related_museum_object_id) {				
					...
					$vs_related_museum.= "<div>".$t_museum->get('ca_objects.preferred_labels')."</div>";
			...
			if ($va_related_holdings_objects = $t_object->get('ca_objects.related.object_id', array('returnWithStructure' => true, 'restrictToTypes' => array('archival', 'library', 'survivor'), 'checkAccess' => $va_access_values))) {
				foreach ($va_related_holdings_objects as $va_key => $va_related_holdings_object_id) {				
					...
					$vs_related_holdings.= "<div class='relatedThumb'>".$t_holding->get('ca_object_representations.media.widepreview');
					$vs_related_holdings.= "<div>".$t_holding->get('ca_objects.preferred_labels')."</div>";
			...
			if ($va_related_collections = $t_object->get('ca_collections.collection_id', array('returnWithStructure' => true, 'checkAccess' => $va_access_values))) {
			...
			<div class='row'>
				<div class='col-sm-12'>
					...
					<div class='container' id='relationshipTable'>
						<ul class='row'>
<?php						
							if ($vs_related_museum) { print '<li><a href="#museumTab">Museum Works</a></li>'; }
