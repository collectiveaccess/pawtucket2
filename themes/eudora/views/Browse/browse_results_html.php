		...
		if(is_array($va_views) && (sizeof($va_views) > 1)){
			foreach($va_views as $vs_view => $va_view_info) {
				if ($vs_current_view === $vs_view) {
					print '<a href="#" class="active"><i class="'.$va_view_icons[$vs_view]['icon'].'"></i></a> ';
				} else {
					print caNavLink($this->request, '<i class="'.$va_view_icons[$vs_view]['icon'].'"></i>', 'disabled', '*', '*', '*', array('view' => $vs_view, 'key' => $vs_browse_key)).' ';
				}
			}
		}
?>
