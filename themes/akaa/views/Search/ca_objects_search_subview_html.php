...
<?php 
				$vs_image = $qr_results->get('ca_object_representations.media.largewidepreview', array("checkAccess" => $va_access_values));
				if(!$vs_image){
...
<?php				
				if($vs_block == "artworks"){
					...
					print "<div>".caDetailLink($this->request, $qr_results->get("ca_objects.preferred_labels"), '', 'ca_objects', $qr_results->get('ca_objects.object_id'))."</div>";
				}
?>					
			</div><!-- end blockResult -->
<?php
