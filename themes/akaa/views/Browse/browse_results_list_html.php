		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit()) {
				...
				if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result')){
					...
					$vs_info = null;
					if ($vs_creator = $qr_res->get('ca_entities.preferred_labels', array('restrictToRelationshipTypes' => array('creator')))) {
						$vs_label_detail_link = "<div>".caDetailLink($this->request, $vs_creator.", ".$qr_res->get("{$vs_table}.preferred_labels"), '', $vs_table, $vn_id)."</div>";
					} else {$vs_creator = null;}
					if ($qr_res->get('ca_objects.date.dates_value')) {
						$vs_info.= "<div>".$qr_res->get('ca_objects.date.dates_value')."</div>";
					}					
					...
					$vs_result_output = "
		<div class='bResultListItemCol col-xs-12 col-sm-6 col-md-4 col-lg-3'>
