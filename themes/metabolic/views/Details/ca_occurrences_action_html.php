						...
						<ifdef code="ca_occurrences.external_link">
							...
							<dd>
								<unit relativeTo="ca_occurrences.external_link" delimiter="<br/>">
									<a href="^ca_occurrences.external_link" target="_blank" rel="noopener">^ca_occurrences.external_link</a> 
								</unit>
							</dd>
						</ifdef>
						<ifdef code="ca_occurrences.phase">
							<dt><?= _t('Phase'); ?></dt>
							<dd>^ca_occurrences.phase%delimiter=,_</dd>
						</ifdef>
					</dl>}}}

					<?php
						if($t_item->get("ca_occurrences.subject")){
							if($subs = caGetBrowseLinks($t_item, 'ca_occurrences.subject', ['template' => '<l>^ca_occurrences.subject</l>', 'linkTemplate' => '^LINK'])) {
					?>
								<dt><?= _t('Themes'); ?></dt>
								<dd><?= join(", ", $subs); ?></dd>
					<?php
				...
				<div class="col">
					{{{<dl class="mb-0">
						<ifcount code="ca_collections" min="1">
							<dt><ifcount code="ca_collections" min="1" max="1"><?= _t('Action'); ?></ifcount><ifcount code="ca_collections" min="2"><?= _t('Actions'); ?></ifcount></dt>
