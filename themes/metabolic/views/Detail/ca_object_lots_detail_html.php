<?php
/* ----------------------------------------------------------------------
 * pawtucket2/themes/default/views/Detail/ca_object_lots_detail_html.php : 
 * ----------------------------------------------------------------------
 * CollectiveAccess
 ...
 * ----------------------------------------------------------------------
 */
	$t_object_lot 			= $this->getVar('t_item');
	$vn_lot_id 		= $t_object_lot->getPrimaryKey();
...
?>
	<div id="detailBody">
		<div id="pageNav">
<?php
			if (($this->getVar('is_in_result_list')) && ($vs_back_link = ResultContext::getResultsLinkForLastFind($this->request, 'ca_object_lots', _t("Back"), ''))) {
...
?>
		...
		<h1><?php print unicode_ucfirst($t_object_lot->getTypeName()).': '.$vs_title; ?></h1>
		<div id="rightCol">	
<?php
...
<?php
			...
			# --- identifier
			if($t_object_lot->get('idno')){
				print "<div class='unit'><b>"._t("Identifier")."</b>: ".$t_object_lot->get('idno')."</div><!-- end unit -->";
			}
			# --- attributes
			$va_attributes = $this->request->config->get('ca_object_lots_detail_display_attributes');
			if(is_array($va_attributes) && (sizeof($va_attributes) > 0)){
				foreach($va_attributes as $vs_attribute_code){
					if($vs_value = $t_object_lot->get("ca_object_lots.{$vs_attribute_code}")){
						print "<div class='unit'><b>".$t_object_lot->getDisplayLabel("ca_object_lots.{$vs_attribute_code}").":</b> {$vs_value}</div><!-- end unit -->";
					}
				}
			}
			# --- description
			if($this->request->config->get('ca_object_lots_description_attribute')){
				if($vs_description_text = $t_object_lot->get("ca_object_lots.".$this->request->config->get('ca_object_lots_description_attribute'))){
					print "<div class='unit'><div id='description'><b>".$t_object_lot->getDisplayLabel("ca_object_lots.".$this->request->config->get('ca_object_lots_description_attribute')).":</b> {$vs_description_text}</div></div><!-- end unit -->";				
?>
...
<?php
			...
			# --- entities
			$va_entities = $t_object_lot->get("ca_entities", array("returnAsArray" => 1, 'checkAccess' => $va_access_values));
...
<?php
			...
			# --- occurrences
			$va_occurrences = $t_object_lot->get("ca_occurrences", array("returnAsArray" => 1, 'checkAccess' => $va_access_values));
...
<?php
			...
			# --- places
			$va_places = $t_object_lot->get("ca_places", array("returnAsArray" => 1, 'checkAccess' => $va_access_values));
			...
			# --- collections
			$va_collections = $t_object_lot->get("ca_collections", array("returnAsArray" => 1, 'checkAccess' => $va_access_values));
			if(sizeof($va_collections) > 0){
				print "<div class='unit'><h2>"._t("Related Collection").((sizeof($va_collections) > 1) ? "s" : "")."</h2>";
				foreach($va_collections as $va_collection_info){
					print "<div>".(($this->request->config->get('allow_detail_for_ca_collections')) ? caNavLink($this->request, $va_collection_info['label'], '', 'Detail', 'Collection', 'Show', array('lot_id' => $va_collection_info['lot_id'])) : $va_collection_info['label']);
					print " (".$va_collection_info['relationship_typename'].")</div>";
				}
				print "</div><!-- end unit -->";
			}
			# --- vocabulary terms
			$va_terms = $t_object_lot->get("ca_list_items", array("returnAsArray" => 1, 'checkAccess' => $va_access_values));
