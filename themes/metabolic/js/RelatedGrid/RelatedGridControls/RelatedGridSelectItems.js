...
const RelatedGridSelectItems = (props) => {
  ...
  return (
    <div id="bSelectOptions">
        ...
        <div className="dropdown-menu dropdown-menu-right" aria-labelledby="selectItemsIcon">
          {showSelectButtons ?
            <a className="dropdown-item" onClick={(e)=>clearSelectedItems(e)}>{(selectedGridItems.length >= 1) ? "Clear selection" : 'Cancel selection'}</a>
            :
            <a className="dropdown-item" onClick={(e)=>displaySelectButtons(e)}>Select items</a>
          }
        </div>
			</div>
		</div>
