...
const getSessionList = (url, callback) => {
  ...
  client
    .query({
      query: gql`
        query { sessionList { sessions { label, sessionKey, user_id, username, email, status, statusDisplay, createdOn, lastActivityOn, source, files, filesImported, totalSize, totalBytes, receivedSize, receivedBytes, errors { filename, message}, warnings { filename, message }, searchUrl }}}`
    })
