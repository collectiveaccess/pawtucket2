...
<?php
		...
		foreach($va_facet_content as $vn_id => $va_item) {
			...
			print "<div class='browseFacetItem col-sm-4 col-md-3'>".caNavLink($va_item['label'].$vs_content_count, '', '*', '*', '*', array('facet' => $vs_facet_name, 'id' => $va_item['id'], 'view' => $vs_view, 'key' => $vs_key))."</div>";
			$vn_c++;
		...
		if ((int)$vn_facet_size >= ((int)$vn_start + (int)$vn_items_per_page)) {
			print caNavLink(caBusyIndicatorIcon($this->request).' '._t('Loading'), 'caNextPage', '*', '*', '*', array('facet' => $vs_facet_name, 'getFacet' => 1, 'key' => $vs_key, 'isNav' => $vb_is_nav ? 1 : 0, 's' => $vn_start + $vn_items_per_page));
		}
		if ($vn_start == 0) {
?>
			</div>
...
<?php
		...
		foreach($va_facet_content as $vn_id => $va_item) {
			...
			$vs_facet_list .= "<div>".caNavLink($va_item['label'].$vs_content_count, '', '*', '*', '*', array('facet' => $vs_facet_name, 'id' => $va_item['id'], 'view' => $vs_view, 'key' => $vs_key))."</div>\n";
			$vn_c++;
		}
