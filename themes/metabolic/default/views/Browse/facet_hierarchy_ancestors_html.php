 ...
 */
	...
	if(is_array($va_ancestors) && sizeof($va_ancestors)){
			...
			print '<div style="float:right;"><a href="#" onClick=\'jQuery("#bHierarchyListMorePanel_'.$vs_facet_name.(($vb_is_nav) ? "Nav" : "").'").load("'.caNavUrl('*', '*', 'getFacetHierarchyLevel', array('facet' => $vs_facet_name, 'key' => $vs_key, 'browseType' => $vs_browse_type, 'isNav' => $vb_is_nav ? 1 : 0, 'id' => (int)$va_ancestor['parent_id'])).'"); jQuery(".bAncestorList_'.$vs_facet_name.(($vb_is_nav) ? "Nav" : "").'").load("'.caNavUrl('*', '*', 'getFacetHierarchyAncestorList', array('facet' => $vs_facet_name, 'browseType' => $vs_browse_type, 'key' => $vs_key, 'isNav' => $vb_is_nav ? 1 : 0)).'"); return false;\'><span class="glyphicon glyphicon-arrow-up"></span></a> '._t('Top').'</div>';
		#}	
		$vn_c = 0;
		foreach($va_ancestors as $va_ancestor){
			$va_ancestor = $va_ancestor['NODE'];
			
			print '<a href="#" onClick=\'jQuery("#bHierarchyListMorePanel_'.$vs_facet_name.(($vb_is_nav) ? "Nav" : "").'").load("'.caNavUrl('*', '*', 'getFacetHierarchyLevel', array('facet' => $vs_facet_name, 'key' => $vs_key, 'browseType' => $vs_browse_type, 'isNav' => $vb_is_nav ? 1 : 0, 'id' => (int)$va_ancestor[$vs_primary_key])).'"); jQuery(".bAncestorList_'.$vs_facet_name.(($vb_is_nav) ? "Nav" : "").'").load("'.caNavUrl('*', '*', 'getFacetHierarchyAncestorList', array('facet' => $vs_facet_name, 'browseType' => $vs_browse_type, 'key' => $vs_key, 'id' => $va_ancestor[$vs_primary_key], 'isNav' => $vb_is_nav ? 1 : 0)).'"); return false;\'>'.caTruncateStringWithEllipsis($va_ancestor[$vs_display_field], 40).'</a>';
