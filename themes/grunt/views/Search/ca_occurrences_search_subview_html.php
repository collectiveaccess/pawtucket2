...
?>
				<span class='multisearchFullResults'><?php print caNavLink($this->request, '<span class="glyphicon glyphicon-list" aria-label="list icon"></span> '._t('Full results'), '', '', 'Search', '{{{block}}}', array('search' => str_replace("/", "", $vs_search))); ?></span> | 
<?php
				}
?>
...
<?php
		...
		while($qr_results->nextHit()) {
			if ($vn_i == 0) { print "<div class='{{{block}}}Set authoritySet'>\n"; $vb_div_open = true; }
				print $qr_results->getWithTemplate("<l><div class='".$vs_block."Result authorityResult'>".$vs_caption_template."</div></l>", array("checkAccess" => $va_access_values));
			$vn_count++;
			$vn_i++;
