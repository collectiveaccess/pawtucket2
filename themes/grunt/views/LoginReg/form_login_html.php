...
?>
					<a href="#" onClick="jQuery('#caMediaPanelContentArea').load('<?php print caNavUrl($this->request, '', 'LoginReg', 'registerForm', null); ?>');"><?php print _t("Click here to create an account"); ?></a>
					<br/>
<?php
					}
?>
...
<?php
					...
					if (!$this->request->config->get(['dontAllowRegistrationAndLogin', 'dont_allow_registration_and_login']) && !$this->request->config->get('dontAllowRegistration')) {
						print caNavLink($this->request, _t("Click here to create an account"), "", "", "LoginReg", "registerForm", array());
					}
