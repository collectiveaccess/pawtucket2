 ...
 */
	...
	$vn_facet_display_length_initial = 15;
...
<?php
				...
				if($vn_facet_size > $vn_facet_display_length_initial){
					print "<h3><a href='#' onclick='jQuery(\"#bMorePanel\").load(\"".caNavUrl($this->request, '*', '*', '*', array('getFacet' => 1, 'facet' => $vs_facet_name, 'view' => $vs_view, 'key' => $vs_key))."\", function(){jQuery(\"#bMorePanel\").show(); jQuery(\"#bMorePanel\").mouseleave(function(){jQuery(\"#bMorePanel\").hide();});}); return false;'>".$va_facet_info['label_singular']."</a></h3>";
				}else{
					print "<h3>".$va_facet_info['label_singular']."</h3>";
				} 
