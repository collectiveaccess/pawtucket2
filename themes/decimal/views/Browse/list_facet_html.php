...
<?php
		...
		foreach($va_facet_content as $vn_id => $va_item) {
			...
			print "<div class='browseFacetItem col-sm-4 col-md-3'>".caNavLink($this->request, ucfirst($va_item['label']).$vs_content_count, '', '*', '*', '*', array('facet' => $vs_facet_name, 'id' => $va_item['id'], 'view' => $vs_view, 'key' => $vs_key))."</div>";
			$vn_c++;
...
<?php
		...
		foreach($va_facet_content as $vn_id => $va_item) {
			...
			$vs_facet_list .= "<div>".caNavLink($this->request, ucfirst($va_item['label']).$vs_content_count, '', '*', '*', '*', array('facet' => $vs_facet_name, 'id' => $va_item['id'], 'view' => $vs_view, 'key' => $vs_key))."</div>\n";
			$vn_c++;
		}
