	...
	if($va_collection_children = $t_item->get('ca_collections.children.collection_id', array('returnAsArray' => true, 'checkAccess' => $va_access_values, 'sort' => 'ca_collections.idno_sort'))){
		...
		if($qr_collection_children->numHits()){
			while($qr_collection_children->nextHit()){
				if($qr_collection_children->get("ca_objects.object_id", array('returnAsArray' => true, 'checkAccess' => $va_access_values)) || $qr_collection_children->get("ca_collections.children.collection_id", array('returnAsArray' => true, 'checkAccess' => $va_access_values, 'sort' => 'ca_collections.idno_sort'))){
...
<?php
					if($qr_collection_children->numHits()){
						while($qr_collection_children->nextHit()) {
							...
							if($vb_link_sublist || $vn_rel_object_count){
								print "<a href='#' class='p-3 d-block bg-body-tertiary text-white fw-medium' hx-target='#collectionLoad' hx-get='".caNavUrl($this->request, '', 'Collections', 'childList', array('collection_id' => $qr_collection_children->get("ca_collections.collection_id")))."'>".$vs_icon." ".$qr_collection_children->get('ca_collections.preferred_labels').", ".$qr_collection_children->get('ca_collections.idno').$vs_record_count."</a>";
							}else{
								...
								if($vb_link_to_detail){
									print caDetailLink($this->request, $vs_icon." ".$qr_collection_children->get('ca_collections.preferred_labels').", ".$qr_collection_children->get('ca_collections.idno')." ".(($o_collections_config->get("link_out_icon")) ? $o_collections_config->get("link_out_icon") : "").$vs_record_count, 'p-3 d-block bg-body-tertiary text-white fw-medium', 'ca_collections',  $qr_collection_children->get("ca_collections.collection_id"));
								}else{
									print "<div class='p-3 d-block bg-body-tertiary fw-medium'>".$vs_icon." ".$qr_collection_children->get('ca_collections.preferred_labels').", ".$qr_collection_children->get('ca_collections.idno').$vs_record_count."</div>";
								}
							}
							print "</div>";	
