...
?>
		<div class="form-group">
			<label><?php print _t('Terms and conditions'); ?></label>
			<div><input name="iAgreeToTerms" value="1" id="iAgreeToTerms" type="checkbox" checked/>
			{{{contribute_terms_conditions}}}</div>
		</div>
		<script type="text/javascript">
			jQuery(document).ready(function() {
				jQuery('#ContributeForm').submit(function() {
					if (jQuery('#iAgreeToTerms').length > 0) {
						if (!jQuery('#iAgreeToTerms').prop('checked')) {
