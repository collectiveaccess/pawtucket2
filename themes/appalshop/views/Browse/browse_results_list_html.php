		...
		if ($vn_start < $qr_res->numHits()) {
			...
			while($qr_res->nextHit()) {
				...
				if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($vs_cache_key,'browse_result')){
					...
					$vs_description = $vs_type = $vs_collection = "";
					if($vs_table == "ca_objects"){
						$vs_description = "";
						...
						if($vs_tmp = $qr_res->getWithTemplate('<unit unique="1" relativeTo="ca_collections"><l>^ca_collections.preferred_labels.name</l></unit>')){
							$vs_description .= "<span class='listcollection'>".$vs_tmp."</span>";
						}
			...
			<div class='bResultListItem' id='row{$vn_id}'>
				...
				<div class='bResultListItemContent'><div class='text-center bResultListItemImg'>{$vs_rep_detail_link}</div>
					<div class='bResultListItemText'>						
						<span class='larger'>{$vs_label_detail_link}</span><br/>
						<small>{$vs_type}</small><br/>
						{$vs_description}
					</div><!-- end bResultListItemText -->
					<div class='bResultLightbox'>{$vs_add_to_set_link}</div>
