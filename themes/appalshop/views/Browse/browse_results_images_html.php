...
if ($start < $qr_res->numHits()) {
	...
	if ($table != 'ca_objects') {
		...
		$va_images = caGetDisplayImagesForAuthorityItems($table, $va_ids, array('version' => 'large', 'relationshipTypes' => caGetOption('selectMediaUsingRelationshipTypes', $va_options, null), 'objectTypes' => caGetOption('selectMediaUsingTypes', $va_options, null), 'checkAccess' => $va_access_values));
	...
	while($qr_res->nextHit()) {
		...
		if(($o_config->get("cache_timeout") > 0) && ExternalCache::contains($cache_key,'browse_result')){
			...
			$image = $qr_res->get('ca_object_representations.media.large', array("checkAccess" => $va_access_values, "class" => $image_class));
			if(!$image){
				if($va_images[$id]){
