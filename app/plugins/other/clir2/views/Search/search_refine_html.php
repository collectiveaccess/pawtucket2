...
<?php
		foreach($va_criteria as $vs_facet_name => $va_row_ids) {
			foreach($va_row_ids as $vn_row_id => $vs_label) {
				if (!in_array($vs_facet_name, array('_search', 'curatorial_selection_facet'))) {
					print $vs_label;
					print caNavLink($this->request, 'x', 'close', 'clir2', $this->request->getController(), 'removeCriteria', array('facet' => $vs_facet_name, 'id' => $vn_row_id));
				}
			}
		}
	print caNavLink($this->request, _t('clear all filters'), 'button', 'clir2',  $this->request->getController(), 'clearCriteria', array()).' <img src="'.$this->request->getThemeUrlPath().'/graphics/arrow_right_gray.gif" width="7" height="8" border="0">';
?>
