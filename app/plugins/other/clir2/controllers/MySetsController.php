 ...
 */
	...
	require_once(__CA_MODELS_DIR__."/ca_occurrences.php");
 	...
 	class MySetsController extends ActionController {
 		# -------------------------------------------------------
 		/** 
 		...
 		private function _getSetID() {
 			...
 			if (!$vn_set_id = $this->request->getParameter('set_id', pInteger)) {			// try to get set_id from request
 				if (!$vn_set_id = $this->request->user->getVar('current_set_id')) {		// get last used set_id
 					$t_set = new ca_sets();
 					$va_sets = caExtractValuesByUserLocale($t_set->getSets(array('table' => 'ca_occurrences', 'user_id' => $this->request->getUserID())));
 		...
 		function index() {
 			...
 			$this->view->setVar('set_description', $t_set->get("ca_sets.description"));
 			...
 			$this->render('MySets/sets_html.php');
 		}
 		# -------------------------------------------------------
 		public function addItem() {
 			...
 			if (!$t_set = $this->_getSet()) { 
				...
				$t_new_set->set('table_num', 67); // 67=ca_occurrences
				...
				if (!$t_new_set->numErrors()) {
					if ($vn_new_set_id = $t_new_set->getPrimaryKey()) {
						$t_new_set->addLabel(array('name' => _t("Your first set")), $g_ui_locale_id, null, true); 				
 			...
 			if (!$t_set) {
 				$va_errors[] = _t('Could not create set for user');
 			} else {
				$pn_item_id = null;
				$pn_occurrence_id = $this->request->getParameter('occurrence_id', pInteger);
				if ($pn_item_id = $t_set->addItem($pn_occurrence_id, array(), $this->request->getUserID())) {
					$va_errors = array();
					$this->view->setVar('message', _t("Successfully added item"));
				} else {
					$va_errors[] = _t('Could not add item to set');
				}
			}
 			
 			$t_row = new ca_occurrences($pn_occurrence_id);
 		...
 		public function saveSetInfo() {
 			...
 			if(!$ps_name){
 				$va_errors_edit_set["name"] = _t("You must enter a name for your set");
 			}
 			...
 			if(sizeof($va_errors_edit_set) == 0){ 		
				if ($t_set->load($pn_set_id) && $t_set->haveAccessToSet($this->request->getUserID(), __CA_SET_EDIT_ACCESS__)) { 
					...
					$va_intro_texts = $t_set->getAttributesByElement('description');
					if(sizeof($va_intro_texts)) {
						// edit existing attribute
						$vn_attribute_id = $va_intro_texts[0]->getAttributeID();
						$t_set->editAttribute($vn_attribute_id, 'description', array('description' => $vs_desc, 'locale_id' => $g_ui_locale_id));
					} else {
						// create new attribute
						$t_set->addAttribute(array('description' => $vs_desc, 'locale_id' => $g_ui_locale_id), 'description');
					}
 		...
 		public function addNewSet() {
 			...
 			$ps_name = $this->request->getParameter('name', pString);
 			if(!$ps_name){
 				$va_errors_new_set["name"] = _t("Please enter the name of your set");
 			}
 			$vs_desc =  $this->request->getParameter('description', pString);
 			...
 			if(sizeof($va_errors_new_set) == 0){
				...
				// create new attribute
				$t_new_set->addAttribute(array('description' => $vs_desc, 'locale_id' => $g_ui_locale_id), 'description');
 		...
 		public function slideshow() {
			...
			$this->view->setVar('t_set', $t_set);
 			...
 			$this->render('MySets/sets_slideshow_html.php');
 		}
 		# -------------------------------------------------------
 		# XML data providers
 		# -------------------------------------------------------
 		public function getSetXML() {
			...
			$this->view->setVar('items', caExtractValuesByUserLocale($t_set->getItems(array('thumbnailVersion' => 'large', 'checkAccess' => $va_access_values, 'user_id' => $this->request->getUserID()))));
 			$this->render('MySets/xml_set_items.php');
 		}
 		# -------------------------------------------------------
 		# Ajax handlers
 		# -------------------------------------------------------
 		public function ReorderItems() {
 			...
 			$this->view->setVar('errors', $va_errors);
 			$this->render('MySets/ajax_reorder_items_json.php');
 		}
 		# -------------------------------------------------------
 		public function DeleteItem() {
 			if ($this->request->isLoggedIn()) { 
				...
				if ($t_set->removeItemByItemID($pn_item_id, $this->request->getUserID())) {
					...
					$va_errors[] = _t('Could not remove item from set');
				}
				$this->view->setVar('set_id', $pn_set_id);
 			...
 			$this->view->setVar('errors', $va_errors);
 			$this->render('MySets/ajax_delete_item_json.php');
 		}
 		# -------------------------------------------------------
 	}
 ?>