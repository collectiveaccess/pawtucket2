...
<?php
			...
			foreach($va_reps as $vn_object_id => $va_rep_info){
				print "<a href='#' ".(($pn_object_id == $va_rep_info['object_id']) ? "class='selectedRep' " : "")."onClick='jQuery(\"#{$vs_container_id}\").load(\"".caNavUrl($this->request, '', 'educational/SetsOverlay', 'getSetsOverlay', array('set_id' => $pn_set_id, 'object_id' => $va_rep_info['object_id'], 'representation_id' => $va_rep_info['representation_id']))."\");'>".$va_rep_info['rep_'.$vs_version]."</a>";
				$i++;
				if($i == $vn_num_cols){
					$i = 0;
					print "<br/>";
