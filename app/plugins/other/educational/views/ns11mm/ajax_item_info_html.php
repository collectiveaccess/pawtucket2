...
<?php
	if($va_item['previous_id']){
		print "<a href='#' onclick=\"caMediaPanel.showPanel('".caNavUrl($this->request, 'educational', 'Show', 'setItemInfo', array('set_item_id' => $va_item['previous_id'], 'set_id' => $vn_set_id))."'); return false;\">"._t("&lsaquo; Previous")."</a>";
	}else{
		print _t("&lsaquo; Previous");
	}
	print "&nbsp;&nbsp;|&nbsp;&nbsp;";
	if($va_item['next_id']){
		print "<a href='#' onclick=\"caMediaPanel.showPanel('".caNavUrl($this->request, 'educational', 'Show', 'setItemInfo', array('set_item_id' => $va_item['next_id'], 'set_id' => $vn_set_id))."'); return false;\">"._t("Next &rsaquo;")."</a>";
	}else{
	...
	print "</div><div style='float:right; width:230px; text-align:left'>";
	if($va_item['label']){
