...
?>
<h1><?php print _t("Take a virtual tour of Coney Island..."); ?></H1>
<div id="galleryLanding">
	<div class="textContent">
<?php
	print $this->render('cihpKiosk/intro_text_html.php');
?>
	</div>
<?php
	foreach($va_set_list as $vn_set_id => $va_set_info){
		...
		if(isset($va_set_descriptions[$vn_set_id]) && $va_set_descriptions[$vn_set_id]){
			...
			if(strlen(utf8_decode($vs_set_text)) > 130){
				$vs_set_text = substr($vs_set_text, 0, 127)."...";
			}
			print "<div class='setText'>".caNavLink($this->request, $vs_set_text, '', 'simpleGallery', 'Show', 'displaySet', array('set_id' => $vn_set_id))."</div>";	
		}
