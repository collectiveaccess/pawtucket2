<?php
/** ---------------------------------------------------------------------
 * app/plugins/Contribute/controllers/BaseSearchController.php : base controller for search interface
 * ----------------------------------------------------------------------
 * CollectiveAccess
  ...
  */
 	...
 	class BaseLookupController extends ActionController {
 		...
 		public function __construct(&$po_request, &$po_response, $pa_view_paths=null) {
 			...
 			$this->opo_plugin_config = Configuration::load($po_request->getAppConfig()->get('application_plugins').'/Share/conf/Share.conf');
 			if (!(bool)$this->opo_plugin_config->get('enabled')) { die(_t('Share plugin is not enabled')); }
 			
			if (!$po_request->isLoggedIn()) {
 				die(_t('Must be logged in to use autocompleter'));
 			}
		...
		public function Get($pa_additional_query_params=null, $pa_options=null) {
 		...
 		public function GetHierarchyAncestorList() {
 			...
 			return $this->render('lookup/'.str_replace(' ', '_', $this->ops_name_singular).'_hierarchy_ancestors_json.php');
 		}
 		# -------------------------------------------------------
 		/**
 		 *
		...
		public function IDNo() {
			...
			return $this->render('lookup/idno_json.php');
		}
		# -------------------------------------------------------
 		/**
		...
		public function Intrinsic() {
			...
			$this->view->setVar('id_list', $va_ids);
			return $this->render('lookup/intrinsic_json.php');
		}
 		# -------------------------------------------------------
 	}
