			...
			<script type='text/javascript'>
				jQuery(document).ready(function() { 
					var <?php print $vs_id_prefix; ?>oHierBrowser{n} = caUI.initHierBrowser('<?php print $vs_id_prefix; ?>_hierarchyBrowser{n}', {
						uiStyle: 'horizontal',
						levelDataUrl: '<?php print caNavUrl($this->request, 'Contribute/lookup', 'ListItem', 'GetHierarchyLevel', array('noSymbols' => 1, 'voc' => 1, 'lists' => join(';', $va_settings['restrict_to_lists']))); ?>',
						initDataUrl: '<?php print caNavUrl($this->request, 'Contribute/lookup', 'ListItem', 'GetHierarchyAncestorList'); ?>',
					...
					jQuery('#<?php print $vs_id_prefix; ?>_hierarchyBrowserSearch{n}').autocomplete(
						'<?php print caNavUrl($this->request, 'Contribute/lookup', 'ListItem', 'Get'); ?>', {minChars: 3, matchSubset: 1, matchContains: 1, delay: 800, extraParams: {noSymbols: 1}}
					);
...
<script type="text/javascript">
	...
	jQuery(document).ready(function() {
		...
		caRelationBundle<?php print $vs_id_prefix; ?> = caUI.initRelationBundle('#<?php print $vs_id_prefix.$t_item->tableNum().'_rel'; ?>', {
			...
			autocompleteUrl: '<?php print caNavUrl($this->request, 'Contribute/lookup', 'Vocabulary', 'Get', array()); ?>',
