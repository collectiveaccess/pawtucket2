			...
			<table class="caListItem">
				<tr>
					...
					<td>
						...
						<a href="<?php print urldecode(caEditorUrl($this->request, 'ca_object_lots', '{lot_id}')); ?>" class="caEditItemButton" id="<?php print $vs_id_prefix; ?>_edit_related_{n}"><?php print caNavIcon($this->request, __CA_NAV_BUTTON_GO__, null, null, array('graphicsPath' => $this->getVar('graphicsPath'))); ?></a>
					</td>
				</tr>
			</table>
		</div>
...
<?php
	if (!$vb_read_only) {
?>	
		<div class='button labelInfo caAddItemButton'><a href='#'><?php print caNavIcon($this->request, __CA_NAV_BUTTON_ADD__, null, null, array('graphicsPath' => $this->getVar('graphicsPath'))); ?> <?php print $vs_add_label ? $vs_add_label : _t("Add lot"); ?></a></div>
<?php
	}
?>
	</div>
...
<script type="text/javascript">
	...
	jQuery(document).ready(function() {
		...
		caRelationBundle<?php print $vs_id_prefix; ?> = caUI.initRelationBundle('#<?php print $vs_id_prefix.$t_item->tableNum().'_rel'; ?>', {
			...
			autocompleteUrl: '<?php print caNavUrl($this->request, 'Contribute/lookup', 'ObjectLot', 'Get', $va_lookup_params); ?>',
<?php
	if ($t_subject->tableName() == 'ca_objects') {
?>
			minRepeats: 0,
