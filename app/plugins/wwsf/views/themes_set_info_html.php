...
<?php
	foreach($va_set_list as $vn_set_id => $va_set_info){
		...
		print "<div class='setImage'>".caNavLink($this->request, $va_item["representation_tag"], '', 'wwsf', 'Themes', 'displaySet', array('set_id' => $vn_set_id))."</div><!-- end setImage -->";
		print "<div class='setTitle'>".caNavLink($this->request, (strlen($va_set_info["name"]) > 120 ? substr($va_set_info["name"], 0, 120)."..." : $va_set_info["name"]), '', 'wwsf', 'Themes', 'displaySet', array('set_id' => $vn_set_id))."</div>";
...
<?php
		if($va_item['caption'] || $va_item['representation_tag_medium']){
			...
			TooltipManager::add(
				"#item{$va_item['item_id']}", $this->render('themes_tooltip_html.php')
			);
		}
	}
