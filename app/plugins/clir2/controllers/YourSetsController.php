 ...
 */
 	...
 	class YourSetsController extends ActionController {
 		# -------------------------------------------------------
 		...
 		public function __construct(&$po_request, &$po_response, $pa_view_paths=null) {
			...
			$this->opo_plugin_config = Configuration::load($this->request->getAppConfig()->get('application_plugins').'/clir2/conf/clir2.conf');
 			...
 			$this->opo_result_context = new ResultContext($po_request, 'ca_occurrences', 'your_sets');
 		}
 		# -------------------------------------------------------
 		public function Index() {
 			...
 			$t_list = new ca_lists();
 			$vn_set_type_id = $t_list->getItemIDFromList('set_types', 'user');
 			$vn_curatorial_selection_id = $t_list->getItemIDFromList('curatorial_selection2', 'yourSets');
 			
 			$o_search = new SetSearch();
			
			$qr_featured_user_sets = $o_search->search("ca_sets.access:1 AND ca_sets.type_id:{$vn_set_type_id} AND ca_sets.curatorial_selection:".$vn_curatorial_selection_id."", array('no_cache' => true));
			$va_sets = array();
			if($qr_featured_user_sets->numHits() > 0){
				while($qr_featured_user_sets->nextHit()){
					$va_set_info = array();
					$va_set_info["set_id"] = $qr_featured_user_sets->get("set_id");
					$va_labels = array();
					$va_labels = $qr_featured_user_sets->getDisplayLabels("");
					$va_set_info["name"] = join($va_labels, ", ");
				
					$va_sets[$qr_featured_user_sets->get("set_id")] = $va_set_info;
				}
			}
 			$t_set = new ca_sets();
