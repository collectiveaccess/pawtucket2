 ...
 */
	...
	?>
			<div class="browseTargetSelect">
			<?php #print _t('Browse for').' '.$this->getVar('browse_selector'); ?>
			<form action='#'><?php print _t('Browse for').': '; ?><select name='target' id="caBrowseTargetSelectorSelect" onchange="window.location = '/index.php/Browse/Index/target/' + jQuery('#caBrowseTargetSelectorSelect').val();">
				<option value='ca_collections' <?php print ($vs_browse_target == "ca_collections") ? "selected='1'" : ""; ?>><?php print _t("Collections"); ?></option>
				<option value='ca_occurrences' <?php print ($vs_browse_target == "ca_occurrences") ? "selected='1'" : ""; ?>><?php print _t("Intellectual Content"); ?></option>
			</select>
			</form>
...
?>
	<h1 style="text-transform:capitalize;"><?php print $this->getVar('mode_type_singular'); ?></h1>
...
<?php
			if (sizeof($va_criteria)) {
				...
				if (sizeof($va_facets)) { 
?>
					<div id="refineBrowse"><span class='refineHeading'><?php print _t('Refine results by'); ?>:</span><br/>
<?php
						$vn_i = 1;
...
<?php
				...
				if(sizeof($va_criteria) != 1){
					print "<span class='criteriaHeading'>"._t("You browsed for: ")."</span>";
				...
				print caNavLink($this->request, _t('start new search')." &rsaquo;", 'startOver', 'clir2', 'Collections', 'Index', array());
