 	...
 	class BaseEditorController extends ActionController {
 		...
 		public function Edit($pa_values=null, $pa_options=null) {
			...
			if (!$t_ui->getPrimaryKey()) {
				$vs_notification_message = _t('There is no configuration available for this editor. Check your system configuration and ensure there is at least one valid configuration for this type of editor.');
			}
 		...
 		public function Save($pa_options=null) {
			...
			if(!$vn_subject_id) {
 			...
 			if(sizeof($va_errors) + sizeof($va_general_errors) > 0) {
				...
				if ($vs_message) { $vs_notification_message = $vs_message."<br/>"; }
		...
		public function getResultContext() {
 		...
 		protected function _getTypeIDForUI($t_subject) {
 			...
 			$vs_notification_message = _t('Type "%1" configured for the Cataloging form "%2" is not valid', $this->opa_ui_info['type'], $this->ops_ui_code);	
 			$this->view->setVar("notification_message", $vs_notification_message);
 			return null;
 		}
		# -------------------------------------------------------
