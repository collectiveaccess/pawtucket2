			...
			<table class="caListItem">
				<tr>
					...
					<td>
						...
						<a href="<?php print urldecode(caEditorUrl($this->request, 'ca_storage_locations', '{location_id}')); ?>" class="caEditItemButton" id="<?php print $vs_id_prefix; ?>_edit_related_{n}"><?php print caNavIcon($this->request, __CA_NAV_BUTTON_GO__, null, null, array('graphicsPath' => $this->getVar('graphicsPath'))); ?></a>
					</td>
				</tr>
			</table>
<?php
			...
			<script type='text/javascript'>
				jQuery(document).ready(function() { 
					var <?php print $vs_id_prefix; ?>oHierBrowser{n} = caUI.initHierBrowser('<?php print $vs_id_prefix; ?>_hierarchyBrowser{n}', {
						uiStyle: 'horizontal',
						levelDataUrl: '<?php print caNavUrl($this->request, 'Contribute/lookup', 'StorageLocation', 'GetHierarchyLevel', array()); ?>',
						initDataUrl: '<?php print caNavUrl($this->request, 'Contribute/lookup', 'StorageLocation', 'GetHierarchyAncestorList'); ?>',
					...
					jQuery('#<?php print $vs_id_prefix; ?>_hierarchyBrowserSearch{n}').autocomplete(
						'<?php print caNavUrl($this->request, 'Contribute/lookup', 'StorageLocation', 'Get'); ?>', {minChars: 3, matchSubset: 1, matchContains: 1, delay: 800, extraParams: {}}
					);
...
<script type="text/javascript">
	...
	jQuery(document).ready(function() {
		...
		caRelationBundle<?php print $vs_id_prefix; ?> = caUI.initRelationBundle('#<?php print $vs_id_prefix.$t_item->tableNum().'_rel'; ?>', {
			...
			autocompleteUrl: '<?php print caNavUrl($this->request, 'Contribute/lookup', 'StorageLocation', 'Get', $va_lookup_params); ?>',
			minChars:1,
