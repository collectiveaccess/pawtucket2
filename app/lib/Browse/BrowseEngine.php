...
class BrowseEngine extends BaseFindEngine {
	...
	public function __construct($pm_subject_table_name_or_num, $pn_browse_id=null, $ps_browse_context='') {
		$this->opo_db = new Db();
		parent::__construct($this->opo_db);
	...
	public function addCriteria($ps_facet_name, $pa_row_ids, $pa_display_strings=null) {
		...
		// force reset for hierarchy facet values (no accumulation)
		if(($va_facet_info['type'] ?? null) == 'hierarchy') { $va_criteria[$ps_facet_name] = []; }
	...
	public function removeCriteria($ps_facet_name, $pa_row_ids) {
		...
		foreach($pa_row_ids as $vn_row_id) {
			...
			if(is_array($va_criteria[$ps_facet_name] ?? null) && !sizeof($va_criteria[$ps_facet_name])) {
	...
	public function getCriterionLabel($ps_facet_name, $pn_row_id) {
		...
		switch($va_facet_info['type']) {
			...
			case 'hierarchy':
				...
				return $t_table->get($this->ops_browse_table_name.".hierarchy.preferred_labels", ['delimiter' => ' ➜ ']);
				break;
			# -----------------------------------------------------
			case 'has':
			...
			case 'field':
				...
				$rels = Datamodel::getManyToOneRelations($this->ops_browse_table_name, $va_facet_info['field']);
				if(is_array($rels) && sizeof($rels) && ($va_facet_info['template'] ?? null)) {
					$t_rel = Datamodel::getInstance($rels['one_table'], false, $pn_row_id);
					$fields = caExtractTagsFromTemplate($va_facet_info['template']);
					
					$acc = [];
					foreach($fields as $k => $f) {
						if($t_rel->hasField($f)) { $acc[$f] = $t_rel->get($f); }
					}
					return caProcessTemplate($va_facet_info['template'], $acc);
				}
			...
			case 'current_value':
				...
				if (isset($va_collapse_map[$table][$va_row_tmp[1]])) {
					...
					return $va_collapse_map[$table][$va_row_tmp[1]];
				} elseif(isset($va_collapse_map[$table]['*'])) {
					// Class is collapsable
					return $va_collapse_map[$table]['*'];
			...
			case 'normalizedWeight':
				...
				if (!($vs_output_units = caGetWeightUnitType($vs_units=caGetOption('units', $va_facet_info, 'g')))) {
					$vs_output_units = Zend_Measure_Weight::GRAM;
				}
	...
	public function execute($pa_options=null) {
		...
		if (is_array($va_criteria) && (sizeof($va_criteria) > 0)) {
			if (!$vb_results_cached) {
				...
				foreach($va_criteria as $vs_facet_name => $va_row_ids) {
					...
					$va_sql_params = [];
						...
						switch($va_facet_info['type']) {
							# -----------------------------------------------------
							case 'hierarchy':
								$children = array_merge($va_row_ids, $vs_target_browse_table_name::getHierarchyChildrenForIDs($va_row_ids, ['includeSelf' => true, 'maxLevels' => ($va_facet_info['restrict_to_top_level'] ?? false) ? 1 : null]));
								...
								foreach($va_row_ids as $vn_row_id) {
									...
									if ($browse_by_first_letter) {
										...
										if(preg_match("!^[^a-z]+$!", $letter)) {
											$twheres[] = "{$vs_label_table_name}.{$vs_label_sort_field} RLIKE '^[^A-Za-z]'";
										} else {
											$twheres[] = "{$vs_label_table_name}.{$vs_label_sort_field} LIKE ?";
											$tparams[] = trim($vn_row_id)."%";
										}
									} else {
										$twheres[] = "{$vs_label_table_name}.{$vs_label_display_field} = ?";
										$tparams[] = trim($va_labels[$vn_row_id] ?? null);
									}	
									...
									$vs_sql = "
										...
										WHERE
											".join(" AND ", $twheres);
									$qr_res = $this->opo_db->query($vs_sql, $tparams);
				...
				if (sizeof($va_res)) {
					...
					if (caACLIsEnabled($t_item, $pa_options)) {
		...
		if ($vb_need_to_cache_facets && !($pa_options['dontCheckFacetAvailability'] ?? false)) {
			$this->loadFacetContent($pa_options);
		}
		if ($vb_need_to_save_in_cache) {
			$this->opo_ca_browse_cache->setParameter('filterDeaccessionedRecords', (bool)($pa_options['filterDeaccessionedRecords'] ?? false) ? 1 : 0);
	...
	public function getFacetWithGroups($ps_facet_name, $ps_group_mode, $ps_grouping_field=null, $pa_options=null){
		...
		$va_types = null;
		if(($t_model = Datamodel::getInstance($va_facet_info['table'])) && (method_exists($t_model, "getTypeList"))) { 
	...
	public function getFacetContent($ps_facet_name, $pa_options=null) {
		...
		if (($va_browse_type_ids = $this->getTypeRestrictionList()) && is_array($va_browse_type_ids) && sizeof($va_browse_type_ids)) {		// type restrictions
			...
			if (is_array($va_facet_info['type_restrictions'] ?? null)) { 		// facet type restrictions bind a facet to specific types; we check them here
		...
		if (($va_browse_source_ids = $this->getSourceRestrictionList()) && is_array($va_browse_source_ids) && sizeof($va_browse_source_ids)) {		// source restrictions
			...
			if (is_array($va_facet_info['source_restrictions'] ?? null)) { 		// facet source restrictions bind a facet to specific sources; we check them here
		...
		switch($va_facet_info['type']) {
			...
			case 'has':
				...
				if ($va_facet_info['element_code'] ?? null) {
					...
					foreach($va_facet_values as $vs_state_name => $va_state_info) {
						...
						if (($t_item = Datamodel::getInstanceByTableName($vs_browse_table_name, true)) && caACLIsEnabled($t_item, $pa_options)) {
			...
			case 'label':
				...
				if (is_array($va_results ?? null) && sizeof($va_results)) {
					if ($va_facet_info['relative_to'] ?? null) {
			...
			case 'hierarchy':
				...
				$parent_ids = [];
				if(is_array($va_results) && sizeof($va_results)) {
					$hier_item_ids = array_unique($va_results);
					...
					while(true) {
						$vs_sql = "
							...
							WHERE 
								{$vs_item_pk} IN (?)
						";
						if(!($qr_res = $this->opo_db->query($vs_sql, [$hier_item_ids]))) {
							break;
						}
						$hier_item_ids = $qr_res->getAllFieldValues('parent_id');
						if((sizeof($hier_item_ids) <= 1) && (!isset($hier_item_ids[0]) || ($hier_item_ids[0] === null))) {
							break;
						}
						$parent_ids = array_unique(array_merge($parent_ids, $hier_item_ids));
					}
					
					if(sizeof($parent_ids)) {
						$va_where_sql[] = "(t.{$vs_item_pk} IN (?))";
						$params[] = $parent_ids;
					}
				} else {
					return false;
				...
				if (($va_browse_type_ids = $this->getTypeRestrictionList()) && is_array($va_browse_type_ids) && sizeof($va_browse_type_ids)) {		// type restrictions
					...
					$va_where_sql[] = "(
						(t.{$type_fld_name} IN (".join(', ', $va_browse_type_ids).')'.($t_subject->getFieldInfo('type_id', 'IS_NULL') ? " OR (t.{$type_fld_name} IS NULL)" : '').")
					)";
				}
				...
				if (($va_browse_source_ids = $this->getSourceRestrictionList()) && is_array($va_browse_source_ids) && sizeof($va_browse_source_ids)) {		// source restrictions
					...
					$va_where_sql[] = "(
						(t.{$source_fld_name} IN (".join(', ', $va_browse_source_ids).')'.($t_subject->getFieldInfo('source_id', 'IS_NULL') ? " OR (t.{$source_fld_name} IS NULL)" : '').")
					)";
				}

				if (isset($pa_options['checkAccess']) && is_array($pa_options['checkAccess']) && sizeof($pa_options['checkAccess']) && $t_item->hasField('access')) {
					$va_where_sql[] = "(t.access IN (".join(',', $pa_options['checkAccess'])."))";
				}

				if ($t_item->hasField('deleted')) {
					$va_where_sql[] = "(t.deleted = 0)";
				}

				if (is_array($va_restrict_to_types) && sizeof($va_restrict_to_types)) {
					$va_restrict_to_type_ids = caMakeTypeIDList($vs_browse_table_name, $va_restrict_to_types, array('dont_include_subtypes_in_type_restriction' => true));
					if (is_array($va_restrict_to_type_ids) && sizeof($va_restrict_to_type_ids)) {
						$va_where_sql[] = "(t.".$t_item->getTypeFieldName()." IN (".join(", ", $va_restrict_to_type_ids).")".($t_item->getFieldInfo('type_id', 'IS_NULL') ? " OR (t.".$t_item->getTypeFieldName()." IS NULL)" : '').")";
					}
				}
				if (is_array($va_exclude_types) && sizeof($va_exclude_types)) {
					$va_exclude_type_ids = caMakeTypeIDList($vs_browse_table_name, $va_exclude_types, array('dont_include_subtypes_in_type_restriction' => true));
					if (is_array($va_exclude_type_ids) && sizeof($va_exclude_type_ids)) {
						$va_where_sql[] = "(t.".$t_item->getTypeFieldName()." NOT IN (".join(", ", $va_exclude_type_ids).")".($t_item->getFieldInfo('type_id', 'IS_NULL') ? " OR (t.".$t_item->getTypeFieldName()." IS NULL)" : '').")";
					}
				}
				...
				if ($vb_check_availability_only) {
					...
					$qr = caMakeSearchResult($vs_browse_table_name, $qr_res->getAllFieldValues($vs_item_pk));
					while($qr->nextHit()) {
						$va_values[$id=$qr->getPrimaryKey()] = [
							'id' => $id,
							'parent_id' => $qr->get("{$vs_browse_table_name}.parent_id"),
							'label' => $qr->get("{$vs_browse_table_name}.hierarchy.preferred_labels", ['delimiter' => ' ➜ ']),
							'sort_label' =>  $qr->get("{$vs_browse_table_name}.preferred_labels.name_sort"),
							'content_count' => null
						];
					   
						if (!is_null($vs_single_value) && ($id == $vs_single_value)) {
							$vb_single_value_is_present = true;
						}
					}
					...
					switch($vn_element_type) {
						case __CA_ATTRIBUTE_VALUE_LIST__:
							...
							if (!caGetOption(['dontExpandHierarchically', 'dont_expand_hierarchically'], $va_facet_info, false)) {
								...
								$qr_res->seek(0);
								$va_ids = $qr_res->getAllFieldValues('item_id');
			...
			case 'current_value':
				...
				if ($vb_check_availability_only) {
					...
					$va_values = $values_by_table = array();
					while($qr_res->nextRow()) {
						if (!($current_table_num = trim($qr_res->get('current_table_num')))) { continue; }
						if (!($current_row_id = trim($qr_res->get('current_row_id')))) { continue; }
						$current_type_id = trim($qr_res->get('current_type_id'));
						//if (!($current_type_id = trim($qr_res->get('current_type_id')))) { continue; }
						$vs_val = "{$current_table_num}:{$current_type_id}:{$current_row_id}";
						if ($va_criteria[$vs_val]) { continue; }		// skip items that are used as browse critera - don't want to browse on something you're already browsing on

						$values_by_table[$current_table_num][$current_type_id][] = $qr_res->getRow();
					}


					foreach($values_by_table as $current_table_num => $current_row_ids_by_type) {
						foreach($current_row_ids_by_type as $current_type_id => $current_row_info) {
							$current_row_ids = array_map(function($v) { return $v['current_row_id'];  }, $current_row_info);
							if(!sizeof($current_row_ids)) { continue; }
							$current_table_name  = Datamodel::getTableName($current_table_num);
							...
							$va_config = ca_objects::getConfigurationForHistoryTrackingCurrentValue($policy, $current_table_name, $current_type_id, array('facet' => isset($va_facet_info['display']) ? $va_facet_info['display'] : null));

							if ($current_table_name == 'ca_storage_locations') {
							...
							while($qr_res->nextHit()) {
								...
								if (isset($va_collapse_map[$current_table_name]) && isset($va_collapse_map[$current_table_name][$type_id]) && $va_collapse_map[$current_table_name][$type_id]) {
										if (!($vs_label = $va_collapse_map[$current_table_name][$type_id])) { continue; }
										$va_values[$vs_id = "{$current_table_num}:{$type_id}"] = array(
											'id' => $vs_id,
											'label' => $vs_label
										);
										continue;
			...
			case 'field':
				...
				$rels = Datamodel::getManyToOneRelations($vs_browse_table_name, $vs_field_name);
				...
				$many_one_flds = null;
				if(is_array($rels) && sizeof($rels) && ($va_facet_info['template'] ?? null)) {
					$va_joins[] = "INNER JOIN {$rels['one_table']} as x ON x.{$rels['one_table_field']} = {$rels['many_table']}.{$rels['many_table_field']}";
					
					$t_rel = Datamodel::getInstance($rels['one_table']);
					$fields = caExtractTagsFromTemplate($va_facet_info['template']);
					foreach($fields as $k => $f) {
						if(!$t_rel->hasField($f)) { unset($fields[$k]); }
					}
				...
				if ($vb_check_availability_only) {
					...
					$vs_pk = $t_item->primaryKey();
					$vs_sql = "
						SELECT COUNT(*) _count, {$vs_browse_table_name}.{$vs_field_name}".($many_one_flds ? ", {$many_one_flds}" : '')."
						FROM {$vs_browse_table_name}
						{$vs_join_sql}
						WHERE
							{$vs_where_sql}
					...
					while($qr_res->nextRow()) {
						...
						} elseif($va_facet_info['template'] ?? null) {
							
							$va_values[$vs_val] = array(
								'id' => str_replace('/', '&#47;', $vs_val),
								'label' => caProcessTemplate($va_facet_info['template'], $qr_res->getRow()),
								'content_count' => $qr_res->get('_count')
							);
						} else {
							$va_values[$vs_val] = array(
...
if (!($va_facet_info['show_all_when_first_facet'] ?? null) || ($this->numCriteria() > 0)) {
				if ($browse_on_self_relation) {
...
// 								}
	...
	public function addResultFilter($ps_field, $ps_operator, $pm_value) {
		...
		$t_table = Datamodel::getInstanceByTableName($this->ops_browse_table_name, true);
	...
	public function getTypeRestrictionList() {
		if (function_exists("caGetTypeRestrictionsForUser")) {
			$va_pervasive_types = caGetTypeRestrictionsForUser($this->ops_browse_table_name);	// restrictions set in app.conf or by associated user role
	...
	public function getSourceRestrictionList() {
		if (function_exists("caGetSourceRestrictionsForUser")) {
			$va_pervasive_sources = caGetSourceRestrictionsForUser($this->ops_browse_table_name);	// restrictions set in app.conf or by associated user role
...
// 								break;
	...
	private function getTableEntryFromMap($facet_info, $key, $table, $type=null) {
		if(is_array($facet_info[$key][$table] ?? null)) {
			if($type && is_array($facet_info[$key][$table][$type] ?? null)) {
				return $facet_info[$key][$table][$type];
			} elseif (is_array($facet_info[$key][$table]['*'] ?? null)) {
				return $facet_info[$key][$table]['*'];
			} elseif (is_array($facet_info[$key][$table]['__default__'] ?? null)) {
