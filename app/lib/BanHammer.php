...
class BanHammer {
	...
	static public function getPluginNames() {
		...
		self::$plugin_names = array_reduce(self::$plugin_names, function($c,$v) { return array_merge($c, array_values($v)); }, []);
	...
	static public function verdict($request, $options=null) {
		...
		if (!in_array($mode, ['absolute', 'average'])) { $mode = 'absolute'; }
		$threshold = (float)(self::$config->get('threshold'));
		if (($threshold < 0) || ($threshold > 1)) { $threshold = 1.0; }
		...
		foreach($plugin_names as $p) {
			...
			if ($prob > 0) { 
				$non_zero_acc += $prob; 
				$non_zero_count++; 
				if (($ttl = $classname::banTTL()) && ($ttl > $max_ttl)) { $max_ttl = $ttl; }
				if ($classname::shouldBanIP()) { $should_ban_ip = true; }
				$non_zero_plugins[] = $p;
			}
		}
