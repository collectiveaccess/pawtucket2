			...
			if ($o_view = BaseMediaViewer::getView($po_request)) {
				...
				if (is_a($t_instance, "ca_object_representations")) {
					$va_viewer_opts = [
						...
						'annotation_load_url' => caNavUrl('*', '*', 'GetAnnotations', array('representation_id' => (int)$t_instance->getPrimaryKey(), $t_subject->primaryKey() => (int)$t_subject->getPrimaryKey())),
						'annotation_save_url' => caNavUrl('*', '*', 'SaveAnnotations', array('representation_id' => (int)$t_instance->getPrimaryKey(), $t_subject->primaryKey() => (int)$t_subject->getPrimaryKey())),
						'download_url' => caNavUrl('*', '*', 'DownloadMedia', array('representation_id' => (int)$t_instance->getPrimaryKey(), $t_subject->primaryKey() => (int)$t_subject->getPrimaryKey(), 'version' => 'original')),
						'help_load_url' => caNavUrl('*', '*', 'ViewerHelp', array()),
						'annotationEditorPanel' => 'caRepresentationAnnotationEditor',
						'read_only' => !$po_request->isLoggedIn(),
						'annotationEditorUrl' => caNavUrl('editor/representation_annotations', 'RepresentationAnnotationQuickAdd', 'Form', array('representation_id' => (int)$t_instance->getPrimaryKey())),
					...
					$va_viewer_opts = [
						...
						'viewer_width' => caGetOption('viewer_width', $pa_data['display'], '100%'), 'viewer_height' => caGetOption('viewer_height', $pa_data['display'], '100%'),
						'viewer_base_url' => $po_request->getBaseUrlPath(),
						'download_url' => caNavUrl('*', '*', 'DownloadMedia', array('media_id' => (int)$t_instance->getPrimaryKey(), $t_subject->primaryKey() => (int)$t_subject->getPrimaryKey(), 'version' => 'original')),
						'help_load_url' => caNavUrl('*', '*', 'ViewerHelp', array())
					];
					...
					$va_viewer_opts = [
						...
						'read_only' => true,
						'viewer_base_url' => $po_request->getBaseUrlPath(),
						'download_url' => caNavUrl('*', '*', 'DownloadMedia', array('value_id' => (int)$t_instance->getPrimaryKey(), $t_subject->primaryKey() => (int)$t_subject->getPrimaryKey(), 'version' => 'original')),
						'help_load_url' => caNavUrl('*', '*', 'ViewerHelp', array()),
						'read_only' => !$po_request->isLoggedIn(),
