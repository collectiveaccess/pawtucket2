...
class Mirador extends BaseMediaViewer implements IMediaViewer {
		...
		if ($o_view = BaseMediaViewer::getView($request)) {
			...
			if ($data['t_subject']) { 
				...
				if ($data['t_instance']) {
					...
					if (is_array($rep_ids = Mirador::getViewerRepresentationIDList($request, $identifier, $data, $options))) {
	...
	public static function getViewerRepresentationIDList($request, $identifier, $data=null, $options=null) {
		if ($t_instance = caGetOption('t_instance', $data, null)) {
			...
			$use_mirador_for_image_list_length = caGetOption('use_mirador_for_image_list_length_at_least', $data['display'], null);
			...
			} elseif (is_a($t_instance, "ca_object_representations") && $data['t_subject'] && $use_mirador_for_image_list_length && ($reps = $data['t_subject']->getRepresentations(['small'], null, [])) && (sizeof($reps) >= $use_mirador_for_image_list_length)) {
	...
	public static function getViewerData($request, $identifier, $data=null, $options=null) {
		...
		if ($o_view = BaseMediaViewer::getView($request)) {
			if ($t_instance = caGetOption('t_instance', $data, null)) {
				...
				$o_view->setVar('id', 'caMediaOverlayMirador_'.$t_instance->getPrimaryKey().'_'.($display_type = caGetOption('display_type', $data, caGetOption('display_version', $data['display'], ''))));
			
				$use_mirador_for_image_list_length = caGetOption('use_mirador_for_image_list_length_at_least', $data['display'], null);
				if (((($display_version = caGetOption('display_version', $data['display'], 'tilepic')) == 'tilepic')) && !$use_mirador_for_image_list_length) {
					$data['resources'] = $t_instance->getFileList(null, null, null, [$display_version, 'preview']);
				} elseif (is_a($t_instance, "ca_object_representations") && $data['t_subject'] && $use_mirador_for_image_list_length && ($reps = $data['t_subject']->getRepresentations(['small', $display_version, 'original'], null, []))) {
					...
					if(sizeof($reps) < $use_mirador_for_image_list_length) {
						$data['resources'][] = [
