...
class BaseModelWithAttributes extends BaseModel implements ITakesAttributes {
	...
	public function getDisplayLabel($ps_field, $options=null) {
		...
		if ($va_tmp[0] == $this->tableName()) {
			...
			if (!$this->hasField($va_tmp[1]) && !in_array($va_tmp[1], array('created', 'modified', 'lastModified')) && !in_array($va_tmp[0], array('created', 'modified', 'lastModified'))) {
				...
				return $this->getAttributeLabel((sizeof($va_tmp) > 2) ? $va_tmp[2] : $va_tmp[1], $options);	
			}
		}
		...
		if (in_array($va_tmp[1], array('preferred_labels', 'nonpreferred_labels'))) {
			...
			if ($va_tmp[0] == $this->tableName() && $vs_autocomplete) {
				...
				if (!caGetOption('nojs', $pa_options, false)) {	
					...
					return caHTMLTextInput(caGetOption('name', $pa_options, $ps_field).$vs_rel_types.$vs_autocomplete, array('value' => $pa_options['values'][$ps_field], 'placeholder' => $pa_options['placeholder'] ?? null, 'class' => $pa_options['class'], 'id' => caGetOption('id', $pa_options, str_replace('.', '_', caGetOption('name', $pa_options, $ps_field))).$vs_autocomplete), $pa_options);
				}
			} else {
				return caHTMLTextInput(caGetOption('name', $pa_options, $ps_field).$vs_rel_types.$vs_autocomplete, array('value' => $pa_options['values'][$ps_field], 'placeholder' => $pa_options['placeholder'] ?? null, 'class' => $pa_options['class'], 'id' => caGetOption('id', $pa_options, str_replace('.', '_', caGetOption('name', $pa_options, $ps_field)))), $pa_options);
			}
		}
	...
	public function getAuthorityElementReferences($pa_options=null) {
		...
		while($qr_res->nextRow()) {
			$va_row = $qr_res->getRow();
			if(!is_array($va_references[$va_row['table_num']][$va_row['row_id']] ?? null)) { $va_references[$va_row['table_num']][$va_row['row_id']] = []; }
			if(!in_array($va_row['element_id'], $va_references[$va_row['table_num']][$va_row['row_id']] ?? null)) { $va_references[$va_row['table_num']][$va_row['row_id']][] = $va_row['element_id']; }
		}
