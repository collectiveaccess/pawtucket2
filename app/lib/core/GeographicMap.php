	...
	# -------------------------------------------------------------------
 	...
 	public function mapFrom($po_data_object, $ps_georeference_field_name, $pa_options=null) {
		$po_request = (isset($pa_options['request']) && $pa_options['request']) ? $pa_options['request'] : null;
 		...
 		$vs_table_name = array_shift($va_tmp);
 		...
 		if (is_subclass_of($po_data_object, 'BaseModel')) {
 			...
 			if (is_array($va_coordinates = $po_data_object->get($ps_georeference_field_name.".$vs_field_name", array('coordinates' => true, 'returnAsArray' => true)))) {
				if ($vs_table_name == $po_data_object->tableName()) {
					$va_coordinates = array($va_coordinates);
				}
				foreach($va_coordinates as $vn_i => $va_coordinate_list) {
					foreach($va_coordinate_list as $vn_i => $va_geoname) {
