...
class AssetLoadManager {
	...
	static function register($ps_package, $ps_library=null, $pn_priority=10, $output_target='header') {
		...
		if ($ps_library) {
			...
			if (isset($va_list[$ps_library]) && $va_list[$ps_library]) {
				if($ps_library === '__importmap__') {
					foreach($va_list[$ps_library] as $k => $d) {
						$g_asset_import_map[$k] = "{$ps_package}/{$d}";
					}
					return true;
				} elseif(is_array($va_list[$ps_library])) {
					return false;;
				}
	...
	static function getLoadHTML($po_request, $pa_options=null) {
		...
		if(is_array($g_asset_import_map) && sizeof($g_asset_import_map) && ($output_target === 'header')) {
