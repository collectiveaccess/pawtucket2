 ...
 *
 */
function caGetBrowseForType($table, $type=null, ?array $options=null) : ?string {
	$browse_types = caGetBrowseConfig()->getAssoc('browseTypes');
	
	$default = null;
	foreach($browse_types as $k => $browse_info) {
		if($browse_info['table'] !== $table) { continue; }
		
		if(!($type_restrictions = caGetOption('restrictToTypes', $browse_info, null))) {
			$default = $k;
			continue; 
		}
		
		if($type && in_array($type, $type_restrictions)) {
			return $k;
		}
	}
