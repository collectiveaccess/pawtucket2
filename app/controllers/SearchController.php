...
class SearchController extends FindController {
	...
	public function __call($function, $args) {
		...
		if (($o_browse->numCriteria() <= $num_base_criteria) && $search_expression) {
		...
		foreach($criteria as $facet_name => $criterion) {
			...
			foreach($criterion as $criterion_id => $criterion) {
				$criteria_for_display[] = array('facet' => $facet_info['label_singular'], 'facet_name' => $facet_name, 'value' => $this->purifier->purify($criterion), 'id' => $criterion_id, 'hide' => $facet_info['hide'] ?? false);
			}
		}
