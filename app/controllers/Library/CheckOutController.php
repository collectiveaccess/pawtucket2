...
class CheckOutController extends ActionController {
	...
	public function GetObjectInfo() {
		...
		switch($vn_status = $t_object->getCheckoutStatus()) {
			...
			case __CA_OBJECTS_CHECKOUT_STATUS_OUT__:
				...
				$vs_status_display = ($vn_current_user_id == $user_id) ? _t('Out with this user') : _t('Out');
		...
		$vb_is_held_by_current_user = ($user_id == $vn_current_user_id);
		
		if (is_array($va_reservations)) {
			$va_tmp = array();
			foreach($va_reservations as $va_reservation) {
				$vb_is_reserved_by_current_user = ($va_reservation['user_id'] == $user_id);
		...
		$va_info = array(
			...
			'isOutWithCurrentUser' => ($user_id == $vn_current_user_id),
	...
	public function SaveTransaction() {
		...
		$va_ret = array('status' => 'OK', 'total' => sizeof($item_list), 'errors' => array(), 'checkouts' => array());
			
		if(is_array($item_list)) {
			$t_object = new ca_objects();
			foreach($item_list as $vn_i => $va_item) {
				...
				if ($va_checkout_info = $t_checkout->objectIsOut($va_item['object_id'])) {
					if ($va_checkout_info['user_id'] == $user_id) {
						// user already has item so skip it
						$va_ret['errors'][$va_item['object_id']] = _t('User already has <em>%1</em>', $vs_name);
						continue;
					}
					try {
						$vb_res = $t_checkout->reserve($va_item['object_id'], $user_id, $va_item['note'], array('request' => $this->request));
						if ($vb_res) {
					...
					try {
						$vb_res = $t_checkout->checkout($va_item['object_id'], $user_id, $va_item['note'], $va_item['due_date'], array('request' => $this->request));
