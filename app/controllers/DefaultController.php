...
class DefaultController extends BasePawtucketController {
	...
	public function __construct($request, $response, $view_paths=null) {
		...
		caSetPageCSSClasses(['staticPage']);
	}
	# -------------------------------------------------------
	/**
	 * Try to render page using url path, checking for dynamic "site page" managed content first, and
	 * then falling back to rendering a static view in the current theme.
	 *
	 * @param string $method
	 * @param array $path
	 *
	...
	public function __call(string $method, array $path) {
		...
		if(!($content = ca_site_pages::renderPageForPath($this, "/".trim(join("/", $path[0]), "/"), [
