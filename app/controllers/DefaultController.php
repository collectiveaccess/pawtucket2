...
class DefaultController extends BasePawtucketController {
	...
	public function __call($method, $path) {
		...
		if(!($content = ca_site_pages::renderPageForPath($this, $path = "/".trim(join("/", $path[0]), "/"), ['incrementViewCount' => true, 'checkAccess' => caGetUserAccessValues()]))) {
			if($path[0][sizeof($path[0])-1] === '_default') {
				$def_path = $path[0];
				array_pop($def_path);
				$content = $content = ca_site_pages::renderPageForPath($this, $path = "/".trim(join("/", $def_path), "/"), ['incrementViewCount' => true, 'checkAccess' => caGetUserAccessValues()]);
			}
		}
		if ($content) {
